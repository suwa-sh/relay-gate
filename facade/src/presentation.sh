...
# 終了コードは tier-facade.md「終了コード」表に従う(0 正常 / 1 業務エラー / 2 バリデーション / 124 RDB タイムアウト)。
...
# timeout_error は RDB 接続タイムアウト(終了コード 124)で終了する。
timeout_error() {
  printf '%s\n' "$1" >&2
  exit 124
}
...
# validate_select_slot_environment は feature flag と監査 actor をまとめて検証する(LP-001: 全パラメータをこの時点で検証)。
# 違反は全件を標準エラーへ出してから終了コード 2 で終了する。
