...
const meta: Meta = {
  title: 'Pages/運用ポータル/blue background中止依頼画面',
  tags: ['autodocs'],
  parameters: {
    layout: 'fullscreen',
  },
    ...
    <OpsPortalShell activeNavKey="control">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        ...
        <Banner variant="info" title="中止依頼を受理しました">
          run_id=run-20260817-blue-014 の中止依頼を受理しました。続けて対話確認画面（confirmコマンド）でABORTEDへの遷移を確定してください。
        </Banner>
      </div>
    </OpsPortalShell>
  ),
...
export const ValidationError: Story = {
  render: () => (
    <OpsPortalShell activeNavKey="control">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        <RunnerResultPanel
          runId="run-20260817-blue-009"
          slot="blue"
          role="background"
          startedAt="2026-08-17T07:30:02+09:00"
          stdout="INFO job started\nINFO job succeeded"
          stderr=""
          exitCode={0}
        />
        ...
        <Banner variant="error" title="中止依頼を拒否しました">
          対象のblue background実行（run_id=run-20260817-blue-009）は状態がSUCCEEDEDです。RUNNING中の実行のみ中止依頼を受け付けます（exit code 1）。
        </Banner>
      </div>
    </OpsPortalShell>
  ),
...
/**
 * CLI単体運用時のヘッドレス表示。relaygate abort blue request コマンドの標準出力を再現する。
 */
export const Headless: Story = {
  render: () => (
    <OpsPortalShell headless>
      <TerminalPanel title="$ relaygate abort blue request --run-id=run-20260817-blue-014">
        {'中止依頼受理: run-20260817-blue-014\n次アクション: relaygate abort blue confirm --run-id=run-20260817-blue-014'}
      </TerminalPanel>
    </OpsPortalShell>
  ),
}
