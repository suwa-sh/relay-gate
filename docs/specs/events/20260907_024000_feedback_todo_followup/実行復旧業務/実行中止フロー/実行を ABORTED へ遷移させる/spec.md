...
graph LR
  ...
  subgraph DB["内部データストア: ジョブキュー兼管理 DB(RDB)"]
    ...
    DB_RR[("rapid_crosscheck_requests\nstatus RUNNING -> ABORTED(abort-rapid-crosscheck)\nstatus REQUESTED -> ABORTED(abort-blue / abort-green。off 以外)")]
  ...
  OPS_GW -->|"SQL: UPDATE ... WHERE run_id = ? AND status = 'RUNNING'(abort-rapid-crosscheck)"| DB_RR
...
| ops repository / gateway(slot) | 成果物再確認 → `aborted.txt` を `.tmp` → `mv` で公開 → (off 以外)`slot_executions` の条件付き UPDATE + `parallel_runs` の無条件行ロック(`SELECT 1 ... FOR UPDATE`)+ `parallel_runs` の UPDATE(`status IN ('STARTED','RUNNING')` のみ)+ `rapid_crosscheck_requests` の条件付き UPDATE(`status = 'REQUESTED'` のみ。未着手の速報比較依頼を ABORTED、completed_at=now) | ファイル正本の中止が成立すれば成功。slot_executions の UPDATE 0 件は `WARN management db not updated` を残して成功(0)、rapid_crosscheck_requests の UPDATE 0 件は正常(依頼が無い、または CLAIMED / RUNNING / 終端済み。ログなし)、DB 接続・SQL 失敗は 6(aborted.txt は残す) |
...
| ops usecase | 実行ログ行(`operator= answer= run_id= role= from=RUNNING to=ABORTED`。未着手の速報比較依頼を ABORTED にしたときは加えて `INFO rapid request aborted run_id=... from=REQUESTED to=ABORTED operator=... answer=yes|yes(--yes)`(answer の表記は `status changed` 行と同じ規則)。0 件のときは出さない) | 監査用の記録(CLP-008) |
...
| 中止対象種別 | 速報比較依頼 | `rapid_crosscheck_requests` を `WHERE run_id = ? AND status = 'RUNNING'` で ABORTED(CLAIMED / RUNNING の依頼の中止経路は本スクリプトのみ) | tier-ops | abort-rapid-crosscheck.sh |
...
| クロスチェック依頼状態 | RUNNING | 中止可(abort-rapid-crosscheck / abort-final-crosscheck) | tier-ops | domain `is_request_abortable` |
| クロスチェック依頼状態 | REQUESTED / CLAIMED / SUCCEEDED / FAILED / ABORTED | abort-rapid-crosscheck / abort-final-crosscheck では中止不可(比較未開始または終端済み)。終了コード 3 | tier-ops | domain `is_request_abortable` |
| クロスチェック依頼状態 | REQUESTED(速報のみ) | abort-blue / abort-green の slot 中止に伴い、対象 run の未着手依頼を条件付き UPDATE で ABORTED にする(off 以外。0 件で可。依頼の状態は stdout に出さない) | tier-ops | usecase(slot 系)/ gateway の UPDATE WHERE 句 |
...
|--------|----------|----------|---------|-------------|
| slot 中止可否判定 | 縦軸 mode(foreground / background / off)× 横軸 status(RUNNING / SUCCEEDED / FAILED / ABORTED)。`background × RUNNING` のみ可。他は状態を変更せず終了コード 3。中止の記録は `aborted.txt`(ファイル正本)で行い、RAPID_CROSSCHECK_MODE が off 以外なら管理 DB の状態も ABORTED に更新する。off でも成果物ファイルだけで中止が成立する。yes で slot を ABORTED にするとき、対象 run に REQUESTED で未着手(worker が claim していない)の速報比較依頼があればそれも ABORTED にする(off 以外。条件付き UPDATE `status = 'REQUESTED'`。0 件は正常)。この REQUESTED → ABORTED は競合窓の保険である: REQUESTED の依頼は両 slot の exitcode.txt 公開後にしか存在せず、その時点で本スクリプトは「exitcode.txt あり = 終端済み」で 3 を返すため、到達するのは成果物再確認と runner の exitcode.txt 公開・dispatcher の依頼作成が同時刻に起きた競合ケースだけ(dispatcher は parallel_runs を FOR UPDATE で読み、本スクリプトも parallel_runs の行を無条件に `SELECT 1 ... FOR UPDATE` でロックしてから条件付き UPDATE に進むため、parallel_runs が COMPLETED でも両者は直列化され、dispatcher が先に COMMIT した依頼を本 UPDATE が拾う。ロック順は slot_executions → parallel_runs(FOR UPDATE)→ parallel_runs 条件付き UPDATE → rapid_crosscheck_requests 条件付き UPDATE)。両 slot 完了直後の依頼を止める本来の経路は dispatcher の判定(UC「両系成功時に速報比較依頼を作成する」の条件「中止済み run の比較依頼作成除外」。parallel_runs ABORTED または slot_executions に ABORTED あり)である(rdra-feedback #14)。CLAIMED / RUNNING の依頼は変更せず abort-rapid-crosscheck の対象とする。ただし CLAIMED のまま中止した依頼は worker の lease 失効で REQUESTED に戻り再 claim されうる(abort-rapid-crosscheck は RUNNING 限定のため止める経路が無い。止めるには abort-rapid-crosscheck の対象拡張が必要。RDRA 変更要望: rdra-feedback #15) | tier-ops | domain `is_slot_abortable`、gateway のファイル書き込みと UPDATE WHERE 句 | background かつ RUNNING の green を中止する / foreground の blue は中止できない / RAPID_CROSSCHECK_MODE=off でも aborted.txt を書いて中止が成立する / REQUESTED の速報比較依頼がある run を中止すると依頼も ABORTED になる / CLAIMED または RUNNING の速報比較依頼は slot の中止では変更されない |
...
| 依頼状態遷移規則 | 停止確認後の中止で ABORTED に遷移する。本 UC の遷移は RUNNING → ABORTED(abort-rapid-crosscheck / abort-final-crosscheck)と REQUESTED → ABORTED(abort-blue / abort-green による未着手の速報比較依頼の中止のみ)。CLAIMED からの遷移は無い | tier-ops | domain `is_request_abortable`、usecase(slot 系)の依頼 UPDATE | RUNNING の速報比較依頼を中止する / REQUESTED の速報比較依頼がある run を中止すると依頼も ABORTED になる |
...
| 状態遷移の表示 | 旧状態・新状態 | 実行ログに `from=RUNNING to=ABORTED`(slot / 依頼)。未着手の速報比較依頼を併せて ABORTED にしたときは `from=REQUESTED to=ABORTED` | 実行ログ | tier-ops |
...
| クロスチェック依頼(速報) | REQUESTED | ABORTED | abort-blue.sh / abort-green.sh に yes と応答(slot の中止に伴う。速報比較依頼のみ。確報比較依頼には適用しない) | 対象 run に status = REQUESTED(worker が claim していない未着手)の速報比較依頼がある。RAPID_CROSSCHECK_MODE が off 以外 | 条件付き UPDATE `WHERE run_id = ? AND status = 'REQUESTED'` で completed_at 記録。0 件は正常(依頼なし / CLAIMED / RUNNING / 終端済み)。CLAIMED / RUNNING の依頼は変更せず abort-rapid-crosscheck の対象のまま(CLAIMED のまま中止した依頼は lease 失効で REQUESTED に戻り再 claim されうる。rdra-feedback #15)。本遷移は競合窓の保険(成果物再確認と exitcode.txt 公開・dispatcher の依頼作成が同時刻に起きたケース)であり、両 slot 完了直後の依頼を止める本来の経路は dispatcher の条件「中止済み run の比較依頼作成除外」(rdra-feedback #14)。実行ログに `from=REQUESTED to=ABORTED` | tier-ops |
...
| 情報 | 中止指示 | run_id・中止対象種別・停止確認応答・指示者・更新後状態(ABORTED)。abort-blue / abort-green は対象 run の REQUESTED の速報比較依頼も ABORTED にする |
...
| 情報 | 速報比較依頼(rapid_crosscheck_request) | RUNNING → ABORTED(abort-rapid-crosscheck)/ REQUESTED → ABORTED(abort-blue / abort-green による未着手依頼の中止)の更新対象 |
...
| 状態 | クロスチェック依頼 | RUNNING → ABORTED(abort-rapid-crosscheck / abort-final-crosscheck)/ REQUESTED → ABORTED(abort-blue / abort-green。速報比較依頼のみ) |
| 状態 | 並行稼働実行 | STARTED → ABORTED / RUNNING → ABORTED |
| 条件 | slot 中止可否判定 | background × RUNNING のみ可。記録先は aborted.txt、off 以外は管理 DB も更新。対象 run の REQUESTED の速報比較依頼も ABORTED にする(CLAIMED / RUNNING は変更しない) |
| 条件 | slot 実行の状態導出規則 | 可否判定の status をファイルから導出する |
| 条件 | 依頼中止可否判定 | RUNNING のみ可 |
| 条件 | 停止確認応答 | yes のときだけ更新 |
| 条件 | 依頼状態遷移規則 | 停止確認後の中止で ABORTED(RUNNING から、および未着手の速報比較依頼は REQUESTED から) |
...
|---|---|---|
| REQ-010 | SPEC-010-01 | background かつ RUNNING の green を中止する(SPEC-010-01 / SPEC-010-03) / foreground の blue は中止できない(SPEC-010-01) / RAPID_CROSSCHECK_MODE=off でも aborted.txt を書いて中止が成立する(SPEC-010-01) / 既に ABORTED の slot への再実行は中止不可として拒否する(SPEC-010-01) / REQUESTED の速報比較依頼がある run を中止すると依頼も ABORTED になる(SPEC-010-01) / CLAIMED または RUNNING の速報比較依頼は slot の中止では変更されない(SPEC-010-01) |
...
Feature: 実行を ABORTED へ遷移させる
  ...
  Scenario: REQUESTED の速報比較依頼がある run を中止すると依頼も ABORTED になる(SPEC-010-01)
    # 競合窓の保険(rdra-feedback #14): 通常は REQUESTED の依頼がある時点で exitcode.txt が公開済みのため 3 となり本経路に到達しない
  ...
  Scenario Outline: CLAIMED または RUNNING の速報比較依頼は slot の中止では変更されない(SPEC-010-01)
    ...
    And 実行ログ abort-blue.sh.log に `rapid request aborted` を含む行は残らず、依頼の中止は abort-rapid-crosscheck.sh の対象のままである
