...
運用者が、通知メールやジョブスケジューラの実行結果から中止対象の run_id を特定し、対象のプロセス・Pod・SSH 接続先の処理を自身で強制終了したうえで、中止スクリプト(abort-blue / abort-green / abort-rapid-crosscheck / abort-final-crosscheck)で RUNNING の slot 実行または比較依頼を ABORTED へ明示的に遷移させる BUC。スクリプトは現在状態を表示して停止確認に yes と応答されたときだけ状態を更新し、自身ではプロセスを停止しない。slot 実行の中止は成果物ディレクトリへの `aborted.txt`(中止日時 1 行)の書き込みを正本とし、RAPID_CROSSCHECK_MODE が off 以外なら管理 DB の状態も ABORTED に更新し、対象 run に REQUESTED で未着手の速報比較依頼があればそれも ABORTED にする(CLAIMED / RUNNING の依頼は abort-rapid-crosscheck の対象のまま)。off でも成果物ファイルだけで中止が成立し、ABORTED にした background slot 実行と速報比較依頼は background 側リランフローの対象になれる。
...
| [実行を ABORTED へ遷移させる](実行を%20ABORTED%20へ遷移させる/spec.md) | 運用者(受益者) | yes 応答時に限り、background かつ RUNNING の slot 実行は `aborted.txt` を書き(off 以外は加えて管理 DB を条件付き UPDATE)、RUNNING の比較依頼は条件付き UPDATE で ABORTED にし、並行稼働実行が STARTED / RUNNING なら併せて ABORTED にする(COMPLETED は変更しない)。abort-blue / abort-green は対象 run の REQUESTED(未着手)の速報比較依頼も ABORTED にする(off 以外。0 件で可) | 中止指示、slot 実行、速報比較依頼、確報比較依頼、並行稼働実行、実行ログ、Runner Result |
...
graph LR
  ...
  ABORT -->|"RUNNING → ABORTED(abort-rapid-crosscheck)/ REQUESTED → ABORTED(abort-blue / abort-green の未着手依頼。off 以外)"| RR
...
BUC 内で関連する状態モデルは 3 つ(slot 実行 / クロスチェック依頼 / 並行稼働実行)。slot 実行は RUNNING → ABORTED、クロスチェック依頼は RUNNING → ABORTED(abort-rapid-crosscheck / abort-final-crosscheck)と REQUESTED → ABORTED(abort-blue / abort-green による未着手の速報比較依頼の中止。速報比較依頼のみ)、並行稼働実行は STARTED → ABORTED と RUNNING → ABORTED をこの BUC が担当する。前半 UC は状態を変更しない。中止対象が RUNNING 以外(および slot の foreground / off)は状態を変更せずエラー終了する。
...
stateDiagram-v2
  ...
  state "クロスチェック依頼(速報 / 確報)" as REQ {
    ...
    R_REQUESTED --> R_ABORTED: 実行を ABORTED へ遷移させる(abort-blue / abort-green。対象 run の未着手の速報比較依頼のみ。off 以外。条件付き UPDATE status='REQUESTED'。0 件は正常)
    R_ABORTED --> [*]
  }
  state "並行稼働実行" as PR {
...
| クロスチェック依頼(速報比較依頼) | REQUESTED | ABORTED | [実行を ABORTED へ遷移させる](実行を%20ABORTED%20へ遷移させる/spec.md)(abort-blue / abort-green。対象 run の未着手依頼のみ。確報比較依頼には適用しない) |
...
BUC.tsv 上で 1 UC のみに紐づく条件(CLI とメールによる提示 / slot 中止可否判定 / 依頼中止可否判定 / 依頼状態遷移規則)は各 UC spec に記載する。slot 中止可否判定は、yes で slot を ABORTED にするとき対象 run の REQUESTED で未着手の速報比較依頼も ABORTED にする(CLAIMED / RUNNING は変更せず abort-rapid-crosscheck の対象)ことを含む(UC「実行を ABORTED へ遷移させる」)。この REQUESTED → ABORTED は競合窓の保険である: REQUESTED の依頼は両 slot の exitcode.txt 公開後にしか存在せず、その時点で abort-blue / abort-green は終端済みとして 3 を返すため、到達するのは成果物再確認と exitcode.txt 公開・dispatcher の依頼作成が同時刻に起きた競合ケースだけ。両 slot 完了直後の依頼を止める本来の経路は dispatcher の条件「中止済み run の比較依頼作成除外」(速報クロスチェックフロー。parallel_runs ABORTED または slot_executions に ABORTED あり)である(rdra-feedback #14)。CLAIMED のまま中止した依頼は worker の lease 失効で REQUESTED に戻り再 claim されうる(止めるには abort-rapid-crosscheck の対象拡張が必要。rdra-feedback #15)。
...
| クロスチェック依頼状態 | REQUESTED、CLAIMED、RUNNING、SUCCEEDED、FAILED、ABORTED(abort-rapid-crosscheck / abort-final-crosscheck の中止可は RUNNING のみ。REQUESTED の速報比較依頼は abort-blue / abort-green の slot 中止に伴い ABORTED になる) | 現在状態を確認して停止確認に応答する, 実行を ABORTED へ遷移させる |
