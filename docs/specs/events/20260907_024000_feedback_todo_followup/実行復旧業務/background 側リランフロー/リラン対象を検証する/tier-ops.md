...
Feature: リラン対象を検証する - 実行監視・復旧ティア
  ...
  Scenario: CLAIMED の速報比較依頼は拒否する
    ...
    Then 終了コード 3 で stderr に `error: source request is not rerunnable run_id=20260830T113000-JOB001-3f9a1c2e role=rapid-crosscheck status=CLAIMED` と `hint: request is still queued; wait for the worker` が出る
