...
- `yes` のとき `aborted.txt` を `.tmp` → `mv` で公開し、RAPID_CROSSCHECK_MODE が off 以外なら加えて管理 DB(`slot_executions` / `parallel_runs`)を条件付き UPDATE で ABORTED にする。同じ run に REQUESTED で未着手の速報比較依頼があればそれも ABORTED にする(`rapid_crosscheck_requests` を `WHERE run_id=? AND status='REQUESTED'` で条件付き UPDATE。0 件は正常。CLAIMED / RUNNING の依頼は `abort-rapid-crosscheck.sh` の対象のまま。stdout には依頼の状態を出さず、実行ログに `INFO rapid request aborted run_id=... from=REQUESTED to=ABORTED operator=... answer=yes` を残す。条件「slot 中止可否判定」)。off では管理 DB に触れず `aborted.txt` だけで完了する(終了コード 0。stdout の出力順は上と同じ)
