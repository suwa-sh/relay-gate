...
|---|---|---|---|---|---|---|---|
| 並行稼働 | blue=foreground / green=background / rapid=background | green(bg)→ blue(fg) | blue | blue | parallel_runs / slot_executions / rapid_runs を作成 | 両 runner が RAPID_CROSSCHECK_RUNNER へ送信。両系成功で速報比較依頼 | green の実行エラー・ハング疑い、速報の異常 |
| 新実装の単独本番 | blue=off / green=foreground / rapid=off | green(fg) | green | green | 接続しない | 送信しない | なし(hang-detector は成果物走査のみで対象無し) |
| 次世代実装との並行稼働 | blue=background / green=foreground / rapid=background | blue(bg)→ green(fg) | green | green | 作成 | 両 runner が送信 | blue の実行エラー・ハング疑い、速報の異常 |
| custom(表に無い有効な組合せ。例: blue=foreground / green=background / rapid=off) | — | background → foreground | foreground | foreground | rapid が off 以外(foreground / background)なら作成、off なら接続しない | rapid が off 以外なら送信 | background slot(hang-detector は off でも成果物を走査) |

- 管理 DB(ジョブキュー兼管理 DB)は relay-gate 内部のデータストアで、外部システムではない。rapid の foreground と background は完了通知・管理 DB 書き込みの範囲で同じ挙動(判定は「off か off 以外か」)
- 世代交代(並行稼働 → 単独本番 → 次世代並行稼働)の切り替え単位は feature flag の変更のみ。次世代並行稼働への移行時は `BLUE_RUNNER` / `BLUE_IMPL` に旧 green の runner と実装版、`GREEN_RUNNER` / `GREEN_IMPL` に次世代の runner と実装版を割り当て、`blue-job-map.csv` を旧 green のジョブマップから作る(UC「slot runner の実体スクリプトを割り当てる」「slot ごとのジョブマップを定義する」)
...
- **実行ログ**(定義元: UC「slot 実行モードを選択して runner を起動する」): `INFO feature flag loaded blue_mode=... green_mode=... rapid_crosscheck_mode=... blue_impl=... green_impl=... operation_mode=parallel`(起動のたび。operation_mode の値は parallel / green_only / next_gen_parallel / custom。設定版は無い)
...
Feature: 切り替えた運用モードで業務ジョブを実行する - facade / slot runner ティア
  ...
  Scenario: facade_sh は同じ引数で feature flag だけを変えると応答元 slot が変わる
    ...
    And feature-flag.env が BLUE_MODE=foreground GREEN_MODE=background RAPID_CROSSCHECK_MODE=off BLUE_IMPL=blue-2.3.1 GREEN_IMPL=green-1.4.0(両 RUNNER はスタブ)である
  ...
  Scenario: facade_sh は実行ログに operation_mode を記録する
    ...
    When `facade.sh JOB001` を実行する
    Then RELAY_GATE_LOG_DIR/facade.sh.log に "operation_mode=custom" と "blue_impl=" "green_impl=" を含む行が出る
    And feature-flag.env を RAPID_CROSSCHECK_MODE=background(管理 DB あり。RAPID_CROSSCHECK_RUNNER / RAPID_CROSSCHECK_WORKER を設定)にして実行すると "operation_mode=next_gen_parallel" が出る
