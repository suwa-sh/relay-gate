...
`facade.sh` の中継フェーズ: presentation(無加工中継)、usecase(完備確認 → 中継 → (RAPID_CROSSCHECK_MODE が off 以外) COMPLETED 更新)、repository(Runner Result 参照、parallel_runs 更新)、gateway(RDB)を新規実装する。UC「slot 実行モードを選択して runner を起動する」の `wait` 完了後に続けて実行される。
...
- **parallel_runs 更新失敗の終了コードの区別**(契約 facade.sh exit 6 との整合): runner 起動前の parallel_runs INSERT(STARTED)失敗(UC「slot 実行モードを選択して runner を起動する」)は終了コード 6。**全 slot 起動後の STARTED→RUNNING UPDATE 失敗と中継後の RUNNING→COMPLETED UPDATE 失敗は終了コードに反映しない**(runner 起動済み・業務結果を優先。実行ログに `ERROR management db update failed table=parallel_runs run_id=...`。契約 facade.sh exit 6 / execution_log)
...
- **実行ログ**: `relay started run_id=... role=blue exit_code=0 stdout_bytes=... stderr_bytes=...` / `relay finished run_id=... duration_ms=...` / `INFO parallel_run status changed from=RUNNING to=COMPLETED run_id=...`(off 以外)/ `ERROR management db update failed table=parallel_runs run_id=...`(中継後の COMPLETED UPDATE 失敗。終了コードは変えない)
...
追加の設定は無い。`RAPID_CROSSCHECK_MODE`(foreground / background / off)は前 UC で feature flag から読み込み済みの値を使う(off 以外なら parallel_runs を更新)。

## データモデル変更

### parallel_runs(RAPID_CROSSCHECK_MODE が off 以外のみ。ジョブキュー兼管理 DB は relay-gate 内部のデータストア)

| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| status | string | RUNNING → COMPLETED | 追加 |
| completed_at | datetime | 中継完了日時(ローカル時刻) | 追加 |
...
- RAPID_CROSSCHECK_MODE が foreground / background のときだけ parallel_runs を更新し、off では管理 DB に触れない(条件: 速報クロスチェック有効判定)
...
Feature: foreground slot の結果をジョブスケジューラへ中継する - facade / slot runner ティア
  ...
  Scenario: facade_sh は RAPID_CROSSCHECK_MODE=background で中継後に parallel_runs を COMPLETED にする
    Given 管理 DB が起動しており feature-flag.env に RAPID_CROSSCHECK_MODE=background がある
    When `facade.sh JOB001` を実行する
  ...
  Scenario: facade_sh は parallel_runs の更新に失敗しても exitcode.txt の値で終了する
    Given RAPID_CROSSCHECK_MODE=background で、中継後に管理 DB が停止している
