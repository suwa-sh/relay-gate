...
| 情報 | ジョブスケジューラ応答 | 確認対象(属性: run_id / 標準出力 / 標準エラー / 終了コード。応答は時刻を持たず、実行日時の履歴はジョブスケジューラの責務) |
| 情報 | Runner Result | 応答の元 |
| 情報 | 並行稼働実行(parallel_run) | run_id で相関(RAPID_CROSSCHECK_MODE が off 以外のとき。ジョブキュー兼管理 DB は relay-gate 内部のデータストア) |
...
|---|---|---|
| REQ-001 | SPEC-001-03 | 並行稼働中の応答は blue の結果だけである(SPEC-001-03) / 単独本番中の応答は green の結果である(SPEC-001-03) |
| REQ-002 | SPEC-002-02 | 並行稼働中の応答は blue の結果だけである(SPEC-001-03) |
| REQ-005 | SPEC-005-05 | 速報が失敗しても応答は変わらない(SPEC-005-05) |
| REQ-012 | SPEC-012-01 | 終了コードだけで成否を判定できる(SPEC-012-01) |
| REQ-012 | SPEC-012-02 | 実行ログと成果物で run を追跡できる(SPEC-012-02) |
...
Feature: 業務ジョブの実行結果を確認する
  ...
  Scenario: 実行ログと成果物で run を追跡できる(SPEC-012-02)
    Given facade.sh JOB001 が run_id=20260830T113000-JOB001-3f9a1c2e で実行された
    ...
    And facade/20260830T113000-JOB001-3f9a1c2e/green/ に background の 3 ファイルがある
```
