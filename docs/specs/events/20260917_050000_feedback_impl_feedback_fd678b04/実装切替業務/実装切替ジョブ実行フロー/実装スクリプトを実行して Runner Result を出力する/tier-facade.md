...
### slot_executions(RAPID_CROSSCHECK_MODE が off 以外のみ。条件「slot 実行の状態導出規則」の管理 DB 側の規則に従い、aborted.txt または exitcode.txt の公開時点の状態を条件付き UPDATE(WHERE status='RUNNING')で一度だけ書く列。abort 後に exitcode.txt を公開した経路だけファイル正本の導出値と一致せず ABORTED のまま残る。正本: rdb-schema.yaml slot_executions.status / 契約 shared_rules.exitcode_to_status.slot_execution)
...
- slot 実行の状態はファイル正本から導出する。exitcode.txt が無く aborted.txt があれば ABORTED、どちらも無ければ RUNNING(条件: slot 実行の状態導出規則)。管理 DB slot_executions.status は同条件の管理 DB 側の規則に従い aborted.txt または exitcode.txt の公開時点の状態を条件付き UPDATE(WHERE status='RUNNING')で一度だけ書き、abort 後に実装が走り切って exitcode.txt を公開した経路だけ導出値(exitcode.txt 優先)と一致せず ABORTED のまま残る(再同期しない。dispatcher の条件「中止済み run の比較依頼作成除外」の判定材料。SPEC-005-02 AC6 / AC9。正本: rdb-schema.yaml slot_executions.status)
