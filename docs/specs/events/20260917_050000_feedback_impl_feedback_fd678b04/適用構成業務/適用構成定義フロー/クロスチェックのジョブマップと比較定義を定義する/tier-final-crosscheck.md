...
ファイル: `$RELAY_GATE_CONFIG_DIR/target-catalog.csv`。形式は slot ジョブマップと同じ CSV(カンマ区切り、1 行目ヘッダー、1 行 1 対象、改行コード LF、セル内に改行は置けない)。セルは二重引用符で囲め、囲んだセル内の二重引用符は `""` と二重化する。`#` で始まる行はコメント。複数の版を同一ファイルに持てる(catalog_version 列で区別)。
