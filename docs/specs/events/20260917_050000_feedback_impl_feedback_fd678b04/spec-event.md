...
| 項目 | 内容 |
|------|------|
| Event ID | 20260917_050000_feedback_impl_feedback_fd678b04 |
| Created At | 2026-09-16T20:00:00Z |
| Source | Spec 差分更新: feedback 20260917_014138_impl_feedback_fd678b04(design 無しモード。実装フェーズ UC fd678b04「feature flag を設定する」からの spec-gap 2 件)。前イベント 20260908_062000_feedback_spec_machine_readable を起点に、(1) CR-001: UC「feature flag を設定する」の Scenario「ジョブ定義を変えずに feature flag だけで運用モードを切り替える(SPEC-001-03)」の When / Then を validate-config.sh --feature-flag の実行結果だけで判定する形に分離し、実行側は UC「slot 実行モードを選択して runner を起動する」の既存 Scenario(ジョブ定義不変の前提を追記)で覆う。同形の Scenario を全 UC の spec.md で走査し UC「slot ごとのジョブマップを定義する」の SPEC-008-05 Scenario も同方針で分離(Scenario 名は不変。usdm[] は変更なし)、(2) CR-002: cli-command-contract.yaml の validate-config.sh に runner_help_probe(1 slot あたり待機上限 4 秒 / blue → green 逐次 / 上限到達時はプロセスグループへ TERM → 0.2 秒後 KILL / 未応答判定基準 / 版行の採用規則 / 準備失敗は終了コード 6 / NFR B.2.1.1 の応答時間予算)を一元化し、UC「slot runner の実体スクリプトを割り当てる」「feature flag を設定する」の tier-facade.md は契約参照に置き換え(5 秒(仮採用)は削除)。trigger_event: rdra:20260908_011000_feedback_slot_status_wording, arch:20260908_015000_feedback_slot_status_wording |
| UC 総数 | 32 |
| API 総数 | 0 |
| 非同期イベント総数 | 4 |
| 業務数 | 5 |
