...
ファイル: `$RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv`。形式は slot ジョブマップと同じ CSV(カンマ区切り、1 行目ヘッダー、1 行 1 job_id、改行コード LF、セル内に改行は置けない)。セルは二重引用符で囲め、囲んだセル内の二重引用符は `""` と二重化する(囲まないセルにはカンマ・二重引用符を含められない)。`#` で始まる行はコメント。先頭のコメント行にファイル全体の宣言を置く。
