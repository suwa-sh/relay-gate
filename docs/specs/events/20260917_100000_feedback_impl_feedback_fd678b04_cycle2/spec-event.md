...
| 項目 | 内容 |
|------|------|
| Event ID | 20260917_100000_feedback_impl_feedback_fd678b04_cycle2 |
| Created At | 2026-09-17T01:00:00Z |
| Source | Spec 差分更新: 2026-09-17 の実装フィードバック cycle 2(design 無しモード。実装フェーズ UC fd678b04「feature flag を設定する」からの spec-gap 1 件: feature-flag.env で拒否する「確報の制御キー」の範囲)。前イベント 20260917_050000_feedback_impl_feedback_fd678b04 を起点に、CR-001: 確報の制御キーの範囲を『キー名が FINAL_ で始まるすべてのキー』(接頭辞判定。キー名の列挙ではない。利用者が事前に指定し 2026-09-17 のレビューで承認済みの値)に一意に定め、正本を cli-command-contract.yaml の config_files.feature-flag.env.validation_rules に置いた。同 UC の spec.md(概要 / 処理フロー / 分岐条件一覧 / 設定所有区分)と tier-facade.md(組合せ検証 / ビジネスルール)を同じ表記(FINAL_ で始まるキー)に揃え、FINAL_ で始まるキーは未知キー warn より先に判定して warn にしない規則を明記。境界を判定する BDD Scenario「FINAL_ で始まる確報設定のキーは未知キーではなく拒否される(SPEC-001-01)」(FINAL_DB_CONN_REF を置く)を E2E に追加し usdm[] の SPEC-001-01 に登録、tier-facade.md のティア完了条件にも同旨の Scenario を追加。facade.sh の起動時検証(同じ検証を共有)の終了コード 2 条件と validate-config.sh の stderr / 終了コード説明、_api-summary.yaml、usdm-acceptance-matrix.md、traceability-matrix.md を追従。判断は decisions/spec-decision-010.yaml。trigger_event: rdra:20260908_011000_feedback_slot_status_wording, arch:20260908_015000_feedback_slot_status_wording |
| UC 総数 | 32 |
| API 総数 | 0 |
| 非同期イベント総数 | 4 |
| 業務数 | 5 |
