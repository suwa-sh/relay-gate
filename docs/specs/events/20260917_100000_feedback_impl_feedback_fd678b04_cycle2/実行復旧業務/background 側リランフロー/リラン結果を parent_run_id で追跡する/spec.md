...
| レイヤー | データモデル | 変換内容 |
|---------|------------|---------|
| ops presentation | 引数(`--run-id 20260830T124500-JOB001-7b2d9e01`, `--limit 100`) | 引数検証 → RunLineageQuery |
...
sequenceDiagram
  ...
  Ops->>Pres: run-lineage.sh --run-id 20260830T124500-JOB001-7b2d9e01
...
| 速報クロスチェック有効判定 | RAPID_CROSSCHECK_MODE=off では parallel_runs が作成されないため系譜を追跡できず終了コード 3(off でのリラン系譜は新 execution-spec.json の `parent_run_id` キーを運用者が辿る) | tier-ops | run-lineage.sh の管理 DB 接続前判定 | 管理 DB が無い構成では系譜を追跡できない |
...
| 内部データストア | ジョブキュー兼管理 DB(RDB) | parallel_runs の参照先(parent_run_id 系譜の参照) |
...
Feature: リラン結果を parent_run_id で追跡する
  ...
  Scenario: 2 回リランした系譜を元の実行まで辿る(SPEC-009-01)
    Given RAPID_CROSSCHECK_MODE=background で parallel_runs に次の 3 行がある(日時はローカル時刻、指示子なし。TZ=UTC 前提)
      | run_id | parent_run_id | job_id | status | requested_at | completed_at |
      | 20260830T113000-JOB001-3f9a1c2e | - | JOB001 | ABORTED | 2026-08-30T11:30:00 | 2026-08-30T12:40:00 |
      | 20260830T124500-JOB001-7b2d9e01 | 20260830T113000-JOB001-3f9a1c2e | JOB001 | ABORTED | 2026-08-30T12:45:00 | 2026-08-30T13:50:00 |
      | 20260830T140000-JOB001-c4d5e6f7 | 20260830T124500-JOB001-7b2d9e01 | JOB001 | COMPLETED | 2026-08-30T14:00:00 | 2026-08-30T15:10:00 |
    And slot_executions に 2 つのリラン run_id の slot=green の行がある(20260830T140000-JOB001-c4d5e6f7 は green slot の終端時に runner が COMPLETED にした)
    When 運用者が `run-lineage.sh --run-id 20260830T140000-JOB001-c4d5e6f7` を実行する
    Then 終了コード 0 で stdout に次の TSV が出る
      | depth | run_id | parent_run_id | job_id | role | run_status | requested_at | completed_at |
      | 2 | 20260830T140000-JOB001-c4d5e6f7 | 20260830T124500-JOB001-7b2d9e01 | JOB001 | green | COMPLETED | 2026-08-30T14:00:00 | 2026-08-30T15:10:00 |
      | 1 | 20260830T124500-JOB001-7b2d9e01 | 20260830T113000-JOB001-3f9a1c2e | JOB001 | green | ABORTED | 2026-08-30T12:45:00 | 2026-08-30T13:50:00 |
      | 0 | 20260830T113000-JOB001-3f9a1c2e | - | JOB001 | - | ABORTED | 2026-08-30T11:30:00 | 2026-08-30T12:40:00 |

  Scenario: 途中の run_id を指定しても系譜全体が出る
    Given 上記 3 行の parallel_runs がある
    When 運用者が `run-lineage.sh --run-id 20260830T124500-JOB001-7b2d9e01` を実行する
    Then 終了コード 0 で stdout のデータ行は 3 行で、1 行目の run_id が 20260830T140000-JOB001-c4d5e6f7、3 行目が 20260830T113000-JOB001-3f9a1c2e である

  Scenario: 速報比較依頼だけを再作成した run は role=rapid-crosscheck で出る
    Given parallel_runs に run_id=20260830T130000-JOB001-a1b2c3d4 parent_run_id=20260830T113000-JOB001-3f9a1c2e があり slot_executions に同 run_id の行が無く rapid_crosscheck_requests に同 run_id の行がある
    When 運用者が `run-lineage.sh --run-id 20260830T130000-JOB001-a1b2c3d4` を実行する
...
```gherkin
  Scenario: 存在しない run_id
    When 運用者が `run-lineage.sh --run-id 20260830T000000-JOB999-00000000` を実行する
    Then 終了コード 3 で stderr に `error: run not found run_id=20260830T000000-JOB999-00000000` が出る

  Scenario: 管理 DB が無い構成では系譜を追跡できない
    Given RAPID_CROSSCHECK_MODE=off である
    When 運用者が `run-lineage.sh --run-id 20260830T113000-JOB001-3f9a1c2e` を実行する
