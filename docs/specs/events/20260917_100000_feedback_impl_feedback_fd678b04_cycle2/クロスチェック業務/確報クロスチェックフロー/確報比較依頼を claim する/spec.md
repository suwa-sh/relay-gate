...
|--------|---------|---------------|---------|----------|
| lease_until の算出 | 現在時刻(ローカル時刻)、FINAL_LEASE_MINUTES(既定 10) | lease_until = now + FINAL_LEASE_MINUTES 分 | 確報比較依頼.lease_until | tier-final-crosscheck |
...
| 現在時刻(now) | システム時刻(ローカル時刻)、テスト専用環境変数 `RELAY_GATE_NOW` | `RELAY_GATE_NOW`(ISO 8601 UTC Z 付き。cli-command-contract.yaml environment_variables で宣言。本番未設定)が設定されていればその値を内部でホストのローカルタイムゾーンへ変換した値、未設定ならシステム時刻(ローカル時刻)。lease_until は管理 DB・実行ログともローカル時刻(指示子なし)で表す。worker は now を SQL のバインド値として渡す(DB の now() に依存しない) | lease_until / 失効判定の now | tier-final-crosscheck |
...
Feature: 確報比較依頼を claim する
  ...
  Scenario: REQUESTED の依頼を claim する(SPEC-006-02)
    Given final_crosscheck_requests に final_crosscheck_id=20260830T210000-final-7b2c9e1f status=REQUESTED requested_at=2026-08-30T21:00:00 の行がある
    ...
    Then 実行ログに "INFO claimed final_crosscheck_id=20260830T210000-final-7b2c9e1f worker_id=final-worker-01 lease_until=2026-08-30T21:10:30" が残る
    And スタブが待機している間、該当行は worker_id=final-worker-01 lease_until=2026-08-30T21:10:30 で、status は CLAIMED または RUNNING である(`--once` は claim 後に同一プロセスで次 UC の比較実行まで進むため、プロセス終了時点の status=CLAIMED は観測できない)
    And スタブに停止シグナルを送ると worker は依頼を終端(FAILED)まで保存して終了コード 0 で終了する

  Scenario: lease 失効かつ未開始の依頼を再取得できる(SPEC-007-01)
    Given final_crosscheck_requests に status=CLAIMED worker_id=final-worker-01 lease_until=2026-08-30T21:10:30 started_at=NULL の行がある
    ...
    Then 該当行はいったん REQUESTED に戻された後、スタブが待機している間は worker_id=final-worker-02 lease_until=2026-08-30T21:21:00 で status は CLAIMED または RUNNING である
...
```gherkin
  Scenario: 比較開始済み(started_at あり)の依頼は lease が失効しても戻さない
    Given final_crosscheck_requests に status=RUNNING worker_id=final-worker-01 lease_until=2026-08-30T21:10:30 started_at=2026-08-30T21:01:00 の行がある
