...
Feature: 確報クロスチェック結果を確認する - tier-worker
  ...
  Scenario: worker presentation層が対象日の確報比較依頼を返す
    Given run_id "run-20260817-901" の確報比較依頼が target_date "2026-08-17" status "SUCCEEDED" で final_crosscheck_requests に存在する
    When tier-workerのpresentation層が GetFinalCrossCheckResultQuery(target_date="2026-08-17") を受け取る
    Then gatewayは final_crosscheck_requests から run_id "run-20260817-901" のレコードを1件取得し、presentation層は終了コード 0 で標準出力へ status "SUCCEEDED" を返す
```
