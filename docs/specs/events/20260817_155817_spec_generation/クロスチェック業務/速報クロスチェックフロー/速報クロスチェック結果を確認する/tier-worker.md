...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| run_id | VARCHAR | 速報比較依頼の一意識別子（PK） | 参照のみ |
...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| run_id | VARCHAR | 速報比較依頼のrun_idを参照するFK（PK） | 参照のみ |
...
Feature: 速報クロスチェック結果を確認する - バックエンドワーカーティア
  ...
  Scenario: run_id指定でSUCCEEDED結果を取得する
    Given rapid_crosscheck_requests に run_id "run-20260817-001" status "SUCCEEDED" が存在する
    And rapid_crosscheck_results に run_id "run-20260817-001" comparison_result "OK" diff_count 0 が存在する
    When worker presentation層が `relaygate rapid-crosscheck result --run-id run-20260817-001` を受け付ける
    Then gateway層はrun_id "run-20260817-001" でLEFT JOINしたSELECTを1回実行し、標準出力に comparison_result "OK" を出力して終了コード 0 で終了する
```
