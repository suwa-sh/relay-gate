...
RUNNING中の速報比較依頼（E-003, AG-003）に対する中止依頼を受理するCLIコマンドをtier-workerに追加する。本UCは状態遷移を発生させず、対象の状態検証のみを行う。実際のABORTEDへの遷移は次UC「対話確認のうえ速報比較依頼をABORTEDへ遷移させる」が担う。
...
- **環境変数**:
  ...
  | 変数名 | 必須 | 説明 |
  |--------|------|------|
  | RELAYGATE_RDB_DSN | Yes | 速報比較依頼テーブル（rapid_crosscheck_requests）へ接続するRDB接続文字列 |
...
Feature: RUNNING中の速報比較依頼の中止を依頼する - tier-worker
  ...
  Scenario: RUNNING中の速報比較依頼の中止依頼をworkerが受理する
    Given rapid_crosscheck_requestsテーブルにrun_id="rg-2026-0817-013", status="RUNNING", worker_id="worker-03"のレコードが存在する
    When relaygate abort rapid-crosscheck request --run-id rg-2026-0817-013 を実行する
    Then 標準出力に "中止依頼を受理しました run_id=rg-2026-0817-013 status=RUNNING" が出力され終了コード0で終了する

  Scenario: RUNNING以外の状態では中止依頼を拒否する
    Given rapid_crosscheck_requestsテーブルにrun_id="rg-2026-0817-014", status="CLAIMED"のレコードが存在する
    When relaygate abort rapid-crosscheck request --run-id rg-2026-0817-014 を実行する
    Then 標準エラーに "中止依頼できません run_id=rg-2026-0817-014 status=CLAIMED（RUNNING状態のみ中止依頼可能です）" が出力され終了コード1で終了する
```
