...
- クロスチェックジョブマップ CSV(`RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv`。1 行目ヘッダー、1 行 1 job_id、行頭 `#` はコメント行(`# target_catalog_path=` 等の宣言行もここに置く)。セルはカンマ区切りで二重引用符で囲め、囲んだセル内の二重引用符は `""`。6 列): `job_id,comparison_type,compare_targets,compare_command,compare_options,definition_version`
  - 確報用の行は `job_id=final-crosscheck`、`comparison_type=full`(step3 canonical C5。速報の job_id ごとの行と同じ CSV に確報行を 1 行置く)
...
- 対象カタログ CSV(slot ジョブマップと同じ CSV 形式。1 行目ヘッダー、1 行 1 対象): `catalog_version,target_type,target_identifier,compare_condition,business_date_handling`。`target_type` は `table` / `file`
...
- 実行ログ(`final-crosscheck-worker.sh.log`): `INFO comparison started final_crosscheck_id=... business_date=... catalog_version=... targets=N` / gateway `INFO compare tool started command=...` / `INFO compare tool finished exit_code=... duration_ms=...` / `INFO request completed final_crosscheck_id=... status=... exit_code=...` / `WARN request not claimable status changed final_crosscheck_id=...`(RUNNING 遷移 UPDATE 0 件)/ `WARN request already terminal final_crosscheck_id=... status=ABORTED`(終端 UPDATE 0 件)
  - ログ行の形式は `_cross-cutting/ux-ui/ui-design.md` のログ行形式(`{script} {run_id} {ローカル出力日時} {LEVEL} {message}`。run_id 欄は final_crosscheck_id)に従う。情報「実行ログ」の属性「出力日時」はこの ローカル出力日時の列に対応する
...
2. `started-at.txt`(中身は UTC ISO 8601 Z 付き。Runner Result Contract により started-at.txt の中身だけ UTC のまま。管理 DB の started_at はローカル時刻)を `.tmp` → `mv`
3. 対象カタログ CSV から catalog_version の行を抜き出して一時 CSV に書く
...
| started_at | datetime | RUNNING 遷移時刻(ローカル時刻) | 追加(更新) |
...
| ファイル | 型 | 説明 | 変更種別 |
|---------|---|------|---------|
| `facade/<final_crosscheck_id>/final-crosscheck/started-at.txt` | text | 比較開始時刻(中身は UTC ISO 8601 Z 付き、1 行。Runner Result Contract。表示・DB 転記時はローカルへ変換する) | 追加 |
| `.../stdout.log` / `.../stderr.log` | text | 比較ツール出力 | 追加 |
| `.../exitcode.txt` | text | 数値 1 行 | 追加 |
| `.../input/target-catalog.csv` | CSV(対象カタログと同じ列・同じ形式) | 比較に使った対象カタログの抜粋(仮採用。Contract 4 ファイルと同階層には置かない) | 追加 |
...
Feature: 比較ツールで日次全量比較を実行して結果を保存する - 確報クロスチェックティア
  ...
  Scenario: 比較 OK を SUCCEEDED として保存する
    Given final_crosscheck_requests に final_crosscheck_id=20260830T210000-final-7b2c9e1f status=REQUESTED requested_at=2026-08-30T21:00:00 catalog_version=v3 business_date=2026-08-30 がある(1 プロセスで claim → RUNNING → 終端まで検証する)
    And crosscheck-job-map.csv の job_id=final-crosscheck 行の compare_command が /opt/compare/stub-ok.sh(終了コード 0、stdout "all 42 targets matched\n")を指す
    ...
    And facade/20260830T210000-final-7b2c9e1f/final-crosscheck/exitcode.txt の中身は "0" で、同ディレクトリに *.tmp は残っていない
  ...
  Scenario: 比較ツールが実行できない
    ...
    When `final-crosscheck-worker.sh --once --worker-id final-worker-01` を実行する
    Then 終了コード 6 で stderr に "error: compare tool launch failed final_crosscheck_id=20260830T210000-final-7b2c9e1f command=/opt/compare/missing.sh" が出る
  ...
  Scenario: catalog_version の行が 0 行
    Given 同じ依頼が status=REQUESTED catalog_version=v9 で、対象カタログ CSV に catalog_version=v9 の行が無い
    When `final-crosscheck-worker.sh --once --worker-id final-worker-01` を実行する
  ...
  Scenario: 比較中に ABORTED された依頼は終端 UPDATE 0 件で終了コード 0
    ...
    And `final-crosscheck-worker.sh --once --worker-id final-worker-01` を起動し、該当行が status=RUNNING になった後に `abort-final-crosscheck.sh --run-id 20260830T210000-final-7b2c9e1f --yes` で ABORTED にする
    When スタブに停止シグナルを送る
    Then worker は終了コード 0 で終了し、該当行は status=ABORTED exit_code=NULL のままである
    And facade/20260830T210000-final-7b2c9e1f/final-crosscheck/ に exitcode.txt が確定名で残り、実行ログに "WARN request already terminal final_crosscheck_id=20260830T210000-final-7b2c9e1f status=ABORTED" が残る
```
