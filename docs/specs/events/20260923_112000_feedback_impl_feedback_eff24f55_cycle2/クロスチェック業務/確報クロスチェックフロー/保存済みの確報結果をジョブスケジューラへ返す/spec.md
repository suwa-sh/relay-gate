...
| 内部データストア | ジョブキュー兼管理 DB(RDB) | 読み出し元(relay-gate 内部の構成要素。外部システムではない) |

## 関連 USDM

| REQ ID | SPEC ID | 対応 BDD Scenario |
|--------|---------|------------------|
| REQ-006 | SPEC-006-03 | exit_code=3 の FAILED をそのまま返す(SPEC-006-03) / ABORTED で exit_code が NULL のとき終了コード 6 を返す ※ 2 件目は応答に状態名を追加しないことを検証する |
