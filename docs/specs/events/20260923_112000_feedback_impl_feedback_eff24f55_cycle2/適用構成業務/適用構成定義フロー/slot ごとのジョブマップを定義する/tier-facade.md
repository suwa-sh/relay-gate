...
|---|---|---|---|---|---|
| job_id | string | Yes | `^[A-Za-z0-9_-]+$`、ファイル内で一意 | `error: job_id is invalid line=N job_id=<v> value=<v>` / `error: duplicate job_id job_id=<v> lines=<n1,n2,...>`(重複した job_id ごとに 1 行。`lines=` はその job_id が現れた全行の物理行番号を、初出行を含めて出現順にカンマ区切りで全部。例: 2 行目と 5 行目なら `lines=2,5`) | JOB_ID |
...
| credential_ref | string | No(末尾の任意列。空可) | 参照名 `^[A-Za-z0-9_.-]*$`。下表「credential_ref の形式の扱い」 | `warn: credential_ref looks like a secret or path line=N job_id=<v>`(値は出さない) | 認証情報参照名。値は書かない |
...
- **credential_ref の形式の扱い**(CLI 契約 `config_files` の `<slot>-job-map.csv` columns.credential_ref が正本)。参照名の形式は適用側の認証情報ストアの運用に依存するため、relay-gate 側では拒否しない。warn は秘密やパスの混入に気づかせるためのもので、参照名として解決できるかは検査しない

| 値 | 扱い | stderr(終了コード) |
|---|---|---|
| 空・列なし | 受理 | なし(execution-spec.json では `null`) |
| 正規表現に合い `BEGIN` を含まない(例 `ssh-key-green`) | 受理 | なし |
| `/` を含む(例 `/home/batch/.ssh/id_green`) | warn して受理 | `warn: credential_ref looks like a secret or path line=N job_id=<v>`(0。拒否しない) |
| `BEGIN` を含む(例 `BEGIN_KEY`。正規表現に合っていても) | warn して受理 | 同上(0) |
| 正規表現に合わず `/` も `BEGIN` も含まない(空白・記号・非 ASCII 文字を含む等。例 `ssh key green`) | warn して受理 | 同上(0) |

  - 1 行に複数の理由が重なっても warn は 1 行
  - credential_ref の値は診断行(validate-config.sh の stdout と stderr の error: / warn: / info:(--verbose を含む)/ hint:)に一切出さない。位置は `line=N` と `job_id=<v>` だけで示し、`value=` を付けない。slot runner の stderr と実行ログにも出さない(契約 conventions.credentials)
  - warn して受理した値も、正規の参照名と同じく execution-spec.json の `slots.<role>.credential_ref` へそのまま転記される。ジョブマップに認証情報の値そのものを置かないのは適用側の責任で、warn はその見落としに気づかせるための検査
...
- 認証情報は参照名で指定する。参照名の形式に合わない値と `BEGIN` を含む値は同じ warn で報告して受理し、credential_ref の値は診断行に出さない(条件: 認証情報の非保存)
