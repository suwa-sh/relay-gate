...
| 名前 | 型 | 必須 | 既定値 | 説明 |
|------|---|------|-------|------|
| `--crosscheck-job-map` | string(path) | Yes(値必須) | — | 検証するクロスチェックジョブマップのパス。既定パス `$RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv` を使う場合も明示的に渡す(契約 validate-config.sh options。値省略は `error: option required option=--crosscheck-job-map` で 2) |
...
### クロスチェックジョブマップ(CSV、ヘッダー行あり。所在は `_inference.md` #5 仮採用)

ファイル: `$RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv`。形式は slot ジョブマップと同じ CSV(カンマ区切り、1 行目ヘッダー、1 行 1 job_id、文字コード UTF-8、改行コード LF、セル内に改行は置けない)。セルは二重引用符で囲め、囲んだセル内の二重引用符は `""` と二重化する(囲まないセルにはカンマ・二重引用符を含められない)。`#` で始まる行はコメント。先頭のコメント行にファイル全体の宣言を置く。形式から外れた入力(NUL バイト・UTF-8 として不正なバイト列・BOM・CR・ヘッダー列名の重複)の拒否(終了コード 2。文言は契約と同文)、読み込みは 1 時点の内容だけで判定する保証、補助コマンド・複製の失敗(終了コード 6)は CLI 契約 `config_input_rules` に従う(値は複写しない。判定する BDD は UC「slot ごとのジョブマップを定義する」に置く)。出力する任意入力の制御文字は `ui-design.md`「制御文字の表記」の可視表記にする。

| 宣言(コメント行) | 必須 | 説明 |
|---|---|---|
| `# exit_code_contract=0:OK,3:NG,6:ERROR` | Yes | 比較ツール終了コード契約への適合宣言。値は固定文字列 |
| `# target_catalog_path=<path>` | Yes | 確報用の対象カタログ CSV の所在(設定所有区分: 対象カタログ参照) |
...
| compare_targets | string | Yes | 非空。比較対象(テーブル・ファイル)の識別子。複数はカンマ区切り(セルを二重引用符で囲む) |
...
| 4 | job_id 一意 | `error: duplicate job_id job_id=... lines=<n1,n2,...>`(重複した job_id ごとに 1 行。lines= はそのキーが現れた全行の物理行番号を初出行を含めて出現順に並べる。契約 validate-config.sh の stderr) |
...
| 10 | 必須列が非空(セル内に改行を含まない) | `error: <column> is empty line=N job_id=... value=` |
| 11 | CSV セルのクォートが正しい(閉じ引用符がある、囲み外に二重引用符が無い) | `error: csv quote is invalid line=N path: ...` |
...
| ファイル | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| `$RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv` | CSV | 上記「設定契約」 | 追加 |
...
Feature: クロスチェックのジョブマップと比較定義を定義する - 速報クロスチェックティア
  ...
  Scenario: 妥当なジョブマップを検証する
    Given /etc/relay-gate/crosscheck-job-map.csv が宣言 3 行 + ヘッダー + JOB001 / JOB002 の 2 行で構成され、compare_command が実行可能である
  ...
  Scenario: exit_code_contract 宣言が無い
    Given crosscheck-job-map.csv に `# exit_code_contract=` の行が無い
    When `validate-config.sh --crosscheck-job-map /etc/relay-gate/crosscheck-job-map.csv` を実行する
    Then 終了コード 2 で stderr に `error: exit_code_contract declaration not found path: /etc/relay-gate/crosscheck-job-map.csv` が出る
