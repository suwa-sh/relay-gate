...
#### 設定契約(ハング検知定期ジョブ設定 = hang-detector.env。RDRA 情報「ハング検知定期ジョブ設定」。所有者: 基盤適用設計者(設定所有区分: ハング検知定期ジョブ設定)。認証情報は値を置かず参照名 HANG_DB_CONN_REF のみ)

| キー | 型 | 必須 | 既定値 | 検証ルール |
|------|---|------|-------|-----------|
| `ALERT_MAIL_TO` | string | Yes | — | 宛先(通知先メールアドレス)。カンマ区切りで複数可。空は終了コード 2 |
| `ALERT_MAIL_CMD` | string | Yes | — | 送信コマンドの絶対パス(例 `/usr/bin/mail`、`/usr/sbin/sendmail`)。実行可能でなければ終了コード 2 |
| `ALERT_SUBJECT_PREFIX` | string | No | `[relay-gate]` | 件名プレフィックス。件名の先頭に付ける。空文字は既定値として扱う |
...
- **通知メール**(ui-design.md「通知メール規約」を正とする):
  - 件名: `{ALERT_SUBJECT_PREFIX}[{warning|error}] {hang-suspected|background-exec-error|rapid-crosscheck-error} run_id={run_id} job_id={job_id} role={blue|green|rapid-crosscheck}`(件名プレフィックスはハング検知定期ジョブ設定の `ALERT_SUBJECT_PREFIX`。既定 `[relay-gate]`。以下の例は既定値)
  ...
  - `detected_at` は判定 UC と同じ now(`RELAY_GATE_NOW` 設定時はその UTC 値をローカル時刻へ変換した値)
  - 本文の日時(`started_at` / `detected_at`)はホストのローカルタイムゾーンの ISO 8601 秒精度・指示子なし(例 `2026-08-30T11:30:05`。run_id の時刻部・実行ログ・管理 DB の *_at 列と同じ時刻軸)。`started_at` のうち started-at.txt 由来の値は中身(UTC Z 付き)をローカル時刻へ変換して載せる(D3-4。ui-design.md「通知メール規約」)
...
- 実行ログ: `INFO alert sent run_id=... role=... kind=... level=... to=...`(メール本文はログに含めない)/ `INFO alert skipped already notified ...` / gateway `INFO mail started cmd=... to=...` / `INFO mail finished exit_code=0 duration_ms=...` / `ERROR mail send failed exit_code=...`
  - ログ行の形式は `_cross-cutting/ux-ui/ui-design.md` のログ行形式(`{script} {run_id} {ローカル出力日時} {LEVEL} {message}`)に従う。情報「実行ログ」の属性「出力日時」はこの ローカル出力日時の列に対応する
...
- **バリデーション**: `ALERT_MAIL_TO` 非空、`ALERT_MAIL_CMD` が実行可能(いずれもハング検知定期ジョブ設定)。起動時(走査前)に検証し、NG なら終了コード 2 で何も走査しない。`ALERT_SUBJECT_PREFIX` は検証しない(未設定・空は既定 `[relay-gate]`)
- **確認プロンプト**: なし
- **冪等性**(通知の二重送信防止):
  - off 以外(foreground / background): `monitor_records.monitor_status` を読み、遷移が発生する場合だけ送る。送信成功時に記録 UC が monitor_status / alerted_at / elapsed_minutes_at_alert を更新する。送信失敗時は更新しない(次回再送)
  ...
  - 遷移表(状態.tsv「監視状態」のとおり): MONITORING(または記録なし)→ HANG_SUSPECTED_NOTIFIED / EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED、HANG_SUSPECTED_NOTIFIED → EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED。EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED / COMPLETED / NOT_MONITORED からは送らない
...
- 設定所有区分: 宛先・送信コマンド・件名プレフィックス・管理 DB 接続参照名はハング検知定期ジョブ設定(hang-detector.env。基盤適用設計者が所有)から読む。認証情報の値は置かない
...
Feature: ハング疑い・実行エラー・比較異常を通知する - 実行監視・復旧ティア
  ...
  Scenario: warning メールを 1 通送る
    Given ハング検知定期ジョブ設定(hang-detector.env)に ALERT_MAIL_TO=ops@example.invalid ALERT_MAIL_CMD=/tmp/relay-gate-test/mail-stub(受け取った件名と本文をファイルに保存して 0 を返す)があり ALERT_SUBJECT_PREFIX は未設定である
    ...
    Then mail-stub が 1 回だけ起動され、件名は "[relay-gate][warning] hang-suspected run_id=20260830T113000-JOB001-3f9a1c2e job_id=JOB001 role=green" である
    And 本文の 1 行目は "kind=hang-suspected"、2 行目は "level=warning"、13 行目は空行、14 行目は "recommended_action: check the process on the execution host; if it is still running normally, wait. if it is hung, stop the process, then run: abort-green.sh --run-id 20260830T113000-JOB001-3f9a1c2e" の 1 行で、本文は 14 行である
    And 11 行目 `artifact_dir:` を除く 1〜13 行目に 80 桁を超える行は無い(11 行目と 14 行目は折り返さず 80 桁を超えてよい)
    And stderr に "info: alert sent run_id=20260830T113000-JOB001-3f9a1c2e role=green kind=hang-suspected level=warning" が出る
  ...
  Scenario: 2 回目の実行では送らない
    ...
    Then mail-stub は起動されず、stderr に "info: alert skipped already notified run_id=20260830T113000-JOB001-3f9a1c2e role=green kind=hang-suspected" が出る

  Scenario: ALERT_SUBJECT_PREFIX で件名プレフィックスを変える(SPEC-008-06)
    Given ハング検知定期ジョブ設定に ALERT_SUBJECT_PREFIX=[relay-gate-prod] があり、判定が HANG_SUSPECTED で monitor_records に該当行が無い
    When `hang-detector.sh` を実行する
    Then mail-stub が受け取った件名は "[relay-gate-prod][warning] hang-suspected run_id=20260830T113000-JOB001-3f9a1c2e job_id=JOB001 role=green" である
  ...
  Scenario: メール送信コマンドが失敗する
    ...
    When `hang-detector.sh` を実行する
    Then 終了コード 6 で stderr に "error: mail send failed run_id=20260830T113000-JOB001-3f9a1c2e role=green kind=background-exec-error cmd_exit_code=1" が出る
