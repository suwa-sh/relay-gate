...
|---|---|---|
| REQ-008 | SPEC-008-05 | 調整は実行済み run に影響しない(SPEC-008-05) / 調整の記録はジョブマップの列に持たない(SPEC-008-05) ※ USDM の受け入れ基準は 3 つに分かれている。基準 1(次回以降の run に反映)は Scenario「調整は実行済み run に影響しない」、基準 2(ジョブマップに調整記録の列が無い)は Scenario「調整の記録はジョブマップの列に持たない」で判定する。基準 3「[自動判定の対象外: 運用文書の記載]」(調整日時と調整根拠を適用構成文書に残す)は relay-gate が読まない文書が対象で、USDM 側で自動判定の対象外と明記されているため BDD Scenario を持たない(運用手順として Scenario の Given に前提として記す) |
