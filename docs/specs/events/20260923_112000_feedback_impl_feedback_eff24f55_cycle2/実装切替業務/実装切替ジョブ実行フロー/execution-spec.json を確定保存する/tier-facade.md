...
- **stdin**: なし
- **環境変数**(facade.sh が feature-flag.env を読んで渡す。runner は feature-flag.env を読まない): `RAPID_CROSSCHECK_MODE`(run 単位キー `rapid_crosscheck_mode`。foreground / background / off)、`BLUE_IMPL`(role=blue の `slots.blue.impl_version`)/ `GREEN_IMPL`(role=green の `slots.green.impl_version`)。通常起動で自 slot の `*_IMPL` が未設定・空なら設定検証エラー(exitcode.txt=2、stderr.log `error: option required option=GREEN_IMPL`)。復元起動(`--execution-spec`)では渡されず、spec の値を使う
...
| 6 | 実行エラー | lock 取得のタイムアウト(30 秒。仮採用)・一時ファイル書き込み失敗・`mv` 失敗は stderr.log `error: execution-spec write failed run_id=... role=... path=...`、既存 JSON の解析失敗(`slots` なし・run_id 不一致を含む)は `error: execution-spec invalid path=... run_id=...`。いずれも Runner Result 3 ファイル(`exitcode.txt=6`)を可能な限り揃える |
...
- **バリデーション**: ExecutionTarget が揃っていること(前フェーズで検証済み)。`credential_ref` / `map_version` は列が無い・空なら `null`。`host` / `user` はローカル実行なら `null`。自 slot の `BLUE_IMPL` / `GREEN_IMPL` が非空であること。復元起動(`--execution-spec`)の spec なし・run_id 不一致・欠落は 2(契約 runner IF exit_codes 2)。通常起動で既存 JSON が解析できない・`slots` を持たない・`run_id` が `--run-id` と一致しない場合は契約どおり 6(stderr.log は `error: execution-spec write failed ...` ではなく `error: execution-spec invalid path=... run_id=...`。上記の終了コード表と同じ)
- **確認プロンプト**: なし
...
参照する設定は前 UC で解決済みの ExecutionTarget と環境変数 `RAPID_CROSSCHECK_MODE` / `BLUE_IMPL` / `GREEN_IMPL`(facade が feature-flag.env から渡す)のみ。設定ファイルを直接読まない。
...
| run_id | string | run_id(`{ローカル yyyymmddThhmmss}-{job_id}-{8 桁 hex}`。成果物ディレクトリ名・管理 DB と同じ値) | 最初の runner | 追加 |
...
| rapid_crosscheck_mode | string | foreground / background / off | 最初の runner | 追加 |
...
| host | string / null | 実行先ホスト。ローカル実行(ジョブマップに host / user 列が無い・空)なら null | 追加 |
| user | string / null | 実行ユーザー。ローカル実行なら null | 追加 |
| script | string | 実装スクリプトパス | 追加 |
| work_dir | string | 作業ディレクトリ | 追加 |
| fixed_params | string[] | 固定引数(fixed_params セルを解析した JSON 配列) | 追加 |
| hang_detect_limit_minutes | integer | この role のハング検知上限 | 追加 |
| credential_ref | string / null | 認証情報参照名(ジョブマップの任意列の値を加工せずに転記。形式の検査・除去はしない)。認証情報の値そのものは書かない | 追加 |
| map_version | string / null | マップ版(ジョブマップの任意列 map_version。列が無ければ null) | 追加 |
| impl_version | string | 実装版(feature flag の BLUE_IMPL / GREEN_IMPL。環境変数経由) | 追加 |
| finalized_at | string | 節の確定保存日時(UTC ISO 8601 秒精度、`Z` 付き。例 `2026-08-30T11:30:01Z`。execution-spec.json は Runner Result Contract のファイル成果物なので started-at.txt / aborted.txt と同じく中身は UTC のまま。表示・実行ログへ出すときはローカルへ変換する。`RELAY_GATE_NOW` 設定時はその値) | 追加 |

## ビジネスルール

- run 開始時に自 slot の節を一度だけ保存し、以後ジョブマップを変更しても上書きしない(条件: 実行設定の確定条件)
- 認証情報は参照名だけを保存する。credential_ref 列の値は加工せずに転記し、鍵ファイルの中身などを relay-gate が読んで書くことはない(条件: 認証情報の非保存)
...
Feature: execution-spec.json を確定保存する - facade / slot runner ティア
  ...
  Scenario: save_slot_spec_once はファイルが無ければ run 単位キーと自 slot 節で新規作成する
    Given RELAY_GATE_ARTIFACT_ROOT/facade/20260830T113000-JOB001-3f9a1c2e/ が空である
    And RELAY_GATE_CONFIG_DIR/green-job-map.csv に JOB001 の行(hang_detect_limit_minutes=60)がある
    When `<runner> --run-id 20260830T113000-JOB001-3f9a1c2e --job-id JOB001 --role green --mode background -- 20260830` を環境変数 RAPID_CROSSCHECK_MODE=off GREEN_IMPL=green-1.4.0 で保存フェーズまで実行する
    Then execution-spec.json が存在し、.tmp と .lock は存在しない
    And JSON の run_id は "20260830T113000-JOB001-3f9a1c2e"、parent_run_id は null、params は ["20260830"]、rapid_crosscheck_mode は "off" である
    And slots.green.mode は "background"、slots.green.hang_detect_limit_minutes は 60、slots.green.impl_version は "green-1.4.0" であり、slots.blue は無い

  Scenario: save_slot_spec_once はローカル実行と任意列なしを null で保存する
    Given RELAY_GATE_CONFIG_DIR/blue-job-map.csv のヘッダーが "job_id,work_dir,script,fixed_params,hang_detect_limit_minutes" で JOB001 の行がある
    When `<runner> --run-id 20260830T113000-JOB001-3f9a1c2e --job-id JOB001 --role blue --mode foreground` を環境変数 BLUE_IMPL=blue-2.9.1 で保存フェーズまで実行する
    Then slots.blue.host / user / credential_ref / map_version は JSON の null であり、slots.blue.impl_version は "blue-2.9.1" である

  Scenario: 保存フェーズは自 slot の *_IMPL が無ければ exitcode.txt=2 で終了する
    Given 正しいジョブマップがある
    When `<runner> --run-id 20260830T113000-JOB001-3f9a1c2e --job-id JOB001 --role green --mode background` を GREEN_IMPL 未設定で保存フェーズまで実行する
    Then 終了コード 2 で終了し、stderr.log に "error: option required option=GREEN_IMPL" が含まれ、execution-spec.json に slots.green は無い
  ...
  Scenario: save_slot_spec_once は同 run 同 role で再起動しても節を増やさない
    ...
    And 実行ログに "INFO execution-spec slot already exists run_id=20260830T113000-JOB001-3f9a1c2e role=green" が出る
  ...
  Scenario: save_slot_spec_once は形式に合わない credential_ref も加工せずに転記する
    Given ジョブマップの credential_ref が /home/batch/.ssh/id_green である(ジョブマップの検証では warn して受理される値)
    And 実行環境に SSH 秘密鍵 /home/batch/.ssh/id_green が存在する
    When 保存フェーズを実行する
    Then slots.green.credential_ref は "/home/batch/.ssh/id_green" であり、鍵ファイルの内容はファイル全体のどこにも含まれない

  Scenario: 保存フェーズは --execution-spec 指定時に何も書かない
    Given execution-spec.json が 2026-08-30T11:30:01(TZ=UTC 前提)の更新日時で存在する
    When `<runner> --run-id 20260830T150000-JOB001-9b8c7d6e --job-id JOB001 --role green --mode background --execution-spec <path>` を保存フェーズまで実行する
