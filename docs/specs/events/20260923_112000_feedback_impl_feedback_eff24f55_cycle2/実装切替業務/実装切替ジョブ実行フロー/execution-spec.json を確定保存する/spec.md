...
run 開始時(並行稼働実行の STARTED 遷移時)に、各 slot runner がジョブマップから解決した自 slot の実行設定(host・user・script・work_dir・fixed_params・hang_detect_limit_minutes・credential_ref・map_version)と実装版(impl_version。出所は feature flag の `BLUE_IMPL` / `GREEN_IMPL`。facade が環境変数で runner に渡す)を、run 単位で 1 ファイルの `facade/<run_id>/execution-spec.json` の `slots.<role>` 節として一度だけ確定保存する。マップ版はジョブマップの任意列 `map_version` を転記し、列が無ければ `null`。ローカル実行(host / user が無い・空)では `host` / `user` を `null` で保存する。run 単位のキー(run_id / parent_run_id / job_id / params / rapid_crosscheck_mode)は最初に書いた runner が書き、以後変更しない。既に自 slot の節があれば何もしない。認証情報は値を保存せず参照名だけを保存する。`--execution-spec <path>` 指定の復元起動では書き込まない。
...
graph LR
  subgraph FACADE["tier-facade"]
    ...
    D["domain\nExecutionSpec(run 単位キー + slots.role 節)\nSlotSpec(mode, host, user, script, work_dir,\nfixed_params, hang_detect_limit_minutes, credential_ref,\nmap_version, impl_version(BLUE_IMPL / GREEN_IMPL 由来), finalized_at)"]
...
| 認証情報の非保存 | JSON には `credential_ref`(参照名)だけを書く。relay-gate は SSH 鍵ファイルの中身や実行環境の鍵パス・パスワード・トークンを自ら読んで書くことはしない。`credential_ref` 列の値は加工せずにそのまま転記する(形式の検査・除去はしない。参照名の形式に合わない値もジョブマップの検証で warn して受理された値としてそのまま転記される。形式の扱いの正本は CLI 契約 `config_files` の `<slot>-job-map.csv` columns.credential_ref) | tier-facade | domain `SlotSpec` | 認証情報の値を保存しない(SPEC-004-03) |
...
|--------|---------|---------------|---------|----------|
| JSON 組み立て | ExecutionTarget、PARAM...、run_id、job_id、role、mode、環境変数 RAPID_CROSSCHECK_MODE / BLUE_IMPL / GREEN_IMPL | 下記スキーマ。文字列は JSON エスケープ。配列は要素順保持。jq 非依存(printf / sed で生成・節挿入)。`impl_version` は自 slot の `BLUE_IMPL`(role=blue)/ `GREEN_IMPL`(role=green)を転記。`map_version` はジョブマップの任意列を転記し列が無ければ `null`。`host` / `user` はローカル実行なら `null` | execution-spec.json 本文 | tier-facade |
...
```json
{
  ...
  "slots": {
    "blue":  {"mode": "foreground", "host": "host-blue-01", "user": "batch", "script": "/opt/legacy/bin/job001.sh", "work_dir": "/var/legacy/work", "fixed_params": [], "hang_detect_limit_minutes": 0, "credential_ref": "ssh-key-blue", "map_version": "map-v3", "impl_version": "blue-2.9.1", "finalized_at": "2026-08-30T11:30:01Z"},
    "green": {"mode": "background", "host": "host-green-01", "user": "batch", "script": "/opt/app/bin/job001.sh", "work_dir": "/var/app/work", "fixed_params": ["--mode", "full"], "hang_detect_limit_minutes": 60, "credential_ref": "ssh-key-green", "map_version": "map-v3", "impl_version": "green-1.4.0", "finalized_at": "2026-08-30T11:30:00Z"}
  }
}
```

- `run_id` は facade が発行した値(`{ローカル yyyymmddThhmmss}-{job_id}-{8 桁 hex}`。成果物ディレクトリ名・管理 DB の `parallel_runs.run_id` と同じ値)
- `rapid_crosscheck_mode` は `foreground` / `background` / `off`(facade から環境変数で渡された値)
- `slots.<role>.impl_version` は feature flag の `BLUE_IMPL` / `GREEN_IMPL`、`map_version` はジョブマップの任意列(無ければ `null`)、`host` / `user` はローカル実行なら `null`
...
| 情報 | 実行設定(execution-spec) | 保存対象。実装版の出所は feature flag の BLUE_IMPL / GREEN_IMPL、マップ版の出所はジョブマップ側の版(map_version) |
| 情報 | ジョブマップ | 解決元(前 UC)。map_version は末尾の任意列 |
| 情報 | ハング検知上限設定 | slots.<role>.hang_detect_limit_minutes |
| 情報 | 並行稼働実行(parallel_run) | execution_spec_uri が参照する。run_id は成果物ディレクトリ名と同じ値 |
...
|---|---|---|
| REQ-001 | SPEC-001-01 | run 開始時に自 slot の節を保存する(SPEC-004-03) ※ AC「実装版は BLUE_IMPL / GREEN_IMPL を出所とする」の実行側。impl_version の出所 |
| REQ-004 | SPEC-004-03 | run 開始時に自 slot の節を保存する(SPEC-004-03) / 既存の節は上書きしない(SPEC-004-03) / 認証情報の値を保存しない(SPEC-004-03) |
| REQ-004 | SPEC-004-04 | ローカル実行と任意列なしのジョブマップでは host / user / credential_ref / map_version を null で保存する(SPEC-004-04) |
| REQ-003 | SPEC-003-03 | 一時ファイル経由で確定保存する(SPEC-003-03) |
| REQ-009 | SPEC-009-02 | 復元起動では保存しない(SPEC-009-02) |
| REQ-011 | SPEC-011-04 | execution-spec.json の run_id は成果物ディレクトリ名と管理 DB の run_id と同じ値である(SPEC-011-04) |
...
Feature: execution-spec.json を確定保存する
  ...
  Scenario: run 開始時に自 slot の節を保存する(SPEC-004-03)
    Given green-job-map.csv のヘッダーが "job_id,host,user,work_dir,script,fixed_params,hang_detect_limit_minutes,credential_ref,map_version" で、JOB001 行が host=host-green-01 user=batch work_dir=/var/app/work script=/opt/app/bin/job001.sh fixed_params="[""--mode"",""full""]" hang_detect_limit_minutes=60 credential_ref=ssh-key-green map_version=map-v3 である
    And feature flag に BLUE_MODE=off GREEN_MODE=foreground RAPID_CROSSCHECK_MODE=off GREEN_IMPL=green-1.4.0 がある
    ...
    And slots.green は mode=foreground host=host-green-01 user=batch script=/opt/app/bin/job001.sh work_dir=/var/app/work fixed_params=["--mode","full"] hang_detect_limit_minutes=60 credential_ref=ssh-key-green map_version=map-v3 impl_version=green-1.4.0 を含む(impl_version は GREEN_IMPL の値)
    And slots.blue は存在しない
    And 実装スクリプトの起動より前に確定保存されている

  Scenario: ローカル実行と任意列なしのジョブマップでは host / user / credential_ref / map_version を null で保存する(SPEC-004-04)
    Given blue-job-map.csv のヘッダーが "job_id,work_dir,script,fixed_params,hang_detect_limit_minutes" で JOB001 行がある
    And feature flag に BLUE_MODE=foreground GREEN_MODE=off RAPID_CROSSCHECK_MODE=off BLUE_IMPL=blue-2.9.1 がある
    When ジョブスケジューラが facade.sh JOB001 を実行する
    Then slots.blue は host=null user=null credential_ref=null map_version=null impl_version=blue-2.9.1 を含む

  Scenario: execution-spec.json の run_id は成果物ディレクトリ名と管理 DB の run_id と同じ値である(SPEC-011-04)
    Given feature flag に BLUE_MODE=foreground GREEN_MODE=background RAPID_CROSSCHECK_MODE=background がある
    When ジョブスケジューラが facade.sh JOB001 を実行する
    Then facade/<run_id>/execution-spec.json の run_id は <run_id> と等しく、parallel_runs.run_id とも等しい
    And <run_id> は ^[0-9]{8}T[0-9]{6}-JOB001-[0-9a-f]{8}$ に一致する

  Scenario: blue と green の両方が保存すると両節が揃う
    Given feature flag に BLUE_MODE=foreground GREEN_MODE=background RAPID_CROSSCHECK_MODE=background があり両ジョブマップに JOB001 の行(blue は hang_detect_limit_minutes=0、green は 60)がある
    ...
    And run 単位キーは 1 組だけで、rapid_crosscheck_mode=background である
    And facade/<run_id>/execution-spec.lock は残っていない

  Scenario: 既存の節は上書きしない(SPEC-004-03)
    Given run_id=20260830T113000-JOB001-3f9a1c2e の execution-spec.json に slots.green が host=host-green-01 map_version=map-v3 で保存済みである
    And その後 green-job-map.csv の JOB001 行を host=host-green-02 map_version=map-v4 に変更した
    ...
    And 実行ログに "execution-spec slot already exists run_id=20260830T113000-JOB001-3f9a1c2e role=green" が出る
  ...
  Scenario: 復元起動では保存しない(SPEC-009-02)
    Given background-rerun.sh が新 run_id=20260830T150000-JOB001-9b8c7d6e のディレクトリに元の execution-spec.json をコピーしている
...
```gherkin
  Scenario: 認証情報の値を保存しない(SPEC-004-03)
    ...
    When run を開始する
    Then execution-spec.json に "ssh-key-green" は含まれるが "/home/batch/.ssh/id_green" と鍵の内容は含まれない(credential_ref が参照名なので、relay-gate が鍵パスや鍵の内容を書く経路は無い)
