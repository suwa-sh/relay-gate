...
- 形式: CSV(カンマ区切り、1 行目ヘッダー、1 行 1 job_id、文字コード UTF-8、改行コード LF)。列はヘッダー名で対応付ける(順序は元資料の並び `job_id,host,user,work_dir,script,fixed_params,hang_detect_limit_minutes` を推奨。検証は名前で行う)
  - 入力の守備範囲(バイト・行・構造の異常は exitcode.txt=2、読み込みは 1 時点の内容だけで判定、補助コマンドの失敗は exitcode.txt=6)は CLI 契約 `config_input_rules` に従う(値は複写しない)。実現手段は縛らない(ファイルを 1 回だけ読み切れば一時ファイルは不要)
  - 1 行取り出しの性能: `grep` などで JOB_ID の候補行を絞ってから該当行だけを bash で解析してよい(CLI 契約 `conventions.runtime_prerequisites.external_commands`。NFR B.2.1.1 を満たす手段)。bash の最低版は 5.0(同 `bash_minimum_version`)
...
- **実行ログ**: `INFO job map resolved job_id=... slot=... host=... map_version=...`(解決成功。ローカル実行なら `host=-`、map_version 列が無ければ `map_version=-`)/ `WARN unknown column column=... path: ...`(未知列)/ `WARN duplicate job_id in job map job_id=... lines=<n1,n2,...>`(重複行は先頭採用。lines= は validate-config.sh の重複報告と同じ規則で、その job_id が現れた全行の物理行番号を初出行を含めて出現順に並べる)/ `ERROR job_id=... not found in job map slot=... map=...`(未定義。stderr.log と同文。exitcode.txt=2)/ `ERROR option required option=--job-id`
...
| 2 | 入力・設定検証エラー | runner IF の引数不足・列挙外、ジョブマップファイルなし、入力の守備範囲の違反(NUL バイト・UTF-8 として不正なバイト列・BOM・CR。契約 `config_input_rules`)、ヘッダーの必須列欠落・host / user の片方だけ・列名の重複、CSV クォート不正、job_id 未定義、fixed_params 不正、hang_detect_limit_minutes が非負整数でない、host / user の片方だけ空、script / work_dir が空。exitcode.txt にも `2` を書く。script / work_dir のパスの形式(絶対 / 相対 / Windows 形式)は違反にしない |
| 6 | 実行エラー | 成果物ディレクトリへの書き込み失敗(3 ファイルを揃えられない場合。実行ログに ERROR)、ジョブマップ読み込みの補助コマンドの失敗(契約 `config_input_rules.internal_failure`。stderr.log に `error: internal command failed commands=... path: ...`、exitcode.txt=6) |
...
| 固定引数不正 | `error: fixed_params is not a json array of strings line=3 job_id=JOB003 value=p1 p2` |
...
| script / work_dir が空 | `error: script is empty line=2 job_id=JOB004 value=`(形式は検査しない。`G:\scripts\xxx.bat` / `./beam-batches` も受理して実行先へそのまま渡す) |
| 形式から外れた入力(守備範囲) | `error: nul byte is not allowed line=2 path: ...` / `error: encoding is not utf-8 line=N path: ...` / `error: byte order mark is not allowed line=1 path: ...` / `error: carriage return is not allowed line=N path: ...`(契約 `config_input_rules` と同文) |
| 必須オプション欠落 | `error: option required option=--job-id`(横断規約の定型文。run_id / role が不明なら成果物は書けないため実行ログにのみ残す) |

## UC ロジック

- **バリデーション**: (1) runner IF の必須オプション → (2) ジョブマップファイルの存在と読み取り権限 → (3) ヘッダー(必須列 5 の存在、host / user は両方あるか両方無い、未知列は warn)→ (4) CSV セルのクォート整合(1 文字ずつの状態機械)→ (5) job_id 行の存在 → (6) host / user の空は両方同時のみ → (7) `fixed_params` が JSON 配列 → (8) `hang_detect_limit_minutes` が `^[0-9]+$` → (9) script / work_dir が非空(パスの形式は検査しない。Windows 形式・相対パスも受理。利用者決定 2026-09-22: ジョブマップは Windows / Linux 両対応のフルパス / 相対パス指定を受け取れる必要がある)。(2) の直後にバイト点検(NUL / UTF-8 / BOM / CR。契約 `config_input_rules`)を行い、違反は解析せず exitcode.txt=2。JSON 配列の判定・要素抽出は UC「slot ごとのジョブマップを定義する」tier-facade の「JSON 配列の判定」と同じ実装を共有し、bash 単独で行う(契約 `conventions.runtime_prerequisites.external_commands`。jq 非依存。ネストした配列・オブジェクトは非対応で検証エラー)
- **確認プロンプト**: なし
