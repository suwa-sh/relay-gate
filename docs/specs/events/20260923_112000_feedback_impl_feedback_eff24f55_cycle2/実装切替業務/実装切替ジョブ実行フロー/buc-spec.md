...
ジョブスケジューラの業務ジョブ定義を変更せずに、facade が feature flag 設定で選んだ slot(blue / green)の runner を foreground / background で起動し、runner がジョブマップで解決した実行先で実装スクリプトを実行して Runner Result Contract の成果物を残す。facade は foreground slot の結果だけを無加工でジョブスケジューラへ中継し、運用者は並行稼働中も単独本番中も同じ見え方で業務ジョブの成否を判定する。run_id(`{ローカル yyyymmddThhmmss}-{job_id}-{8 桁 hex}`)はモードによらず facade が発行し、速報クロスチェック有効時(RAPID_CROSSCHECK_MODE が foreground / background)だけ run_id で相関付けた parallel_run / slot 実行 / rapid_run を relay-gate 内部のジョブキュー兼管理 DB に記録する。slot 実行の状態は成果物ファイル(exitcode.txt / aborted.txt)から導出する。管理 DB slot_executions.status は runner / abort-* の条件付き UPDATE(WHERE status='RUNNING')で一度だけ終端値を書く列で、abort 後に実装が走り切って exitcode.txt を公開した経路だけ導出値(exitcode.txt 優先)と一致せず ABORTED のまま残る(再同期しない。正本: rdb-schema.yaml slot_executions.status)。

## 所属 UC 一覧

| UC名 | アクター | 主な操作 | 関連情報 |
|------|---------|---------|---------|
| [slot 実行モードを選択して runner を起動する](slot%20実行モードを選択して%20runner%20を起動する/spec.md) | 運用者(受益者)/ ジョブスケジューラ(起動元) | feature flag(9 キー)を読み込み、両 slot foreground を拒否し、background → foreground の順に runner を起動して foreground の PID だけを待機する。run_id は常に発行し、RAPID_CROSSCHECK_MODE が off 以外のとき速報クロスチェック設定(rapid-crosscheck.env)の管理 DB 接続参照名で接続し parallel_run を STARTED → RUNNING にする | ジョブ起動要求、feature flag 設定、slot runner 割当、速報クロスチェック設定、並行稼働実行(parallel_run)、slot 実行、速報実行(rapid_run)、実行ログ |
| [ジョブマップで JOB_ID から実行先を解決する](ジョブマップで%20JOB_ID%20から実行先を解決する/spec.md) | 運用者(受益者) | slot runner が自 slot のジョブマップ(CSV)から JOB_ID の実行先(host・user・script・work_dir・fixed_params・hang_detect_limit_minutes。host / user はローカル実行では省略可)を解決し、固定引数の後ろに PARAM... を連結する。未定義なら 3 ファイルを揃えて非 0 終了 | ジョブ起動要求、ジョブマップ、ハング検知上限設定、Runner Result |
...
| [実装スクリプトを実行して Runner Result を出力する](実装スクリプトを実行して%20Runner%20Result%20を出力する/spec.md) | 運用者(受益者)/ リモート実行ホスト・現行実装・新実装(外部) | 実行先ホストへ SSH 接続(ローカル実行では直接起動)して実装スクリプトを実行し、started-at.txt / stdout.log / stderr.log / exitcode.txt を原子的に出力する。状態はファイル正本から導出し exitcode 0 で SUCCEEDED、非 0 で FAILED(exitcode.txt が無く aborted.txt があれば ABORTED)。background slot リラン由来 run は終端時に parallel_run を COMPLETED にする | 実行設定(execution-spec)、slot 実行、Runner Result、並行稼働実行(parallel_run)、実行ログ |
| [foreground slot の結果をジョブスケジューラへ中継する](foreground%20slot%20の結果をジョブスケジューラへ中継する/spec.md) | 運用者(受益者)/ ジョブスケジューラ(応答先) | foreground の 3 ファイルを標準出力・標準エラー・終了コードとして無加工で中継する。background と速報の結果は含めず待たない。RAPID_CROSSCHECK_MODE が off 以外のとき parallel_run を COMPLETED にする | Runner Result、ジョブスケジューラ応答、並行稼働実行(parallel_run)、slot 実行 |
...
graph LR
  ...
  CFG -->|"feature flag 設定 / slot runner 割当 / 速報クロスチェック設定(off 以外)"| UC1
  ...
  UC4 -->|"Runner Result + slot 実行(SUCCEEDED / FAILED)/ 並行稼働実行(リラン由来 run の COMPLETED)"| RAPID
...
| feature flag 設定 | R | - | - | - | - | - |
| slot runner 割当 | R | - | - | - | - | - |
| 速報クロスチェック設定 | R(off 以外。RAPID_DB_CONN_REF のみ) | - | - | - | - | - |
| ジョブマップ | - | R | R | - | - | - |
| ハング検知上限設定 | - | R | R | - | - | - |
| 実行設定(execution-spec) | - | - | C | R | - | - |
| 並行稼働実行(parallel_run) | C / U | - | R(間接) | U(リラン由来 run の COMPLETED) | U | R(間接) |
| slot 実行 | C / U | - | - | U | R | - |
| 速報実行(rapid_run) | C | - | - | - | - | - |
...
BUC 内の UC が遷移 UC になっている状態モデルは、並行稼働実行・slot 実行・速報実行の完了状況の 3 つ。STARTED / RUNNING → ABORTED(実行復旧業務。slot 実行の ABORTED は aborted.txt で記録)、両系未完了以降の完了状況の遷移(速報クロスチェックフロー)、background-rerun による `[*]` → STARTED / RUNNING(実行復旧業務)、速報比較依頼だけを新規作成したリラン由来 run の RUNNING → COMPLETED(速報クロスチェックフローの UC「比較ツールでジョブ単位比較を実行して結果を登録する」)は別 BUC の UC が担うため図に含めない。

```mermaid
stateDiagram-v2
  state "並行稼働実行(RAPID_CROSSCHECK_MODE が off 以外のみ)" as PR {
    [*] --> STARTED: slot 実行モードを選択して runner を起動する
    STARTED --> RUNNING: slot 実行モードを選択して runner を起動する
    RUNNING --> COMPLETED: foreground slot の結果をジョブスケジューラへ中継する(facade 起動の run)
    RUNNING --> COMPLETED: 実装スクリプトを実行して Runner Result を出力する(background slot リラン由来 run)
    COMPLETED --> [*]
  }
  state "slot 実行(状態はファイル正本 exitcode.txt / aborted.txt から導出)" as SE {
    state "RUNNING" as SE_RUNNING
    [*] --> SE_RUNNING: slot 実行モードを選択して runner を起動する
    SE_RUNNING --> SUCCEEDED: 実装スクリプトを実行して Runner Result を出力する(exitcode.txt=0)
    SE_RUNNING --> FAILED: 実装スクリプトを実行して Runner Result を出力する(exitcode.txt≠0)
    SUCCEEDED --> [*]
    FAILED --> [*]
  }
  state "速報実行の完了状況(off 以外のみ)" as RR {
...
| 並行稼働実行 | RUNNING | COMPLETED | [foreground slot の結果をジョブスケジューラへ中継する](foreground%20slot%20の結果をジョブスケジューラへ中継する/spec.md)(facade 起動の run) |
| 並行稼働実行 | RUNNING | COMPLETED | [実装スクリプトを実行して Runner Result を出力する](実装スクリプトを実行して%20Runner%20Result%20を出力する/spec.md)(background slot リラン由来 run。slot の終端時) |
...
| 速報クロスチェック有効判定 | RAPID_CROSSCHECK_MODE が foreground または background のとき runner は完了通知を送信し、速報管理 DB に書き込み、parallel_run を作成・更新する(両値は同じ挙動)。off のときは完了通知を送信せず管理 DB へ接続も書き込みもせず、parallel_run も作成しない | slot 実行モードを選択して runner を起動する, 実装スクリプトを実行して Runner Result を出力する(spec), foreground slot の結果をジョブスケジューラへ中継する(spec) |
| slot 実行の状態導出規則 | slot 実行の状態は成果物ファイルから導出する。exitcode.txt があれば 0 で SUCCEEDED / 非 0 で FAILED、無く aborted.txt があれば ABORTED、どちらも無ければ RUNNING。速報クロスチェック有効時は管理 DB の slot_executions.status に aborted.txt / exitcode.txt 公開時点の状態を条件付き更新(RUNNING のときだけ)で一度だけ書き、abort-blue / abort-green で ABORTED にした後に実装が走り切って exitcode.txt を公開しても ABORTED のまま残してファイル正本の導出値へ再同期しない。管理 DB の値は条件「中止済み run の比較依頼作成除外」の判定材料に使う(BUC.tsv 上の適用 UC は実行復旧業務 / 実行監視業務。本 BUC では Runner Result を出力する UC が spec で採用) | 実装スクリプトを実行して Runner Result を出力する(spec) |
...
| 速報クロスチェックモード | foreground、background、off | slot 実行モードを選択して runner を起動する, 実装スクリプトを実行して Runner Result を出力する, foreground slot の結果をジョブスケジューラへ中継する |
| Runner Result 成果物種別 | started-at.txt、stdout.log、stderr.log、exitcode.txt、aborted.txt(abort-blue / abort-green が中止時のみ生成) | ジョブマップで JOB_ID から実行先を解決する, 実装スクリプトを実行して Runner Result を出力する, foreground slot の結果をジョブスケジューラへ中継する, 業務ジョブの実行結果を確認する |
| 設定所有区分 | feature flag、slot ジョブマップ、速報クロスチェック設定(所有者: 基盤適用設計者) | slot 実行モードを選択して runner を起動する, ジョブマップで JOB_ID から実行先を解決する |
