...
| [両系成功時に速報比較依頼を作成する](両系成功時に速報比較依頼を作成する/spec.md) | 運用者(受益者・自動) | 完了状況を 両系未完了 → 片系完了 → 両系成功 / いずれか失敗 → 比較依頼作成済み へ進め、両系成功のときだけ条件付き INSERT で rapid_crosscheck_request を 1 件作成する。中止済みの run(並行稼働実行が ABORTED、または完了通知の対象 slot の slot 実行が ABORTED。判定材料は parallel_run.status と slot_executions.status)では両系成功でも作成せず、完了事実だけを記録して実行ログに警告を残す(中止済み run の比較依頼作成除外。判断主体は dispatcher) | 完了通知、速報実行(rapid_run)、速報比較依頼(rapid_crosscheck_request)、並行稼働実行(parallel_run)、slot 実行、速報クロスチェック設定、実行ログ |
...
graph LR
  ...
  EXEC -->|"parallel_run.status / 対象 slot の slot 実行の status(run が中止済み = parallel_run ABORTED または対象 slot の slot 実行 ABORTED なら依頼を作らない)"| UC2
...
| slot 実行 | R | R(slot_executions.status。完了通知の対象 slot の行だけ SELECT。ABORTED なら依頼を作らない) | - | - | - |
...
BUC 内の UC が遷移 UC になっている状態モデルは、速報実行の完了状況、クロスチェック依頼(速報比較依頼)、並行稼働実行(リラン由来 run の RUNNING → COMPLETED のみ)の 3 つ。`[*]` → 両系未完了 は実装切替ジョブ実行フロー、background-rerun による依頼の再作成(並行稼働実行の `[*]` → STARTED → RUNNING)は実行復旧業務が担うため図に含めない。ただし本 BUC が作成した速報比較依頼が ABORTED になる 3 経路、すなわち未着手(REQUESTED)の依頼が abort-blue / abort-green で ABORTED になる遷移(競合窓の保険)、claim 済み(CLAIMED)の依頼が abort-rapid-crosscheck で ABORTED になる遷移(claim 済み worker は CLAIMED → RUNNING の条件付き UPDATE が 0 件になり比較を開始しない)、比較中(RUNNING)の依頼が abort-rapid-crosscheck で ABORTED になる遷移(終端 UPDATE が 0 件になり comparison_results を登録しない)は、いずれも UC「実行を ABORTED へ遷移させる」(実行中止フロー)が担うが、本 BUC の依頼ライフサイクルの終端として図に示す。run が中止済み(並行稼働実行 ABORTED、または完了通知の対象 slot の slot 実行 ABORTED)では「両系成功 → 比較依頼作成済み」の遷移は起きず、両系成功に留まる(条件「中止済み run の比較依頼作成除外」)。
...
stateDiagram-v2
  state "速報実行の完了状況" as RR {
    ...
    両系成功 --> 比較依頼作成済み: 両系成功時に速報比較依頼を作成する(中止済み run = parallel_run ABORTED または対象 slot の slot 実行 ABORTED では遷移せず両系成功に留まる)
    比較依頼作成済み --> [*]
    いずれか失敗 --> [*]
  }
  state "クロスチェック依頼(速報比較依頼)" as REQ {
    ...
    REQUESTED --> ABORTED: 実行を ABORTED へ遷移させる(別 BUC 実行中止フロー。abort-blue / abort-green の競合窓の保険、または abort-rapid-crosscheck)
    CLAIMED --> ABORTED: 実行を ABORTED へ遷移させる(別 BUC 実行中止フロー。abort-rapid-crosscheck。claim 済み worker は比較を開始しない)
    CLAIMED --> RUNNING: 比較ツールでジョブ単位比較を実行して結果を登録する(条件付き UPDATE。0 件なら比較を開始しない)
    RUNNING --> ABORTED: 実行を ABORTED へ遷移させる(別 BUC 実行中止フロー。abort-rapid-crosscheck。比較中の中止。終端 UPDATE が 0 件になり comparison_results を登録しない)
...
| 速報実行の完了状況 | 両系成功 | 比較依頼作成済み | [両系成功時に速報比較依頼を作成する](両系成功時に速報比較依頼を作成する/spec.md)(run が中止済み(並行稼働実行 ABORTED、または完了通知の対象 slot の slot 実行 ABORTED)では遷移せず両系成功に留まる) |
...
| クロスチェック依頼 | REQUESTED | ABORTED(abort-blue / abort-green による未着手の速報比較依頼の中止。条件付き UPDATE status='REQUESTED'。0 件は正常。両系の完了通知で依頼が作成された直後に slot を中止した場合の競合窓を塞ぐ保険で、dispatcher 側の中止済み run 判定と併用する。または abort-rapid-crosscheck による明示中止。速報比較依頼のみ) | [実行を ABORTED へ遷移させる](../../実行復旧業務/実行中止フロー/実行を%20ABORTED%20へ遷移させる/spec.md)(別 BUC 実行中止フロー) |
| クロスチェック依頼 | CLAIMED | ABORTED(abort-rapid-crosscheck による claim 済み速報比較依頼の中止。停止確認 yes 後の条件付き UPDATE status IN ('REQUESTED','CLAIMED','RUNNING')。claim 済み worker は RUNNING への条件付き UPDATE が 0 件になり比較を開始しない。速報比較依頼のみ) | [実行を ABORTED へ遷移させる](../../実行復旧業務/実行中止フロー/実行を%20ABORTED%20へ遷移させる/spec.md)(別 BUC 実行中止フロー) |
| クロスチェック依頼 | CLAIMED | RUNNING(条件付き UPDATE status='CLAIMED' AND worker_id=自 worker。0 件なら比較を開始せず comparison_results も登録しない) | [比較ツールでジョブ単位比較を実行して結果を登録する](比較ツールでジョブ単位比較を実行して結果を登録する/spec.md) |
| クロスチェック依頼 | RUNNING | ABORTED(abort-rapid-crosscheck による比較中の速報比較依頼の中止。停止確認 yes 後の条件付き UPDATE status IN ('REQUESTED','CLAIMED','RUNNING')。worker の終端 UPDATE が 0 件になり comparison_results を登録しない。速報比較依頼のみ) | [実行を ABORTED へ遷移させる](../../実行復旧業務/実行中止フロー/実行を%20ABORTED%20へ遷移させる/spec.md)(別 BUC 実行中止フロー) |
...
| 条件名 | 条件の説明 | 適用 UC |
|--------|----------|--------|
| 依頼状態遷移規則 | 依頼は REQUESTED で作成され、claim で CLAIMED、比較開始で RUNNING(条件付き UPDATE status='CLAIMED' AND worker_id=自 worker。速報は 0 件なら比較を開始しない)、exitcode 0 で SUCCEEDED、非 0 または実行エラーで FAILED、停止確認後の中止で ABORTED に遷移する(速報比較依頼は REQUESTED / CLAIMED / RUNNING から abort-rapid-crosscheck で ABORTED。未着手 REQUESTED の速報比較依頼は abort-blue / abort-green でも ABORTED になる(競合窓の保険)。確報比較依頼は RUNNING からのみ abort-final-crosscheck で ABORTED)。速報と確報で状態集合は同一 | 両系成功時に速報比較依頼を作成する, 速報比較依頼を claim する, 比較ツールでジョブ単位比較を実行して結果を登録する |
| 中止済み run の比較依頼作成除外 | 並行稼働実行(parallel_run)が ABORTED、**または**完了通知の対象 slot の slot 実行が ABORTED(成果物ディレクトリに aborted.txt が公開済み)のいずれかに該当する run を中止済み run とし、速報クロスチェック runner(dispatcher)は完了通知を受けても速報比較依頼を作成しない。並行稼働実行は foreground slot の結果を中継した時点で COMPLETED になるため、foreground 完了後に background slot を abort-blue / abort-green で中止した典型経路は slot 実行 ABORTED の側で除外する。判定材料は管理 DB の parallel_run.status と slot_executions.status(ファイル正本 aborted.txt のミラー)で、同一トランザクションで読む(parallel_runs は FOR UPDATE で読み、abort-blue / abort-green の parallel_runs 無条件行ロック(`SELECT 1 ... FOR UPDATE`。parallel_runs が COMPLETED でも取る)と直列化する。対象 slot の slot_executions は SELECT のみ)。判定表は縦軸 3 値(並行稼働実行 ABORTED / 対象 slot の slot 実行 ABORTED / いずれでもない)× 横軸両系成功判定。完了事実(blue_status / green_status と成果物 URI)は rapid_run に記録し、実行ログに警告を残す(reason=parallel_run_aborted / slot_execution_aborted)。判断主体は速報クロスチェック runner であり、slot runner は自 slot の中止状態を判断せず通常どおり完了通知を送る。RAPID_CROSSCHECK_MODE=off では完了通知も比較依頼も存在しないため速報有効時だけに適用する。abort-blue / abort-green の REQUESTED → ABORTED は競合窓の保険であり本条件と併用する | 両系成功時に速報比較依頼を作成する |
| 依頼中止可否判定 | 速報比較依頼は REQUESTED / CLAIMED / RUNNING のいずれかのとき abort-rapid-crosscheck で ABORTED へ遷移できる。確報比較依頼は RUNNING のときだけ abort-final-crosscheck で ABORTED へ遷移できる。中止できる状態でない場合は状態を変更せずエラー終了する(運用者側の判定は UC「実行を ABORTED へ遷移させる」)。CLAIMED の速報比較依頼を中止する際の競合規則: abort-rapid-crosscheck は停止確認 yes のときだけ status IN (REQUESTED, CLAIMED, RUNNING) を条件とする条件付き UPDATE で ABORTED にし、速報クロスチェック worker は claim 後・比較開始前の CLAIMED → RUNNING を条件付き UPDATE で行い、更新件数が 0 件なら比較を開始せず comparison_results も登録しない | 比較ツールでジョブ単位比較を実行して結果を登録する |
...
| 速報クロスチェックモード | foreground、background、off(foreground / background は同じ挙動) | 速報クロスチェック runner へ完了通知を送信する, 両系成功時に速報比較依頼を作成する, 速報比較依頼を claim する, 速報比較結果を参照する |
