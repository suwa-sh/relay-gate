...
| REQ-005 | SPEC-005-04 | blue の完了通知が rapid_runs に登録される(SPEC-005-01) / RAPID_CROSSCHECK_MODE=foreground でも background と同じく通知する(SPEC-005-04) / RAPID_CROSSCHECK_MODE=off では通知しない(SPEC-005-04) ※ 1 件目は background で通知・DB 書き込みを行う経路 |
