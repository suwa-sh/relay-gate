...
|---|---|---|
| REQ-010 | SPEC-010-01 | background かつ RUNNING の green を中止する(SPEC-010-01 / SPEC-010-03) / foreground の blue は中止できない(SPEC-010-01) / RAPID_CROSSCHECK_MODE=off でも aborted.txt を書いて中止が成立する(SPEC-010-01) / 既に ABORTED の slot への再実行は中止不可として拒否する(SPEC-010-01) / REQUESTED の速報比較依頼がある run を中止すると依頼も ABORTED になる(SPEC-010-01) / CLAIMED または RUNNING の速報比較依頼は slot の中止では変更されない(SPEC-010-01) / 依頼作成直後に slot を中止すると依頼も ABORTED になり完了通知で再作成されない(SPEC-010-01) ※ 受け入れ条件 8 件の対応: AC1 / AC5 = background かつ RUNNING の green を中止する、AC2 = foreground の blue は中止できない、AC3 = RAPID_CROSSCHECK_MODE=off でも aborted.txt を書いて中止が成立する、AC4 = 既に ABORTED の slot への再実行は中止不可として拒否する(導出 ABORTED。ハング検知側の終端は UC「background 実行の経過時間と終了状態を判定する」)、AC6 = REQUESTED の速報比較依頼がある run を中止すると依頼も ABORTED になる、AC7 = CLAIMED または RUNNING の速報比較依頼は slot の中止では変更されない、AC8 = 依頼作成直後に slot を中止すると依頼も ABORTED になり完了通知で再作成されない |
| REQ-010 | SPEC-010-02 | RUNNING の速報比較依頼を中止する(SPEC-010-02) / REQUESTED または CLAIMED の速報比較依頼を中止すると worker は比較を開始しない(SPEC-010-02) / 終端状態の速報比較依頼は状態を変更せずエラー終了する(SPEC-010-02) / RUNNING でない確報比較依頼は状態を変更せずエラー終了する(SPEC-010-02) ※ 受け入れ条件 5 件の対応: AC1 = RUNNING の速報比較依頼を中止する、AC2 = REQUESTED または CLAIMED の速報比較依頼を中止すると worker は比較を開始しない、AC4 = 終端状態の速報比較依頼は状態を変更せずエラー終了する、AC5 = RUNNING でない確報比較依頼は状態を変更せずエラー終了する。AC3(worker 側の RUNNING 条件付き UPDATE 0 件)は UC「比較ツールでジョブ単位比較を実行して結果を登録する」で覆う |
