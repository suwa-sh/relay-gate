...
| 名前 | 型 | 必須 | 既定値 | 説明 |
|------|---|------|-------|------|
| `--run-id` | string | Yes | なし | 対象 run_id(`{ローカル yyyymmddThhmmss}-{job_id}-{8 hex}`)。`abort-final-crosscheck.sh` では値を `final_crosscheck_id` として解釈する(方針資料が 4 スクリプトとも `--run-id` を用いるため引数名を揃える)。`abort-blue.sh` / `abort-green.sh` では、対象 run に REQUESTED で未着手の速報比較依頼があればそれも ABORTED にする(競合窓の保険。条件「slot 中止可否判定」。後半 UC)。`abort-rapid-crosscheck.sh` は依頼が REQUESTED / CLAIMED / RUNNING のとき、`abort-final-crosscheck.sh` は RUNNING のとき ABORTED へ(条件「依頼中止可否判定」。後半 UC) |
...
- **終了コード**(4 スクリプト共通。後半の中止不可 3 / DB 失敗 6 は後半 UC に記載):
  | コード | 意味 | 条件 |
  |-------|------|------|
  | 0 | 成功 | yes 応答後に後半の状態更新が完了した(abort-blue / abort-green: RAPID_CROSSCHECK_MODE が off 以外なら加えて `rapid_crosscheck_requests` を条件付き UPDATE(`WHERE run_id=? AND status='REQUESTED'`)で ABORTED(completed_at=now)にする。0 件は正常(依頼が無い、または CLAIMED / RUNNING / 終端済み)。競合窓の保険で dispatcher 側の中止済み run 判定と併用。CLAIMED / RUNNING の依頼は変更しない。abort-rapid-crosscheck: REQUESTED / CLAIMED / RUNNING の速報比較依頼を status IN 条件付き UPDATE で ABORTED。abort-final-crosscheck: RUNNING のみ。詳細は後半 UC) |
...
- **確認プロンプト**: 現在状態を stdout に出し切ってから、stderr にプロンプトを出し stdin を 1 行読む。依頼系は現在状態が REQUESTED / CLAIMED / RUNNING のいずれでもプロンプトを出す(REQUESTED の速報比較依頼には worker プロセスが無いが確認は省略しない)。`yes`(小文字完全一致)のみ肯定。空 Enter・`y`・`YES`・`no` は否定(意図的な壁)。`--yes` でプロンプト省略。非 TTY(`[ ! -t 0 ]`)かつ `--yes` なしは `error: interactive confirmation required (use --yes for non-interactive)`、終了コード 2(現在状態も出さずに終了する)
