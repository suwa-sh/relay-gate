...
事前検証(UC「リラン対象を検証する」)を通過した `--role rapid-crosscheck` のリランについて、background-rerun が業務ジョブを再実行せず、新しい run_id を発行して `parallel_runs`(parent_run_id = 元 run_id)、`rapid_runs`(元 run の blue / green の成果物 URI を引き継ぎ、完了状況は BOTH_SUCCEEDED → REQUEST_CREATED)、`rapid_crosscheck_requests`(新 run_id 主キー、job_id は元 run のもの、REQUESTED)を 1 トランザクションで作成する。作成後は速報クロスチェック worker の通常の claim と比較実行に委ねる。この parallel_run は foreground slot を持たないため、依頼が終端状態(SUCCEEDED / FAILED)になった時点で速報クロスチェック worker が COMPLETED にする(他 UC「比較ツールでジョブ単位比較を実行して結果を登録する」)。本 UC で作られた run は `execution-spec.json` を持たないが、前段 UC「リラン対象を検証する」は `--role rapid-crosscheck` で `execution-spec.json` を要求せず依頼レコードだけで元実行を特定するため、この run を再度 `--source-run-id` に指定して数珠つなぎにリランできる(parent_run_id は直前のリラン元を指す)。tier-rapid-crosscheck 側は依頼レコードの作成契約(rapid 側テーブルへの書き込み規則)を担う。
...
| ops presentation(出力) | `role=rapid-crosscheck` / `job_id` / `request_status=REQUESTED` / `blue_artifact_uri:` / `green_artifact_uri:` / `run_id=` / `parent_run_id=`(契約の固定順) | 追跡の起点を渡す |
...
sequenceDiagram
  ...
  alt 元 rapid_runs が無い / artifact_uri が空
    ...
    Domain-->>UC: R2 = 20260830T130000-JOB001-a1b2c3d4
    ...
    Repo->>GW: COMMIT
    UC-->>Pres: 作成結果
    Pres-->>Sched: stdout role=rapid-crosscheck / job_id / request_status=REQUESTED / blue_artifact_uri / green_artifact_uri / run_id=R2 / parent_run_id=R1、終了コード 0
  end
  Note over DB,Worker: 以降は worker の通常 poll / claim(REQUESTED → CLAIMED → RUNNING → SUCCEEDED / FAILED)に委ねる(他 UC)。依頼の終端時に worker は parent_run_id 非 null かつ slot_executions 0 件の parallel_runs を RUNNING → COMPLETED にする
```
...
| 速報クロスチェックモード | foreground / background | 管理 DB がある前提で本 UC に到達する | tier-ops | 前段 UC |
| 速報クロスチェックモード | off | 元依頼が存在せず前段 UC で拒否(終了コード 3) | tier-ops | 前段 UC |
...
|--------|---------|---------------|---------|----------|
| 新 run_id | now(`RELAY_GATE_NOW` 設定時はその値)、元 parallel_runs.job_id、乱数 | `{ローカル yyyymmddThhmmss}-{job_id}-{8 hex}`(時刻部はプロセスのローカルタイムゾーンへ変換した値。タイムゾーン指示子なし。BDD の例示は TZ=UTC 前提)。既存の run_id と衝突すれば取り直す(最大 3 回)。末尾 8 桁は乱数のため BDD の Then は形式 `^[0-9]{8}T[0-9]{6}-{job_id}-[0-9a-f]{8}$` と時刻部で検証し、固定値を要求しない | run_id | tier-ops |
...
| 指示日時 | now(ローカル時刻。`RELAY_GATE_NOW` 設定時はその値) | 情報「リラン指示」の属性「指示日時」= 新 run の `parallel_runs.requested_at` および実行ログ行(`INFO rapid request recreated`)の ローカル時刻列 | requested_at / 実行ログ | tier-ops |
...
| 並行稼働実行 | STARTED | RUNNING | 速報比較依頼を REQUESTED で作成した | 依頼 INSERT 成功(同一トランザクション) | 以降、依頼が終端状態(SUCCEEDED / FAILED)になった時点で速報クロスチェック worker が RUNNING → COMPLETED にする(遷移 UC「比較ツールでジョブ単位比較を実行して結果を登録する」。判定規則: `parallel_runs.parent_run_id IS NOT NULL` かつ当該 run_id の `slot_executions` 行が 0 件。条件付き UPDATE `WHERE status='RUNNING'`)。依頼が RUNNING の間に中止されれば実行中止フローが ABORTED にする | tier-ops |

運用注記(UC「実行を ABORTED へ遷移させる」と共通):

- 本 UC で作成した parallel_runs は foreground 中継が無いため、依頼が SUCCEEDED / FAILED になった時点で worker が COMPLETED にする(`run-lineage.sh` に COMPLETED として現れる)。依頼が RUNNING の間に `abort-rapid-crosscheck.sh` で中止した場合は、併更新(`WHERE status IN ('STARTED','RUNNING')`)で ABORTED になる(状態「並行稼働実行」の RUNNING → COMPLETED / RUNNING → ABORTED)
- 成果物ディレクトリ: background-rerun 自身は `facade/<新 run_id>/` を作らない。比較実行時に worker が `facade/<新 run_id>/rapid-crosscheck/{started-at.txt, stdout.log, stderr.log, exitcode.txt}` を作る(他 UC「比較ツールでジョブ単位比較を実行して結果を登録する」)。この run ディレクトリは `execution-spec.json` と blue / green 節を持たない。hang-detector(他 UC「background 実行の経過時間と終了状態を判定する」tier-ops.md 走査手順 1)は `<role>/started-at.txt`(role ∈ {blue, green})が無い run ディレクトリを `execution-spec.json` を読まずに飛ばすため、新 run_id の slot 監視は発生せず、依頼の監視は管理 DB の依頼走査(off 以外)で行われる
...
| 状態 | 並行稼働実行 | 初期 → STARTED → RUNNING(以降、依頼の終端時に worker が COMPLETED にする。他 UC) |
...
| REQ-009 | SPEC-009-01 | ABORTED の速報比較依頼を新しい依頼として再作成する(SPEC-009-03) / 再作成した run をさらに再作成して系譜を伸ばす(SPEC-009-01) ※ 1 件目で parent_run_id の設定を検証する |
| REQ-011 | SPEC-011-06 | 再作成した依頼が終端すると parallel_runs は COMPLETED になる |
...
Feature: 速報比較依頼だけを新規作成する
  ...
  Scenario: ABORTED の速報比較依頼を新しい依頼として再作成する(SPEC-009-03)
    Given RAPID_CROSSCHECK_MODE=background で RELAY_GATE_NOW=2026-08-30T13:00:00Z であり、run_id=20260830T113000-JOB001-3f9a1c2e(R1)の rapid_crosscheck_requests が status=ABORTED、rapid_runs の blue_artifact_uri=file:///var/relay-gate/facade/20260830T113000-JOB001-3f9a1c2e/blue green_artifact_uri=file:///var/relay-gate/facade/20260830T113000-JOB001-3f9a1c2e/green、parallel_runs.job_id=JOB001 である
    When ジョブスケジューラが `background-rerun.sh --source-run-id 20260830T113000-JOB001-3f9a1c2e --role rapid-crosscheck` を起動する
    Then 終了コード 0 で stdout の最終 2 行が `run_id={新 run_id}` と `parent_run_id=20260830T113000-JOB001-3f9a1c2e` であり、{新 run_id}(R2)は正規表現 `^20260830T130000-JOB001-[0-9a-f]{8}$` に一致し、それより前に `request_status=REQUESTED` がある
    And rapid_crosscheck_requests に run_id={新 run_id} job_id=JOB001 status=REQUESTED worker_id=NULL lease_until=NULL requested_at=2026-08-30T13:00:00 がある
    And rapid_runs に run_id={新 run_id} blue_artifact_uri / green_artifact_uri が R1 と同じ値で completion_status=REQUEST_CREATED がある
    And parallel_runs に run_id={新 run_id} parent_run_id=20260830T113000-JOB001-3f9a1c2e status=RUNNING がある
  ...
  Scenario: 再作成した依頼が終端すると parallel_runs は COMPLETED になる
    Given 上記で作成された run_id={新 run_id}(R2)の依頼を worker-01 が claim して比較を実行した
    When 比較ツールが終了し worker が依頼を SUCCEEDED(または FAILED)にする
    Then parallel_runs の run_id={新 run_id} の status が RUNNING から COMPLETED になり completed_at が設定される(遷移 UC「比較ツールでジョブ単位比較を実行して結果を登録する」。parent_run_id 非 null かつ slot_executions 0 件の run が対象)
    And `run-lineage.sh --run-id {新 run_id}` の run_status 列が COMPLETED で出る
  ...
  Scenario: 再作成した run をさらに再作成して系譜を伸ばす(SPEC-009-01)
    Given RAPID_CROSSCHECK_MODE=background で {R2}(上記シナリオで再作成された新 run_id。`^20260830T130000-JOB001-[0-9a-f]{8}$` に一致)は R1 から `--role rapid-crosscheck` で再作成された run であり、facade/{R2}/execution-spec.json は存在しない
    And R2 の rapid_crosscheck_requests が status=FAILED、rapid_runs の blue_artifact_uri / green_artifact_uri が R1 と同じ値、parallel_runs.parent_run_id=20260830T113000-JOB001-3f9a1c2e である
    ...
    Then 終了コード 0 で stdout の最終行が `parent_run_id={R2}` であり、最終 2 行目の `run_id={R3}` は `^20260830T133000-JOB001-[0-9a-f]{8}$` に一致する
...
```gherkin
  Scenario: 元 run の成果物 URI が無い
    ...
    When `background-rerun.sh --source-run-id 20260830T113000-JOB001-3f9a1c2e --role rapid-crosscheck` を起動する
    Then 終了コード 3 で stderr に `error: source artifacts not found run_id=20260830T113000-JOB001-3f9a1c2e role=rapid-crosscheck missing=green_artifact_uri` が出る
    And 新しいレコードは作成されない
