...
| Runner Result 成果物種別 | started-at.txt | 起動時に UTC ISO 8601 秒精度 Z 付きを 1 行(Runner Result Contract。中身だけは UTC のまま。表示・DB 転記時はローカルへ変換) | tier-facade | repository `runner_result_repo` |
...
| slot 実行の状態導出規則 | 状態はファイル正本から導出する: exitcode.txt があれば 0 で SUCCEEDED / 非 0 で FAILED。exitcode.txt が無く aborted.txt があれば ABORTED。どちらも無ければ RUNNING。両方あれば exitcode.txt を優先(中止後に実装が終了した場合)。RAPID_CROSSCHECK_MODE が off 以外なら同条件の管理 DB 側の規則に従い、管理 DB の slot_executions.status は runner / abort-* が aborted.txt または exitcode.txt の公開時点の状態を条件付き UPDATE(WHERE status='RUNNING')で一度だけ書く。abort-blue / abort-green が ABORTED にした後に実装が走り切って exitcode.txt を公開した場合、ファイル正本の導出値は SUCCEEDED / FAILED だが DB 列は runner の終端 UPDATE が 0 件のため ABORTED のまま残す(再同期しない。dispatcher の条件「中止済み run の比較依頼作成除外」の判定材料。SPEC-005-02 AC6 / AC9)。この経路以外は両者が一致し、slot 実行の状態判定(abort / rerun / hang-detector)は常にファイル正本で行う | tier-facade | domain `judge_slot_status` / repository `slot_execution_repo` | 中止後に実装が終了した場合は exitcode.txt を優先して状態を導出する(SPEC-010-01) / RAPID_CROSSCHECK_MODE=off では管理 DB に触れず完了通知も送らない(SPEC-005-04) |
...
| started_at | 起動時刻 | `date -u +%Y-%m-%dT%H:%M:%SZ`。`RELAY_GATE_NOW` 設定時はその UTC 値(Z 付き)をそのまま書く(ローカルへ変換しない。Runner Result Contract。契約 environment_variables.RELAY_GATE_NOW) | started-at.txt | tier-facade |
...
|---|---|---|
| REQ-003 | SPEC-003-01 | 実行終了後に 3 ファイルが揃う(SPEC-003-01) / runner の終了コードは exitcode.txt と一致する(SPEC-003-01) / 起動時に started-at.txt を出力する(SPEC-003-01) |
| REQ-003 | SPEC-003-02 | SSH 失敗でも 3 ファイルを揃える(SPEC-003-02) |
| REQ-003 | SPEC-003-03 | 書き込み途中の確定名ファイルは存在しない(SPEC-003-03) |
| REQ-003 | SPEC-003-04 | background でも同じ 3 ファイルを残す(SPEC-003-04) |
| REQ-004 | SPEC-004-04 | ローカル実行の slot は SSH せず runner のプロセスユーザーで実行する(SPEC-004-04) |
| REQ-005 | SPEC-005-04 | RAPID_CROSSCHECK_MODE=off では管理 DB に触れず完了通知も送らない(SPEC-005-04) / RAPID_CROSSCHECK_MODE=foreground でも background と同じく slot_executions を更新し完了通知を送る(SPEC-005-04) |
| REQ-009 | SPEC-009-02 | 復元起動はジョブマップを再解決しない(SPEC-009-02) |
| REQ-010 | SPEC-010-01 | 中止後に実装が終了した場合は exitcode.txt を優先して状態を導出する(SPEC-010-01) / 中止済み slot で runner が再起動されても実装を再実行しない(SPEC-010-01) |
| REQ-011 | SPEC-011-06 | background slot をリランした run は slot の終端時に parallel_run を COMPLETED にする(SPEC-011-06) |
