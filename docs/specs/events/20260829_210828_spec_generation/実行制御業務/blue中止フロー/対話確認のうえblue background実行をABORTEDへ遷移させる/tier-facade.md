...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| status | VARCHAR | RUNNING/UNKNOWN → ABORTED へ更新（runner_result_eventsへの履歴INSERTと同一transactionでUPSERT） | 変更 |
...
Feature: 対話確認のうえblue background実行をABORTEDへ遷移させる - tier-facade
  ...
  Scenario: 対象がUNKNOWNのbackground実行を対話確認y応答でABORTEDへ確定する
    Given execution_specs に run_id "3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57" の行と slot_execution_specs の (run_id "3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57", slot_type "blue") の行が存在する
    And runner_results に (run_id "3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57", slot_type "blue", role_type "background", attempt_id "att-blue-0001")、status "UNKNOWN" の起動試行（timeoutにより結果取得不能）が中止依頼済みで存在する
    And 環境変数 RELAYGATE_OPERATOR が "ops-tanaka" に設定されている
    When `relaygate abort blue confirm --run-id 3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57` を実行し対話確認で "y" と応答する
    Then 終了コード 0 で終了する
    And runner_result_events に event_name "attempt_aborted" の履歴が追加され、同一transactionで runner_results の status が "UNKNOWN" から "ABORTED" に更新される（推測でFAILEDへは確定しない）
    And audit_logs に event_name "abort_confirmed"、operation "abort"、outcome "succeeded"、actor "ops-tanaka" のレコードが1件追加される
