...
RUNNING中の確報比較依頼（E-005, AG-004）を、運用者の対話確認を経てABORTED状態へ明示的に遷移させるCLIコマンドをtier-workerに追加する。UPDATE時にWHERE句でstatus='RUNNING'を条件とすることで、対話確認中に他プロセスが状態を変化させた場合の競合を検知する。操作は監査イベント（abort_confirmed）としてhash-chain lock契約に従い記録する。
...
|---|---|---|
| AbortConfirmDialog | `src/components/domain/AbortConfirmDialog.tsx` | runId=run_id, target="確報比較依頼", impactSummary="ABORTEDへ遷移すると取消不可。リリース判断材料として利用できなくなる" |
...
Feature: 対話確認のうえ確報比較依頼をABORTEDへ遷移させる - tier-worker
  ...
  Scenario: abort_final_crosscheck_confirm_対話確認y応答の場合_ABORTEDへ更新し監査イベントを記録すること
    # Arrange
    ...
    # Act
    When 環境変数RELAYGATE_OPERATOR="ops-tanaka"の下でTTY接続から relaygate abort final-crosscheck confirm --run-id e57a03c8-9d21-4b6f-8a34-1c7e5b9d206f を実行し "y" を入力する
    # Assert
    Then final_crosscheck_requestsテーブルのrun_id="e57a03c8-9d21-4b6f-8a34-1c7e5b9d206f"のstatusが"ABORTED"に更新される
  ...
  Scenario: abort_final_crosscheck_confirm_状態競合の場合_業務エラーとすること
    # Arrange
    Given execution_specsテーブルにrun_id="e57a03c8-9d21-4b6f-8a34-1c7e5b9d206f"の行が存在する
    And final_crosscheck_requestsテーブルのrun_id="e57a03c8-9d21-4b6f-8a34-1c7e5b9d206f"がconfirmコマンド呼び出し直前にworkerによりstatus="FAILED"へ更新されている
    # Act & Assert
    When relaygate abort final-crosscheck confirm --run-id e57a03c8-9d21-4b6f-8a34-1c7e5b9d206f --yes を実行すると、UPDATE文の更新件数が0となり、標準エラーに "状態が変化したため中止できません run_id=e57a03c8-9d21-4b6f-8a34-1c7e5b9d206f" が出力され終了コード1で終了する
```
