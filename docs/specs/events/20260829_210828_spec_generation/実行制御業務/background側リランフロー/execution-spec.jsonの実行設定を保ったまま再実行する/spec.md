...
sequenceDiagram
  ...
  DB-->>FGW: {job_id: daily-settlement, hang_detect_limit_minutes: 30} / {host: blue-host-01, exec_user: batchuser, script_path: /opt/blue/run.sh, work_dir: /opt/relaygate/work, impl_version: blue-2.3.1, job_map_version: v1.4.0} / {attempt_id: att-blue-0001, status: FAILED}
  ...
  FGW->>DB: BEGIN
  FGW->>DB: INSERT INTO execution_specs (run_id, parent_run_id, job_id, additional_args, hang_detect_limit_minutes) VALUES ('7a2e4b91-...', '3f8c9d2e-...', 'daily-settlement', 元の値, 30)
  FGW->>DB: INSERT INTO slot_execution_specs (run_id, slot_type, host, exec_user, script_path, work_dir, fixed_args, impl_version, credential_ref, job_map_version) VALUES ('7a2e4b91-...', 'blue', 'blue-host-01', 'batchuser', '/opt/blue/run.sh', '/opt/relaygate/work', 元の値, 'blue-2.3.1', 元の参照名, 'v1.4.0')
  ...
  FGW->>Ext: SSH経由でblue実装のbackground roleを起動（run_id=7a2e4b91-8c63-4d5f-b012-9e3c7a1d6b84。秘密鍵は複製したcredential_refから解決、argvはfixed_args+additional_argsを要素順のまま連結。ジョブマップは読まない）
  Ext-->>FGW: 起動受理
...
Feature: execution-spec.jsonの実行設定を保ったまま再実行する
  ...
  Scenario: 完了済みのbackground実行を元の実行設定のまま新run_idで再実行する
    ...
    And slot_execution_specsテーブルに新規run_idのslot_type="blue"でhost="blue-host-01"、exec_user="batchuser"、script_path="/opt/blue/run.sh"、work_dir="/opt/relaygate/work"、impl_version="blue-2.3.1"、job_map_version="v1.4.0"の行が追加される
  ...
  Scenario: job map変更後もリランは既存run_idの変更前実行設定を保持したまま使用する
    ...
    And slot_execution_specsテーブルに(run_id="3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57", slot_type="blue")、host="blue-host-01"、script_path="/opt/blue/run.sh"、impl_version="blue-2.3.1"、job_map_version="v1.4.0"の行が存在する
    ...
    And blueのジョブマップ（RELAYGATE_JOB_MAP_PATH_BLUE）がjob_map_version v1.5.0へ更新され、job_id="daily-settlement"のhostが"blue-host-02"、script_pathが"/opt/blue-v2/run.sh"へ変更されている
    When 環境変数RELAYGATE_OPERATOR="ops-tanaka"の下で運用者が relaygate rerun run --target background --run-id 3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57 を実行する
    Then 既存run_id="3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57"のslot_execution_specs行はjob_map_version="v1.4.0"、host="blue-host-01"、script_path="/opt/blue/run.sh"のまま一切上書きされない
    And 新規run_id（parent_run_id="3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57"）のslot_execution_specs行はjob_map_version="v1.4.0"、host="blue-host-01"、script_path="/opt/blue/run.sh"で作成される（リランはジョブマップを読まないため、ジョブマップv1.5.0の値は使用されない）
...
```gherkin
  ...
  Scenario: 元のslot modeがforegroundまたはoffのためリランせずエラー終了する
    Given execution_specsテーブルにrun_id="3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57"、job_id="daily-settlement"の行が存在する
    And slot_execution_specsテーブルに(run_id="3f8c9d2e-5b41-4a7e-9c13-6d2a8b0f1e57", slot_type="blue", job_map_version="v1.4.0")の行のみが存在する（BLUE_MODE=foreground, GREEN_MODE=off で確定済み）
