...
- **標準エラー**: stderr.log 相当の内容をCLI標準エラーへ流す。relay-gate自身のエラーで応答する場合は、取得可能なら stderr.log の内容を先に出力し、続けて relay-gate のエラー内容（原因と次アクション）を併記する
- **終了コード**: exitcode.txt の値を 0 を含む全値そのままプロセス終了コードとして透過する（丸め・再割り当てをしない）。relay-gate 自身のエラーだけを退避コード 125（実行結果未確定・取得不能・中止済み）/ 124（relay-gate のバリデーションエラー）へ分離する
