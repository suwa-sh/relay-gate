...
feature flag設定（BLUE_MODE/GREEN_MODE/RAPID_CROSSCHECK_MODE）に基づきblue/green実装のslotを選択して起動し、background roleを非同期実行、foreground roleの標準出力・標準エラー・終了コードのみをジョブスケジューラへ応答する。execution specはrun共通（execution_specs）とslot別（slot_execution_specs）に分離して起動時に一度だけ確定し、起動試行はrunner_result_events（append-only履歴）とrunner_results（snapshot）へ(run_id, slot_type, role_type, attempt_id)のidentityで記録する。起動UCは起動イベントの送出に失敗した試行をFAILED、送出がtimeoutした試行をUNKNOWN（推測でFAILEDを確定しない）へ補償記録し、起動受付までで応答する（foreground完了は待たない）。foreground完了待機（上限はexecution_specs.hang_detect_limit_minutes）はforeground応答UCの責務である。実行設定のINSERT・STARTING記録・起動前監査イベント（audit_logs + audit_chain_heads）の追記は同一transactionでcommitし、commitできない場合は外部slotを起動しない（起動前監査ゲート）。移行運用責任者はRunner実行結果を確認し、段階的切替の判断材料とする。

## 所属 UC 一覧

| UC名 | アクター | 主な操作 | 関連情報 |
|------|---------|---------|---------|
| [feature flag設定に基づきslotを選択して起動する](feature flag設定に基づきslotを選択して起動する/spec.md) | 運用者（ジョブスケジューラ起動契機） | feature flag設定を参照しexecution spec（run共通 + slot別）を確定・起動試行をSTARTINGで記録・起動前監査を追記のうえblue/green起動。起動イベント送出失敗はFAILED、送出timeoutはUNKNOWNへ補償記録（slot_launch_failed / slot_launch_timeout）。起動受付までで応答しforeground完了を待たない | execution-spec.json、Runner実行結果、audit_logs |
...
| [foreground roleの標準出力・標準エラー・終了コードを応答する](foreground roleの標準出力・標準エラー・終了コードを応答する/spec.md) | 運用者（ジョブスケジューラ） | foreground役割の最新試行（attempt_no最大）のstatus確定を待機（上限hang_detect_limit_minutes）したうえで、実行結果を3項目に限定してジョブスケジューラへ中継。終了コードはexitcode.txtの値を0含む全値そのまま透過し、relay-gateエラー時のみ退避コード125/124で応答 | Runner実行結果 |
...
feature flag設定に基づく起動UCがrun共通execution spec（execution_specs）とslot別実行設定（slot_execution_specs）を確定し、起動試行をSTARTINGで記録する。起動イベントの送出に失敗した試行は起動UC自身がFAILED、送出がtimeoutした試行はUNKNOWN（推測でFAILEDを確定しない）へ補償記録し、起動受付までで応答する（foreground完了は待たない）。送出に成功した試行を起点にbackground起動UCが起動試行をRUNNING（失敗時FAILED、結果取得不能時UNKNOWN）へ遷移させ、foreground応答UCがforeground試行のstatus確定を待機（上限はexecution_specs.hang_detect_limit_minutes）したうえで結果を参照する。各状態遷移はrunner_result_eventsへの履歴INSERTとrunner_resultsのsnapshot UPSERTを同一transactionで行い、監査イベントはaudit_chain_headsのrun_id行を排他ロックして直列化のうえaudit_logsへ追記する。移行運用責任者向けの確認UCは両者のRunner実行結果と履歴・監査イベントを横断的に参照する。
...
| Runner実行結果（runner_result_events 履歴 + runner_results snapshot） | C（STARTING記録）/U（送出失敗・timeoutの補償記録） | C/U（履歴INSERT + snapshot UPSERT） | R | R |
| audit_logs（+ audit_chain_heads） | C | C | | R |

## 状態遷移全体図

Runner実行状態は6値（STARTING/RUNNING/SUCCEEDED/FAILED/UNKNOWN/ABORTED）をとる。起動UCが起動試行をSTARTINGで受け付け（起動イベント送出失敗時はFAILED、送出timeout時はUNKNOWNへ起動UC自身が補償記録）、background起動UCがRUNNING（起動失敗時FAILED、SSH起動タイムアウト等の結果取得不能時UNKNOWN。推測でFAILEDを確定しない）へ遷移させる。RUNNING以降のSUCCEEDED/FAILED/UNKNOWNへの確定（ハング監視業務が担当）とABORTEDへの遷移（実行制御業務のblue/green中止フローが担当。対話確認による明示的操作でのみ発生）は本BUCの範囲外だが、業務全体の状態遷移経路の起点として明示する。

```mermaid
stateDiagram-v2
  [*] --> STARTING: feature flag設定に基づきslotを選択して起動する（起動受付）
  STARTING --> FAILED: feature flag設定に基づきslotを選択して起動する（起動イベント送出失敗の補償記録）
  STARTING --> UNKNOWN: feature flag設定に基づきslotを選択して起動する（起動イベント送出timeout。推測でFAILEDを確定しない）
  STARTING --> RUNNING: background roleを起動する（起動確認成功）
  STARTING --> FAILED: background roleを起動する（起動確認のSSH接続失敗）
  STARTING --> UNKNOWN: background roleを起動する（SSH起動確認タイムアウト等の結果取得不能）
...
| Runner実行状態 | STARTING | FAILED | [feature flag設定に基づきslotを選択して起動する](feature flag設定に基づきslotを選択して起動する/spec.md)（起動イベント送出失敗の補償記録） |
| Runner実行状態 | STARTING | UNKNOWN | [feature flag設定に基づきslotを選択して起動する](feature flag設定に基づきslotを選択して起動する/spec.md)（起動イベント送出timeout。推測でFAILEDを確定しない） |
| Runner実行状態 | STARTING | FAILED | [background roleを起動する](background roleを起動する/spec.md)（起動確認のSSH接続失敗） |
...
| hang_detect_limit_minutes | run共通の1値。起動UCがbackground roleに選ばれたslotのジョブマップ値を採用してexecution_specsへ保存し（両slotがbackgroundなら大きい方、backgroundが無ければ起動対象の唯一のslotの値）、foreground応答UCが完了待機の上限として参照する | feature flag設定に基づきslotを選択して起動する（採用・保存）、foreground roleの標準出力・標準エラー・終了コードを応答する（待機上限として参照） |
...
| relay-gateエラーの退避終了コード | relay-gate自身のエラーを業務ジョブの終了コードと区別するために125/124を予約する。実行結果未確定・取得不能・中止済み（UNKNOWN/ABORTED含む）は125、relay-gateのバリデーションエラーは124。foregroundのexitcode.txtの値は0を含む全値をそのままジョブスケジューラへ透過し、退避終了コードへ丸めない（UNKNOWNを推測でFAILED相当の業務終了コードへ変換しない）。bash予約の126/127は使用しない | foreground roleの標準出力・標準エラー・終了コードを応答する |
