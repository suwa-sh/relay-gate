...
| slot_execution_specs.run_id, slot_type, host, exec_user, script_path, work_dir, fixed_args, impl_version, credential_ref, job_map_version | - | background対象slotのslot別実行設定（job_map_versionはslot別のジョブマップ版） | 参照のみ |
