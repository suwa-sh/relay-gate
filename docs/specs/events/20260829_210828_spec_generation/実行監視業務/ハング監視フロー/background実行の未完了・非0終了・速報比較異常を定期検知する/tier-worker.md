...
hang-detector を CronJob 定期実行として追加する。Runner実行結果（STARTING/RUNNING/UNKNOWN状態かつrole_type='background'の起動試行。UNKNOWNは実結果ファイルの回収再試行対象）の走査によるハング疑い・background実行エラーの検知、速報比較結果（NG判定）の走査による速報クロスチェック異常の検知を行い、ハング検知記録を作成するとともにbackground slot実行状態をSUCCEEDED/FAILED/UNKNOWNへ確定する（推測でFAILEDを確定せず、ABORTEDへは自動遷移させない）。状態確定はrunner_result_eventsへの履歴INSERTとrunner_resultsのsnapshot UPSERTを同一transactionで行い（LR-002）、slot_final_status監査イベントをaudit_logsへ追記する。
...
- **入力**: RDBの `runner_results` テーブル（status IN ('STARTING','RUNNING','UNKNOWN') AND role_type='background'）、`rapid_crosscheck_results` テーブル（comparison_result='NG'）をポーリング
...
- **処理フロー**:
  1. STARTING・RUNNING・UNKNOWN状態かつrole_type='background'の起動試行（run_id, slot_type, role_type, attempt_id）を全件取得する（foreground/rapid-crosscheckは対象外とする）。UNKNOWNの起動試行は実結果ファイルの回収を再試行し、回収できた場合のみSUCCEEDED/FAILEDへ確定する。STARTINGで走査に現れる試行は、起動イベントの送出には成功したが起動確認（RUNNING）前の試行だけである。起動イベント送出に失敗・timeoutした試行は起動UC（feature flag設定に基づきslotを選択して起動する）がFAILED / UNKNOWNへ補償記録済みのためSTARTINGには残らない。送出timeoutによりUNKNOWNとなった試行はUNKNOWNの回収再試行（実結果ファイルが回収できた場合のみSUCCEEDED/FAILEDへ確定）の対象に含まれる
...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| status | VARCHAR | 実行状態（STARTING/RUNNING/SUCCEEDED/FAILED/UNKNOWN/ABORTED）。本UCによりSTARTING/RUNNING→SUCCEEDED/FAILED/UNKNOWN、および実結果回収時のUNKNOWN→SUCCEEDED/FAILEDへ更新される（ABORTEDへは遷移させない） | 変更 |
...
- hang_detect_limit_minutesはrun共通の1値であり、background roleに選ばれたslotのジョブマップ値としてexecution_specsに保存された値を使用する（role別・slot別の値は持たない）。ハードコードしない
- exitcode.txtが未出力かつ経過時間がhang_detect_limit_minutesを超過した場合のみハング疑いとして記録する（超過前は記録対象外とする）
- Runner実行結果走査は status が STARTING・RUNNING・UNKNOWN かつ role_type='background' に限定する。foreground・rapid-crosscheck役割の行は本UCの検知対象に含めない。UNKNOWN行は実結果ファイルの回収再試行のみを行い、回収できるまでUNKNOWNのまま維持する
...
- background slot実行状態のSTARTING/RUNNING→SUCCEEDED/FAILED/UNKNOWN遷移は、起動イベント送出失敗/timeoutの補償記録（起動UC「feature flag設定に基づきslotを選択して起動する」によるSTARTING→FAILED/UNKNOWN）と起動確認（「background roleを起動する」によるSTARTING→RUNNING/FAILED/UNKNOWN）を除き本UCのみがトリガーとなる（それ以外の他UCからの直接更新は行わない。ABORTEDへの遷移は中止確認UCの対話確認でのみ発生する）
...
Feature: background実行の未完了・非0終了・速報比較異常を定期検知する - tier-worker
  ...
  Scenario: role_type='background'以外のRUNNING行は走査対象に含めない
    ...
    When gateway層が `SELECT * FROM runner_results WHERE status IN ('STARTING','RUNNING','UNKNOWN') AND role_type='background'` を実行する
