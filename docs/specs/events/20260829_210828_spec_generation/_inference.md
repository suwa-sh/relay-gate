...
## 差分反映 (event_id: 20260819_114307_spec_generation)

feedback request `20260818_164000_rdra_followup_6078c4ed` の reconciliation 専用 stage として実施した。direct work unit は持たず、causal work unit `CR-6078c4ed-008#1` / `CR-6078c4ed-009#1` / `CR-6078c4ed-010#1` への追随のみを行った。

### 判断の見直し

| # | 項目 | 前回の採用値 | 今回の変更 | 理由 |
|---|------|-------------|-----------|------|
| 2 | エラーハンドリング戦略 | 全 CLI で終了コード規約 0/1/2/124/130 | `relaygate concurrent-run respond-foreground` のみ透過契約（exitcode.txt 値の全値透過 + 退避コード 125/124）へ例外化 | ジョブスケジューラが業務ジョブの終了コードとして解釈する唯一のコマンドであり、写像すると既存ジョブ直接実行との互換性を失う（USDM SPEC-002-01/002-03/002-04、arch SR-006/SP-002）。運用者向けコマンドは従来規約を維持する |
| 3 | RDB 正規化レベル | 3NF 基本 + Event/Snapshot 併用 | 比較定義に SCD2（有効期間つき世代管理）を追加適用 | 同一 job_id に複数世代の比較定義を持ち、依頼作成時点の世代で比較内容を固定する必要がある（USDM SPEC-012-03、infra comparison_definition_versioning） |

### 反映範囲の判定

- CR-6078c4ed-008（slot 別実行設定・attempt identity・6 値実行状態）: 仕様本体は前回 run で反映済み。今回は USDM へ新規追加された SPEC-003-04 / SPEC-009-05 の逆引きトレーサビリティを整備し、未対応 criterion を解消する Scenario を補完した
- CR-6078c4ed-009（比較定義）: データストア契約・速報/確報の比較実行 UC・速報比較依頼作成 UC へ反映。`rdra-feedback.md` の変更要望 #1 を解消済みへ更新した
- CR-6078c4ed-010（終了コード透過）: CLI コマンド契約と UC spec（データフロー・処理フロー・BDD）を透過契約へ整合させた

### 確認推奨項目

なし（上流 stage で確定済みの前提のみを採用し、新規の低確信度判断は発生していない）。
