...
- **`RELAY_GATE_NOW` の適用先**: 新 run_id の時刻部(プロセスのローカルタイムゾーンへ変換した値。spec の例示は TZ=UTC 前提)、`restored_at`、`parallel_runs.requested_at`、`slot_executions.started_at`(DB の now() を使う INSERT / UPDATE にもこの値を渡す)。未設定なら実時刻
