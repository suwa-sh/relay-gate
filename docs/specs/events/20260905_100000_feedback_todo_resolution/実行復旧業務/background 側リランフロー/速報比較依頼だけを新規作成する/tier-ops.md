...
- **`RELAY_GATE_NOW` の適用先**: 新 run_id の時刻部(プロセスのローカルタイムゾーンへ変換した値。spec の例示は TZ=UTC 前提)、`parallel_runs.requested_at`、`rapid_crosscheck_requests.requested_at`(DB の now() を使う INSERT にもこの値を渡す)。未設定なら実時刻。runner を起動しないため環境変数の引き継ぎは無い
