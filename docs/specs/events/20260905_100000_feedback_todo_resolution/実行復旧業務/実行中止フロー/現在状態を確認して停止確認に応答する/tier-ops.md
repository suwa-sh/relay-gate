...
| 名前 | 型 | 必須 | 既定値 | 説明 |
|------|---|------|-------|------|
| `--run-id` | string | Yes | なし | 対象 run_id(`{ローカル yyyymmddThhmmss}-{job_id}-{8 hex}`)。`abort-final-crosscheck.sh` では値を `final_crosscheck_id` として解釈する(方針資料が 4 スクリプトとも `--run-id` を用いるため引数名を揃える) |
...
- **stdout**: 現在状態を plain(`key=value`、パスは `key: value`)で固定順に出す。応答が yes 以外のときは末尾に `status={現在状態}` を 1 行追加する
  ...
  | 7 | `started_at` | `facade/<run_id>/<role>/started-at.txt` の中身(UTC ISO 8601。無ければ `-`) |
...
Feature: 現在状態を確認して停止確認に応答する - 実行監視・復旧ティア
  ...
  Scenario: abort-green.sh が現在状態を固定順で出しプロンプトを stderr に出す
    ...
    Then stdout の 1〜8 行目のキーが run_id, job_id, role, mode, status, pid, started_at, artifact_dir の順で、5 行目が `status=RUNNING`、6 行目が `pid=12345`、9 行目が `status=RUNNING` である
  ...
  Scenario: abort-final-crosscheck.sh が依頼の現在状態を出す
    Given final_crosscheck_requests に final_crosscheck_id=20260830T020000-final-1a2b3c4d status=RUNNING worker_id=worker-db-01 lease_until=2026-08-30T02:15:00Z started_at=2026-08-30T02:05:00Z がある
    When `abort-final-crosscheck.sh --run-id 20260830T020000-final-1a2b3c4d --yes` を実行する
    Then stdout の 1〜7 行目が run_id=20260830T020000-final-1a2b3c4d / job_id=- / role=final-crosscheck / status=RUNNING / worker_id=worker-db-01 / lease_until=2026-08-30T02:15:00Z / started_at=2026-08-30T02:05:00Z である
