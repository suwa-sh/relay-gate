...
Feature: 比較ツールで日次全量比較を実行して結果を保存する - 確報クロスチェックティア
  ...
  Scenario: 比較 OK を SUCCEEDED として保存する
    Given final_crosscheck_requests に final_crosscheck_id=20260830T210000-final-7b2c9e1f status=REQUESTED requested_at=2026-08-30T21:00:00Z catalog_version=v3 business_date=2026-08-30 がある(1 プロセスで claim → RUNNING → 終端まで検証する)
