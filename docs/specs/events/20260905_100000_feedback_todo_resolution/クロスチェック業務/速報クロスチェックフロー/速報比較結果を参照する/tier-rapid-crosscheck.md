...
- **管理 DB 接続先**: `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env` の `RAPID_DB_CONN_REF`(UC「速報比較依頼を claim する」tier-rapid-crosscheck.md の設定ファイル)。`RAPID_CROSSCHECK_MODE` が off 以外(foreground / background)で欠落なら終了コード 2
