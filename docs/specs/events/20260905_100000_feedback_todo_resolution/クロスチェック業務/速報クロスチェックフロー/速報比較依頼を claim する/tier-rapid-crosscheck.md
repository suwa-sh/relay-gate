...
- **設定ファイル(読む順)**:
  ...
  2. off 以外のとき `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env`(env 形式。確報の `final-crosscheck.env` / 監視の `hang-detector.env` と同じ流儀。契約 `cli-command-contract.yaml` の config_files が正)
  | キー | 必須 | 既定 | 説明 |
  |---|---|---|---|
  | `RAPID_DB_CONN_REF` | Yes(`RAPID_CROSSCHECK_MODE` が off 以外のとき) | — | 速報管理 DB 接続情報の参照名(値は置かない。`HANG_DB_CONN_REF` と同じ参照名を指してよい)。rapid-crosscheck-runner.sh / rapid-crosscheck-worker.sh / rapid-crosscheck-result.sh が読む |
...
| `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env` | env | `RAPID_DB_CONN_REF` / `RAPID_LEASE_SEC` / `RAPID_POLL_INTERVAL_SEC`(上記「設定ファイル」。off 以外のときのみ読む) | 追加 |
...
Feature: 速報比較依頼を claim する - 速報クロスチェックティア
  ...
  Scenario: --once で REQUESTED を 1 件 claim する
    Given rapid_crosscheck_requests に status=REQUESTED の行が run_id=20260830T113000-JOB001-3f9a1c2e(requested_at=2026-08-30T11:45:10Z)と run_id=20260830T113500-JOB002-1a2b3c4d(requested_at=2026-08-30T11:50:10Z)の 2 件ある
