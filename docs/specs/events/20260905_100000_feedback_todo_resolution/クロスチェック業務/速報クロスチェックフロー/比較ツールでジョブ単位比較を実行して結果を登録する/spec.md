...
| レイヤー | データモデル | 変換内容 |
|---------|------------|---------|
| usecase | ExecuteComparison | RUNNING 遷移 → 比較定義解決 → 成果物 dir 作成 → 比較ツール起動 → 結果保存 → comparison_result 登録 → リラン由来 run の COMPLETED 遷移 |
