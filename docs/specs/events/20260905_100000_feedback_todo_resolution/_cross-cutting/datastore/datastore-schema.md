...
| completed_at | datetime | YES | SUCCEEDED / FAILED / ABORTED へ遷移した日時(UTC)。ABORTED では aborted.txt の中止日時と同値。未終端は NULL。情報.tsv: 終了時刻(UC 間で finished_at / completed_at が混在していたため completed_at に統一) |
...
| status | string | NO | クロスチェック依頼の状態。値: REQUESTED(未 claim), CLAIMED(worker が lease 取得), RUNNING(比較ツール起動後), SUCCEEDED(exit_code=0), FAILED(非 0、または比較定義なし・起動失敗。条件「比較結果の登録条件」により exit_code=6 相当で終端し comparison_results は登録しない), ABORTED(運用者の明示中止) |
