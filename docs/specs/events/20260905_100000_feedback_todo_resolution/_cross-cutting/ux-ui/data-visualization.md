...
- run_id 形式が時系列ソート可能(`{ローカル yyyymmddThhmmss}-...`。`ui-design.md`「出力フォーマット」。利用者決定 2026-09-05)なため、`run_id` 列だけでも時系列が読める(時刻部はローカルタイムゾーン、`requested_at` 列は UTC。両者を突き合わせるときはタイムゾーン差に注意する)
