...
- 設定ファイルは 7 種(feature-flag.env / blue-job-map.csv・green-job-map.csv(`<role>-job-map.csv`)/ crosscheck-job-map.csv / target-catalog.csv / rapid-crosscheck.env / hang-detector.env / final-crosscheck.env)。`rapid-crosscheck.env`(定義元 UC-10、`RAPID_DB_CONN_REF` / `RAPID_LEASE_SEC` / `RAPID_POLL_INTERVAL_SEC`)は管理 DB を使う全コマンド(facade.sh / slot runner を含む)が readers として宣言されている。`hang-detector.env`(RDRA 情報「ハング検知定期ジョブ設定」。所有者は基盤適用設計者。`ALERT_MAIL_TO` / `ALERT_MAIL_CMD` / `ALERT_SUBJECT_PREFIX` / `HANG_DB_CONN_REF` + 運用チューニング値)は hang-detector.sh だけが読み、定義元 UC は UC-18「background 実行の経過時間と終了状態を判定する」と UC-19「ハング疑い・実行エラー・比較異常を通知する」(契約 `config_files[hang-detector.env].defined_in_uc`)。slot ジョブマップの列は `job_id / host / user / work_dir / script / fixed_params / hang_detect_limit_minutes[, credential_ref][, map_version]`(ヘッダー名で対応付け。host / user は両方省略でローカル実行、credential_ref / map_version は末尾の任意列、実装版列は無い)
...
| UC-23 | [実行を ABORTED へ遷移させる](<../実行復旧業務/実行中止フロー/実行を ABORTED へ遷移させる/spec.md>) | 実行復旧業務 / 実行中止フロー | ops | abort-*(uses。後半。abort-blue / abort-green は aborted.txt を書き、off 以外は加えて slot_executions / parallel_runs を条件付き UPDATE。abort-rapid-crosscheck / abort-final-crosscheck は依頼の条件付き UPDATE) |
