...
| キー | 所在 | 必須 | 検証 |
|---|---|---|---|
| RAPID_DB_CONN_REF | `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env` | off 以外のとき Yes | ファイル不在・キー欠落は実装を起動せず exitcode.txt=2、stderr.log に `error: config file not found path: <path>` または `error: option required option=RAPID_DB_CONN_REF path: <path>`(3 ファイルは揃える)。off では読まない(契約 `config_files.rapid-crosscheck.env`) |
...
### slot_executions(RAPID_CROSSCHECK_MODE が off 以外のみ。ファイル正本と同じ値を保持する二重マッピング)
...
| completed_at | datetime | slot 終端日時(UTC) | 追加 |
...
- slot 実行の状態はファイル正本から導出する。exitcode.txt が無く aborted.txt があれば ABORTED、どちらも無ければ RUNNING。管理 DB は同じ値を保持する(条件: slot 実行の状態導出規則)
