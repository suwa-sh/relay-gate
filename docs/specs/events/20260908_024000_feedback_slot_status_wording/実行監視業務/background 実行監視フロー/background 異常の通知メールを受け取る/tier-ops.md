...
- **書式**: メール(`text/plain; charset=UTF-8`)。宛先はハング検知定期ジョブ設定(`hang-detector.env`)の `ALERT_MAIL_TO`、件名プレフィックスは同設定の `ALERT_SUBJECT_PREFIX`(既定 `[relay-gate]`。以下の例は既定値)
...
`{ALERT_SUBJECT_PREFIX}[{warning|error}] {hang-suspected|background-exec-error|rapid-crosscheck-error} run_id={run_id} job_id={job_id} role={blue|green|rapid-crosscheck}`
...
#### 本文(行順固定。1 行 1 事実。80 桁制限は 1〜13 行目のうち 11 行目 artifact_dir を除く行に適用。artifact_dir と 14 行目の recommended_action は折り返さず 1 行で、80 桁を超えてよい)

| 行 | 内容 | 例 |
|---|---|---|
| 1 | `kind={通知種別}` | `kind=hang-suspected` |
| 2 | `level={warning|error}` | `level=warning` |
| 3 | `run_id={run_id}` | `run_id=20260830T113000-JOB001-3f9a1c2e` |
| 4 | `job_id={job_id}` | `job_id=JOB001` |
| 5 | `role={role}` | `role=green` |
| 6 | `started_at={ローカル日時}`(ISO 8601 秒精度・指示子なし。started-at.txt 由来の値はローカル時刻へ変換済み) | `started_at=2026-08-30T11:30:05` |
...
| 11 | `artifact_dir: {絶対パス}` | `artifact_dir: /var/relay-gate/facade/20260830T113000-JOB001-3f9a1c2e/green` |
| 12 | `detected_at={ローカル日時}`(ISO 8601 秒精度・指示子なし。run_id の時刻部と同じ時刻軸) | `detected_at=2026-08-30T12:45:00` |
| 13 | (空行) | |
...
- **受信の保証**: 同じ run_id + role + kind のメールは 1 通。ハング疑い(warning)の後に実行エラー(error)が来ることがある(別種別)。運用者が `abort-*` で中止した対象(slot は aborted.txt あり / 依頼は ABORTED)は次回の判定で監視記録が COMPLETED に終端し、以後メールは来ない。メールが来ないことは「異常なし」または「hang-detector.sh 自体の失敗(送信失敗は終了コード 6 でジョブスケジューラの定期ジョブが失敗する)」のどちらかであり、後者はジョブスケジューラの定期ジョブの実行結果で検知する
...
Feature: background 異常の通知メールを受け取る - 実行監視・復旧ティア
  ...
  Scenario: 件名だけで重要度・種別・対象を判別できる
    Given hang-detector.sh が run_id=20260830T113000-JOB001-3f9a1c2e job_id=JOB001 role=green のハング疑いを通知した
    When 運用者がメールクライアントで件名を読む
    Then 件名は "[relay-gate][warning] hang-suspected run_id=20260830T113000-JOB001-3f9a1c2e job_id=JOB001 role=green" と完全一致する(ALERT_SUBJECT_PREFIX が既定値のとき)

  Scenario: 本文の推奨対処に run_id 付きのコマンドが書かれている
    Given 同じメールを受け取った
    When 運用者が本文を読む
    Then 14 行目は "recommended_action: check the process on the execution host; if it is still running normally, wait. if it is hung, stop the process, then run: abort-green.sh --run-id 20260830T113000-JOB001-3f9a1c2e" の 1 行で、本文は 14 行である
    And 本文に HTML タグ・添付ファイルは無く、11 行目 artifact_dir を除く 1〜13 行目に 80 桁を超える行は無い(artifact_dir と 14 行目は折り返されず 80 桁を超えてよい)
