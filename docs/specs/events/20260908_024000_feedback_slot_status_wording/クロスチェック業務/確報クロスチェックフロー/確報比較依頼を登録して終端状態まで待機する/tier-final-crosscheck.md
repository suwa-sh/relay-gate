...
| `--catalog-version` | string | Yes | — | 対象カタログの版。対象カタログ CSV の `catalog_version` 列に存在する値 |
...
- 対象カタログの所在は `RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv` のヘッダーコメント `# target_catalog_path=<絶対パス>` / `# catalog_version=<既定版>` から解決する(step3 canonical C5。定義は G2 UC「クロスチェックのジョブマップと比較定義を定義する」を正とする)。`--catalog-version` は既定版より優先する
- 対象カタログ CSV(1 行目ヘッダー、1 行 1 対象、行頭 `#` はコメント。セルはカンマ区切りで二重引用符で囲める。囲んだセル内の二重引用符は `""`): `catalog_version,target_type,target_identifier,compare_condition,business_date_handling`。`--catalog-version` の値を持つ行が 1 行以上あることを起動時に検証する
...
- エラーメッセージ(1 回だけ出す。CLR-003):
  ...
  - `error: target_catalog_path declaration not found path: /etc/relay-gate/crosscheck-job-map.csv` + `hint: add a header comment line "# target_catalog_path=<absolute path>"`(終了コード 2)
  - `error: catalog version not found catalog_version=v3 path: /etc/relay-gate/target-catalog.csv`
...
- **バリデーション**: `--business-date` は `^[0-9]{4}-[0-9]{2}-[0-9]{2}$` かつ `date -u -d` で妥当。`--catalog-version` は非空・改行を含まない(対象カタログ CSV の `catalog_version` セルの値と一致比較する)。両方とも欠落は終了コード 2。feature flag ファイルは読まない(確報クロスチェック非起動)
- **確認プロンプト**: なし
...
- **クラッシュ耐性**: INSERT 前に落ちた場合は何も残らない。INSERT 後に runner プロセスが落ちても依頼は REQUESTED のまま残り、worker が通常どおり処理する。ジョブスケジューラ側の再実行は新しい依頼を作る(残った依頼は運用者が `abort-final-crosscheck.sh` で整理する。RUNNING 以外は abort-final-crosscheck では中止できない(条件「依頼中止可否判定」)。runner が落ちて polling が無い REQUESTED / CLAIMED の残骸は worker が通常どおり処理して終端させる(polling 中の未着手依頼は runner 側の polling 上限 FINAL_POLL_LIMIT_SEC で扱う))
...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| final_crosscheck_id | string | 主キー。`{ローカル yyyymmddThhmmss}-final-{8 hex}`(時刻部はホストのローカルタイムゾーン。タイムゾーン指示子なし) | 追加 |
...
| requested_at | datetime | 登録日時(ローカル時刻) | 追加 |
...
Feature: 確報比較依頼を登録して終端状態まで待機する - 確報クロスチェックティア
  ...
  Scenario: 依頼を REQUESTED で登録して polling を開始する
    ...
    And RELAY_GATE_CONFIG_DIR/crosscheck-job-map.csv のヘッダーコメントに # target_catalog_path=/etc/relay-gate/target-catalog.csv がある
    And /etc/relay-gate/target-catalog.csv に catalog_version=v3 の行が 2 行ある
