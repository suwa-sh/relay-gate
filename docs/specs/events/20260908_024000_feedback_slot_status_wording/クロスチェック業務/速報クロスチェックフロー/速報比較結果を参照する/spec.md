...
| 速報クロスチェックモード | foreground / background | 管理 DB を参照して結果を出す(両値で同じ挙動) | tier-rapid-crosscheck | rapid-crosscheck-result.sh |
| 速報クロスチェックモード | off | 速報管理 DB を使わないため終了コード 3 | tier-rapid-crosscheck | rapid-crosscheck-result.sh |
...
| run_id の解析 | `--run-id` の値 | 先頭 15 文字(ローカルタイムゾーンの `yyyymmddThhmmss`。タイムゾーン指示子なし)と末尾 8 文字(hex)の形式検証(`^[0-9]{8}T[0-9]{6}-{job_id}-[0-9a-f]{8}$`)。不一致は終了コード 2 | 検証結果 | tier-rapid-crosscheck |
...
| 情報 | 速報クロスチェック設定 | 管理 DB 接続参照名(RAPID_DB_CONN_REF)だけを読む(spec 追加採用。BUC.tsv 未紐付け。tier-rapid-crosscheck) |
| 条件 | 速報結果の位置付け | 適用 |
| 条件 | 比較ツール終了コードの対応 | 適用 |
| 画面 | rapid-crosscheck 結果参照出力(→ CLI 出力) | stdout / stderr / 終了コード |
| イベント | comparison_result の参照 | 管理 DB(RDB)への SELECT |
| 内部データストア | ジョブキュー兼管理 DB(RDB) | 参照先(relay-gate 内部の構成要素。外部システムではない) |
...
Feature: 速報比較結果を参照する
  ...
  Scenario: 比較 OK の依頼を参照する(SPEC-011-02)
    ...
    And rapid_crosscheck_requests に run_id=20260830T113000-JOB001-3f9a1c2e, job_id=JOB001, status=SUCCEEDED, exit_code=0, worker_id=worker-01 の行がある
    And comparison_results に run_id=20260830T113000-JOB001-3f9a1c2e, comparison_result_id=c0a8f1d2, comparison_type=job, status=OK, difference_count=0, report_uri=file:///var/relay-gate/reports/20260830T113000-JOB001-3f9a1c2e/job.html, compared_at=2026-08-30T11:47:01 の行がある
    When 運用者が `rapid-crosscheck-result.sh --run-id 20260830T113000-JOB001-3f9a1c2e` を実行する
    Then 終了コードは 0 である
    And stdout の 1 行目は `run_id=20260830T113000-JOB001-3f9a1c2e`、3 行目は `request_status=SUCCEEDED`、6 行目は `exit_code=0` である
    And stdout の TSV ヘッダー行は `comparison_result_id	comparison_type	status	difference_count	report_uri	compared_at` で、続く 1 行に `c0a8f1d2	job	OK	0	file:///var/relay-gate/reports/20260830T113000-JOB001-3f9a1c2e/job.html	2026-08-30T11:47:01` が出る
    And final_crosscheck_requests は参照されない

  Scenario: 比較 NG の依頼を参照する(SPEC-005-05)
    Given rapid_crosscheck_requests に run_id=20260830T113000-JOB001-3f9a1c2e, status=FAILED, exit_code=3 の行がある
  ...
  Scenario: --show-output で比較ツールの出力本文を確認する
    Given rapid_crosscheck_requests に run_id=20260830T113000-JOB001-3f9a1c2e, stdout="diff: 12 rows", stderr="" の行がある
    When 運用者が `rapid-crosscheck-result.sh --run-id 20260830T113000-JOB001-3f9a1c2e --show-output` を実行する
    Then 終了コードは 0 である
...
```gherkin
  Scenario: 存在しない run_id を参照する
    Given rapid_crosscheck_requests に run_id=20260830T113000-JOB999-00000000 の行が無い
    When 運用者が `rapid-crosscheck-result.sh --run-id 20260830T113000-JOB999-00000000` を実行する
    Then 終了コードは 3 である
    And stderr に `error: rapid crosscheck request not found run_id=20260830T113000-JOB999-00000000` が出る
    And stdout は 0 行である
