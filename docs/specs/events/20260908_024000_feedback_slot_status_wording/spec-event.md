...
| 項目 | 内容 |
|------|------|
| Event ID | 20260908_024000_feedback_slot_status_wording |
| Created At | 2026-09-08T02:40:00Z |
| Source | Spec 差分更新: feedback 20260908_slot_status_wording(design 無しモード。文言追従のみ)。前イベント 20260907_131000_feedback_abort_consistency を起点に、RDRA 条件「slot 実行の状態導出規則」の管理 DB 側の新文言(slot_executions.status は aborted.txt / exitcode.txt 公開時点の状態を条件付き更新で一度だけ書き、abort 後は ABORTED のまま再同期しない)への参照を rdb-schema / CLI 契約 / 該当 UC で揃え、USDM SPEC-005-02 AC9 の BDD シナリオを追加、rdra-feedback #16 を解消済みへ。trigger_event: rdra:20260908_011000_feedback_slot_status_wording, arch:20260908_015000_feedback_slot_status_wording |
| UC 総数 | 32 |
| API 総数 | 0 |
| 非同期イベント総数 | 4 |
| 業務数 | 5 |
