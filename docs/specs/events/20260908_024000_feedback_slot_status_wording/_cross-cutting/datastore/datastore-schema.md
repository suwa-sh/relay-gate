...
| comparison_type | string | NO | 比較定義の比較種別。値: job(速報。crosscheck-job-map.csv の該当 job_id 行の comparison_type を転記)/ full(確報。速報側の本テーブルには現れない)。cli-command-contract.yaml の crosscheck-job-map.csv.comparison_type enum [job, full] と一致させる。比較定義が無い job_id では条件「比較結果の登録条件」により本テーブルに INSERT せず依頼を FAILED(error_summary=comparison definition not found job_id=...)で終端する。情報.tsv: comparison_type |
...
| monitor_status | string | NO | 監視状態(状態モデル: 監視状態)。値: NOT_MONITORED(監視対象外), MONITORING(監視中), HANG_SUSPECTED_NOTIFIED(ハング疑い通知済み), EXEC_ERROR_NOTIFIED(実行エラー通知済み), COMPARE_ERROR_NOTIFIED(比較異常通知済み), COMPLETED(正常終了・中止済み対象の終端)。遷移(状態モデル「監視状態」): MONITORING → HANG_SUSPECTED_NOTIFIED / EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED / COMPLETED(exitcode 0、または監視対象が ABORTED で中止済み終端)、HANG_SUSPECTED_NOTIFIED → EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED / COMPLETED(通知後に対象が終端または ABORTED になった場合。「通知後正常終了」はこの遷移の別名)。hang_judgement(ハング検知判定結果 6 値: NOT_TARGET / COMPLETED / MONITORING / HANG_SUSPECTED / EXEC_ERROR / COMPARE_ERROR)から導出する。EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED / COMPLETED は終端 |
