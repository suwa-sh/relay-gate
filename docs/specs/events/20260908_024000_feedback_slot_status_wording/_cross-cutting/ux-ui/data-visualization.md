...
| 8 | `requested_at` | ローカル ISO 8601(指示子なし) | rapid_crosscheck_request |
| 9 | `completed_at` | ローカル ISO 8601(指示子なし)または `-` | rapid_crosscheck_request |
...
| 6 | `compared_at` | ローカル ISO 8601(指示子なし) | 比較日時 |
...
### 2. 監視記録の警告傾向(`hang-detect-trend.sh [--job-id JOB_ID] [--role ROLE] [--since ローカル日時] [--limit N]`)

`monitor_records`(RAPID_CROSSCHECK_MODE が off 以外。`monitor_status` は 6 値 NOT_MONITORED / MONITORING / HANG_SUSPECTED_NOTIFIED / EXEC_ERROR_NOTIFIED / COMPARE_ERROR_NOTIFIED / COMPLETED)を job_id × role で集計する。off では実行ログにのみ残るため、`error: management db is not configured (RAPID_CROSSCHECK_MODE=off)`、終了コード 3(grep 手順は運用ガイドに記載する)。
...
| 5 | `completed_after_alert_count` | integer | 通知後に正常終了した run 数(遷移 HANG_SUSPECTED_NOTIFIED → COMPLETED。中止済み(`aborted.txt`)による終端も含む。調整根拠になる件数) |
...
- 集計期間の既定は `--since` 未指定で 3 ヶ月前(NFR C.6.1.1 のログ保管期間と揃える)。`--since` の入力形式はローカル ISO 8601 秒精度・指示子なし(`2026-06-01T00:00:00`。表示と同じ時刻軸。`monitor_records.judged_at` と直接比較する。`Z` / オフセット付きは形式不正で終了コード 2)
...
| 6 | `run_status` | `STARTED` / `RUNNING` / `COMPLETED` / `ABORTED` | parallel_run の状態(リラン由来 run は background slot または速報比較依頼の終端で COMPLETED に到達する) |
| 7 | `requested_at` | ローカル ISO 8601(指示子なし) | parallel_run の requested_at |
| 8 | `completed_at` | ローカル ISO 8601(指示子なし)または `-` | |
...
- run_id 形式が時系列ソート可能(`{ローカル yyyymmddThhmmss}-...`。`ui-design.md`「出力フォーマット」。利用者決定 2026-09-05)なため、`run_id` 列だけでも時系列が読める(時刻部も `requested_at` 列もホストのローカルタイムゾーンで、run_id の時刻部と同じ時刻軸なので突き合わせに変換は不要。利用者決定 2026-09-07)
...
- **全体サマリー**: 日次メールサマリー(arch CTP-010)は本規約の範囲外。採用する場合は件名 `[relay-gate][info] daily-summary date={business_date}`、本文は本ファイル 2. の警告傾向 TSV(`hang-detect-trend.sh --since {前日 00:00:00(ローカル)}` の出力)をそのまま貼る形を推奨する(仕様は未確定。todo。`ui-design.md`「通知メール規約」と同文)
