...
facade が foreground slot の終了を待ったのち、その `stdout.log` を標準出力へ、`stderr.log` を標準エラーへ、`exitcode.txt` の値を終了コードとして、装飾・プレフィックス・追記を一切付けずにジョブスケジューラへ中継する。background slot の完了や速報クロスチェックの結果は応答に含めず待機もしない。中継が完了したら `RAPID_CROSSCHECK_MODE` が off 以外(foreground / background)のときだけ parallel_run を RUNNING → COMPLETED にする(facade が起動した run のみ。foreground slot を持たないリラン由来 run の COMPLETED は slot runner / 速報クロスチェック worker が担う)。
...
graph LR
  ...
  R1 -->|"ファイル読み込み"| ART
  G -->|"SQL UPDATE(off 以外のみ)"| PR
  P -->|"無加工中継"| JOB
```

| レイヤー | データモデル | 変換内容 |
|---------|------------|---------|
| usecase | RelayForegroundResultCommand | foreground の `wait` 完了 → 3 ファイル完備確認 → presentation へ中継依頼 → (off 以外) COMPLETED 更新 |
...
| 速報クロスチェックモード | foreground | background と同じ。中継完了で parallel_runs を COMPLETED | tier-facade | repository `parallel_run_repo` |
| 速報クロスチェックモード | background | 中継完了で parallel_runs を COMPLETED | tier-facade | repository `parallel_run_repo` |
| 速報クロスチェックモード | off | 管理 DB に触れない(parallel_run は存在しない) | tier-facade | usecase `relay_foreground_result` |
...
| 速報クロスチェック有効判定 | RAPID_CROSSCHECK_MODE が foreground / background のときだけ parallel_runs を更新。off では管理 DB に触れない | tier-facade | usecase `relay_foreground_result` | 中継完了で parallel_run を COMPLETED にする(SPEC-011-01) |
...
|-----------|--------|--------|---------|---------|---------|----------|
| 並行稼働実行 | RUNNING | COMPLETED | foreground の 3 ファイル中継完了 | RAPID_CROSSCHECK_MODE が off 以外、parallel_runs.status=RUNNING、facade が起動した run(parent_run_id=null) | completed_at を設定。background / 速報の完了は待たない。リラン由来 run(foreground slot なし)の同遷移は UC「実装スクリプトを実行して Runner Result を出力する」/「比較ツールでジョブ単位比較を実行して結果を登録する」が担う | tier-facade |
...
| 情報 | ジョブスケジューラ応答 | 中継先(属性: run_id / 標準出力 / 標準エラー / 終了コード。応答は時刻を持たず、実行日時の履歴はジョブスケジューラの責務) |
| 情報 | 並行稼働実行(parallel_run) | COMPLETED 更新 |
| 情報 | slot 実行 | foreground slot の特定 |
| 条件 | ジョブスケジューラ応答の決定 / 速報結果の位置付け / CLI とメールによる提示 / 速報クロスチェック有効判定 / Runner Result 完備条件 | 分岐条件一覧を参照 |
| バリエーション | slot 実行モード / Runner Result 成果物種別 / 速報クロスチェックモード(foreground / background / off)/ ジョブスケジューラ起動ジョブ種別 | バリエーション一覧を参照 |
| 画面 | facade 応答出力(→ CLI 出力) | stdout / stderr / 終了コード |
| イベント | foreground 結果の無加工中継 | 応答 |
| 外部システム | ジョブスケジューラ | 応答先 |
| 内部データストア | ジョブキュー兼管理 DB(RDB) | RAPID_CROSSCHECK_MODE が off 以外のときの更新先(relay-gate 内部の構成要素) |
| 状態 | 並行稼働実行 | 状態遷移一覧を参照 |
...
|---|---|---|
| REQ-002 | SPEC-002-02 | foreground の 3 ファイルをそのまま中継する(SPEC-002-02) / 速報クロスチェックが失敗しても応答は変わらない(SPEC-005-05) |
| REQ-005 | SPEC-005-05 | 速報クロスチェックが失敗しても応答は変わらない(SPEC-005-05) |
| REQ-011 | SPEC-011-01 | 中継完了で parallel_run を COMPLETED にする(SPEC-011-01) |
| REQ-012 | SPEC-012-01 | foreground の 3 ファイルをそのまま中継する(SPEC-002-02) |
...
Feature: foreground slot の結果をジョブスケジューラへ中継する
  ...
  Scenario: 速報クロスチェックが失敗しても応答は変わらない(SPEC-005-05)
    Given BLUE_MODE=foreground GREEN_MODE=background RAPID_CROSSCHECK_MODE=background で blue は終了コード 0、green は終了コード 1 で終了する
    When ジョブスケジューラが facade.sh JOB001 を実行する
  ...
  Scenario: 中継完了で parallel_run を COMPLETED にする(SPEC-011-01)
    Given RAPID_CROSSCHECK_MODE=background で parallel_runs の run_id が RUNNING である
...
```gherkin
  Scenario: foreground の 3 ファイルが揃わない場合は終了コード 6
    ...
    And RAPID_CROSSCHECK_MODE=background の parallel_runs は RUNNING のままである
```
