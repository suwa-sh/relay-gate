...
| 並行稼働実行の状態(STARTED / RUNNING / COMPLETED / ABORTED) | relay-gate 内部のジョブキュー兼管理 DB `parallel_runs`(RAPID_CROSSCHECK_MODE が off 以外のとき)、`run-lineage.sh --run-id` |
...
|---|---|---|---|
| 並行稼働(blue foreground / green background / 速報 background) | blue | green、速報 | 従来の現行実装の結果と同じ |
| 新実装の単独本番(blue off / green foreground / 速報 off) | green | なし | 新実装の結果。管理 DB 不要 |
| 次世代実装との並行稼働(blue background / green foreground / 速報 background) | green | blue、速報 | 新実装の結果 |
...
| 2 | relay-gate の設定・引数検証エラー | 実装は起動していない(foreground slot がちょうど 1 つでない / feature flag の必須キー欠落 / runner 実体なし等)。stderr の `error:` を読む(UC「slot 実行モードを選択して runner を起動する」) |
