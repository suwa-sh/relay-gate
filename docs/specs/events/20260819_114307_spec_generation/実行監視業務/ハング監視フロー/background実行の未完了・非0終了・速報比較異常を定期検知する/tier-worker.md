...
- **処理フロー**:
  1. STARTING・RUNNING・UNKNOWN状態かつrole_type='background'の起動試行（run_id, slot_type, role_type, attempt_id）を全件取得する（foreground/rapid-crosscheckは対象外とする）。UNKNOWNの起動試行は実結果ファイルの回収を再試行し、回収できた場合のみSUCCEEDED/FAILEDへ確定する
