...
- **入力**: `rapid_crosscheck_requests`（status='REQUESTED'の行、およびlease失効判定対象のCLAIMED行）、`runner_results`（依頼のblue_run_id/blue_attempt_id/green_run_id/green_attempt_idで特定した起動試行の実行結果）、`comparison_definitions`（依頼のjob_id・comparison_definition_valid_fromで特定した比較定義世代）
...
- **処理フロー**:
  ...
  6. gateway層が依頼の (job_id, comparison_definition_valid_from) で `comparison_definitions` から該当世代を1件取得する（target_tables/target_files/comparator_id）
  7. domain層が解決した比較定義のtarget_tables/target_filesを比較範囲としてcomparator_idの比較実装でstdout/stderr/exit_codeを比較し差分件数・OK/NG判定を算出する
  8. usecase層が速報比較依頼status（SUCCEEDED/FAILED）を確定しgateway層へ永続化を依頼する
  9. gateway層が `rapid_crosscheck_requests` を更新し `rapid_crosscheck_results` へINSERTする
  10. presentation層が処理結果を構造化ログへ出力する
- **エラーハンドリング**:
...
| comparison_definition_valid_from | DATETIME | 依頼時点で解決済みの比較定義世代のvalid_from。(job_id, comparison_definition_valid_from) で comparison_definitions の世代を1件に固定する | 参照のみ |
...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| job_id / valid_from | VARCHAR / DATETIME | 比較定義世代のidentity（複合PK）。依頼の (job_id, comparison_definition_valid_from) で1件に特定する | 参照のみ |
| valid_to | DATETIME | 有効期間の終了日時（現行世代はNULL）。世代の解決自体は依頼作成時に完了しており、本UCでは参照情報 | 参照のみ |
| target_tables / target_files / comparator_id | TEXT / TEXT / VARCHAR | この世代の比較対象テーブル・比較対象ファイル・比較実装識別子。比較範囲と比較実装の決定に使用する | 参照のみ |
...
- 比較範囲・比較実装は、依頼が保持する (job_id, comparison_definition_valid_from) で解決した比較定義世代のtarget_tables/target_files/comparator_idに従う。実行時に最新世代を再解決せず、依頼作成時に固定された世代を適用する（SPEC-012-03）
- 比較定義の該当世代を解決できない場合は比較を実行せずエラーとして標準エラーへ記録する（FK制約・部分一意索引・有効期間の排他制約により該当世代は常に1件以下に保証される）
...
Feature: 速報クロスチェックを実行し差分を検知する - バックエンドワーカーティア
  ...
  Scenario: 依頼が保持する比較定義世代を1件解決して比較に適用する
    Given comparison_definitions に現行世代 (job_id="daily-settlement", valid_from="2026-08-01T00:00:00+09:00", valid_to=NULL, target_tables="settlement_summary,settlement_detail", target_files="settlement-report.csv", comparator_id="comparator-settlement-v2") の行が存在する
    ...
    When worker presentation層がCronJobトリガーを受け付ける
    Then gateway層は comparison_definitions から (job_id="daily-settlement", valid_from="2026-08-01T00:00:00+09:00") の1件だけを取得し、domain層は comparator_id="comparator-settlement-v2" の比較実装で target_tables="settlement_summary,settlement_detail" と target_files="settlement-report.csv" を比較範囲として比較を実行する
```
