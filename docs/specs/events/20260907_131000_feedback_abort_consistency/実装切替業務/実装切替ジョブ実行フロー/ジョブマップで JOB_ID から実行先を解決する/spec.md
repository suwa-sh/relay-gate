...
| CSV 行の分解 | ヘッダー行、データ行 | カンマ区切り。セルは二重引用符で囲める(囲まない場合はカンマ・二重引用符を含められない)。囲んだセル内の `""` は `"` 1 文字。bash 単独の 1 文字ずつの状態機械で解析し、ネスト・複数行セルは不可(`error: csv quote is invalid line=N path: ...`)。行頭 `#` はコメント行、改行コードは LF。ヘッダー列名で位置を決める(列順に依存しない) | ExecutionTarget | tier-facade |
...
| REQ-004 | SPEC-004-04 | CSV の列名で読み替えなしに実行先を解決する / 二重引用符で囲んだ fixed_params セルを 2 引数として解析する / host と user の列が無いジョブマップはローカル実行として解決する / credential_ref と map_version の列が無くても解決できる |
...
```gherkin
  ...
  Scenario: fixed_params が JSON 配列でない行は解決失敗にする
    ...
    Then runner は終了コード 2 で終了し、stderr.log に "error: fixed_params is not a json array line=3 job_id=JOB003 value=p1 p2" が含まれる
