...
| slot 実行の状態導出規則 | slot 実行の状態は成果物ファイルから導出する。exitcode.txt があれば 0 で SUCCEEDED / 非 0 で FAILED、無く aborted.txt があれば ABORTED、どちらも無ければ RUNNING。速報クロスチェック有効時は管理 DB にも同じ状態を保持する(BUC.tsv 上の適用 UC は実行復旧業務 / 実行監視業務。本 BUC では Runner Result を出力する UC が spec で採用) | 実装スクリプトを実行して Runner Result を出力する(spec) |
