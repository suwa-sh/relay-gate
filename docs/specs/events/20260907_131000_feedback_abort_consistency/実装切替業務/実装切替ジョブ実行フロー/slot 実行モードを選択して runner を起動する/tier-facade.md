...
#### 読み込む設定(速報クロスチェック設定 = 管理 DB 接続。RAPID_CROSSCHECK_MODE が off 以外のときのみ)

情報「速報クロスチェック設定」(`$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env`。所有者: 基盤適用設計者(設定所有区分))。属性は管理 DB 接続参照名(RAPID_DB_CONN_REF)/ lease 期間(秒。RAPID_LEASE_SEC)/ worker の poll 間隔(秒。RAPID_POLL_INTERVAL_SEC)の 3 つで、facade が読むのは RAPID_DB_CONN_REF だけ(lease / poll は速報クロスチェック worker が読む。契約 `config_files.rapid-crosscheck.env.readers`)。

| キー | 所在 | 必須 | 検証 |
|---|---|---|---|
| RAPID_DB_CONN_REF(属性「管理 DB 接続参照名」) | `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env` | off 以外のとき Yes | ファイル不在は `error: config file not found path: <path>`、キー欠落は `error: option required option=RAPID_DB_CONN_REF path: <path>`。いずれも終了コード 2(runner を起動しない)。off では読まず、ファイルが存在しなくても slot 実行できる(契約 `config_files.rapid-crosscheck.env`) |

- 値は接続情報の参照名のみ(arch CTP-002)。認証情報の値(パスワード・接続文字列)は速報クロスチェック設定に置かない。facade は参照名から解決した接続情報を実行ログ・stderr に出さない
...
- **バリデーション**(presentation → domain。runner を 1 つも起動する前にすべて実施):
  ...
  2'. 速報クロスチェック設定(RAPID_CROSSCHECK_MODE≠off のときだけ): rapid-crosscheck.env の存在 → RAPID_DB_CONN_REF の存在。off では読まない
...
速報クロスチェック設定(rapid-crosscheck.env。情報「速報クロスチェック設定」)のキー定義は契約 `config_files.rapid-crosscheck.env` を正本とし、この UC は RAPID_DB_CONN_REF を読み取り専用で従う(RAPID_LEASE_SEC / RAPID_POLL_INTERVAL_SEC は UC「速報比較依頼を claim する」が定義)。
...
| requested_at | datetime | 発行日時(ローカル時刻) | 追加 |
| completed_at | datetime(null 可) | 中継完了日時 | 追加 |

### slot_executions(情報: slot 実行。off 以外のみ。INSERT 順序は仮採用: _inference.md #7。status は runner / abort-* の条件付き UPDATE(WHERE status='RUNNING')で一度だけ終端値を書く列。abort 後に exitcode.txt を公開した経路だけファイル正本(条件「slot 実行の状態導出規則」)の導出値と一致せず ABORTED のまま残る。正本: rdb-schema.yaml slot_executions.status)
...
| completed_at | datetime(null 可) | SUCCEEDED / FAILED / ABORTED へ遷移した日時(ローカル時刻) | 追加 |
...
Feature: slot 実行モードを選択して runner を起動する - facade / slot runner ティア
  ...
  Scenario: facade_sh は速報クロスチェック設定の RAPID_DB_CONN_REF を参照名としてだけ使い認証情報の値を出さない(SPEC-005-06)
    Given feature-flag.env に BLUE_MODE=foreground GREEN_MODE=off RAPID_CROSSCHECK_MODE=background があり管理 DB が起動している
    And RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env に RAPID_DB_CONN_REF=mgmt-db RAPID_LEASE_SEC=600 RAPID_POLL_INTERVAL_SEC=30 だけがあり、パスワード・接続文字列の値は無い
    And RDB クライアント CLI のパスに引数と環境変数を記録して実 DB へ転送するラッパーを置く
    When `facade.sh JOB001` を実行する
    Then parallel_runs に run_id の行が status=RUNNING で 1 件ある
    And RDB クライアントラッパーの記録には参照名 mgmt-db があり、実行ログと stderr には参照名から解決した接続情報の値が無い

  Scenario: facade_sh は RAPID_CROSSCHECK_MODE=off で RDB クライアントを呼ばず rapid-crosscheck.env も読まない(SPEC-005-06)
    ...
    And blue と green のスタブ runner の起動記録がある
