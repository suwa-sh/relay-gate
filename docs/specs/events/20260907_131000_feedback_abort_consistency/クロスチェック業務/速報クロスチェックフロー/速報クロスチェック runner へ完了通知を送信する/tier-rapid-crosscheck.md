...
- **アクセス権**: slot runner からの内部呼び出し(RAPID_CROSSCHECK_MODE が off 以外のとき。slot runner は feature flag の `RAPID_CROSSCHECK_RUNNER` が指す本コマンドの実体を起動する)。運用者は直接起動しない(通知失敗の復旧で同一引数を再実行する場合を除く)
...
| `--job-id` | string | Yes | — | JOB_ID(英数字・`_`・`-`)。同じ run_id の parallel_runs.job_id と不一致なら `warn: job_id mismatch run_id=... notified=... recorded=...` を出して処理は継続 |
...
- **stdin**: なし
- **設定ファイル**: `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env`(情報「速報クロスチェック設定」。所有者は基盤適用設計者(設定所有区分)。契約 `cli-command-contract.yaml` の config_files が正。キー定義は UC「速報比較依頼を claim する」tier-rapid-crosscheck.md「設定ファイル」)
  | キー | 必須 | 説明 |
  |---|---|---|
  | `RAPID_DB_CONN_REF` | Yes | 速報管理 DB 接続情報の参照名(情報「速報クロスチェック設定」の属性「管理 DB 接続参照名」。値は置かず参照名のみ)。本コマンドは RAPID_CROSSCHECK_MODE が off 以外のときだけ起動されるため常に必須。`RAPID_LEASE_SEC` / `RAPID_POLL_INTERVAL_SEC` は本コマンドでは使わない |
...
- **stderr**: `error: option required option=--run-id` / `error: invalid exit_code value=abc` / `error: unknown subcommand subcommand=foo` / `error: config file not found path: ...`(2)/ `error: option required option=RAPID_DB_CONN_REF path: ...`(2)/ `error: rapid run not found run_id=...` / `error: management db connection failed run_id=... conn_ref=...`(6)/ `error: management db update failed run_id=... role=...` / `warn: job_id mismatch run_id=... notified=... recorded=...`
- **終了コード**:
  | コード | 意味 | 条件 |
  |-------|------|------|
...
| blue_completed_at | datetime | 通知受信時刻(ホストのローカルタイムゾーン。タイムゾーン指示子なし。RDB のタイムスタンプ型に委ねる) | 追加 |
| green_completed_at | datetime | 同上 | 追加 |
| completion_status | string | PENDING / ONE_COMPLETED / BOTH_SUCCEEDED / ANY_FAILED / REQUEST_CREATED(次 UC で更新) | 追加 |

## ビジネスルール

- 完了通知の系統独立: サブコマンドが示す系統の列だけを更新する。通知元 slot の aborted.txt の有無は受信側でも参照しない(中止済み run の扱いは dispatcher が管理 DB の parallel_runs.status と対象 slot の slot_executions.status(aborted.txt のミラー)で判断する。判定キー = 並行稼働実行 ABORTED または対象 slot の slot 実行 ABORTED。UC「両系成功時に速報比較依頼を作成する」)
- Runner Result 完備条件: exit_code 0 → SUCCEEDED、非 0 → FAILED
- 速報クロスチェック有効判定: 本コマンドは RAPID_CROSSCHECK_MODE が off 以外(foreground / background)のときだけ起動される前提。自身は feature flag を読まない
- 完了通知失敗の扱い: 本コマンドの非 0 終了は送信側で自動検知されない。運用者の同一引数再実行に対して先勝ちの冪等で応じる
- 速報と確報のモデル分離
...
Feature: 速報クロスチェック runner へ完了通知を送信する - 速報クロスチェックティア
  ...
  Scenario: blue-completed で blue 列を更新する
    Given rapid_runs に run_id=20260830T113000-JOB001-3f9a1c2e, blue_status=NULL の行がある
    When `rapid-crosscheck-runner.sh blue-completed --run-id 20260830T113000-JOB001-3f9a1c2e --job-id JOB001 --exit-code 0 --artifact-uri file:///var/relay-gate/facade/20260830T113000-JOB001-3f9a1c2e/blue` を実行する
  ...
  Scenario: exit_code が非 0 なら FAILED として登録する
    Given rapid_runs に run_id=20260830T113000-JOB001-3f9a1c2e, green_status=NULL の行がある
  ...
  Scenario: 必須オプションが欠落している
    When `rapid-crosscheck-runner.sh blue-completed --run-id 20260830T113000-JOB001-3f9a1c2e --job-id JOB001` を実行する
  ...
  Scenario: rapid_runs に行が無い
    Given rapid_runs に run_id=20260830T113000-JOB002-aaaaaaaa の行が無い
    When `rapid-crosscheck-runner.sh blue-completed --run-id 20260830T113000-JOB002-aaaaaaaa --job-id JOB002 --exit-code 0 --artifact-uri file:///var/relay-gate/facade/20260830T113000-JOB002-aaaaaaaa/blue` を実行する
    Then 終了コード 3 で stderr に `error: rapid run not found run_id=20260830T113000-JOB002-aaaaaaaa` が出る

  Scenario: 再通知は既存値を保持して 0 で終了する(先勝ち)
    Given rapid_runs に run_id=20260830T113000-JOB001-3f9a1c2e, blue_status=SUCCEEDED, blue_completed_at=2026-08-30T11:46:00 の行がある
    When `rapid-crosscheck-runner.sh blue-completed --run-id 20260830T113000-JOB001-3f9a1c2e --job-id JOB001 --exit-code 3 --artifact-uri file:///var/relay-gate/facade/20260830T113000-JOB001-3f9a1c2e/blue` を実行する
    Then 終了コード 0 で stderr は 0 行、rapid_runs.blue_status は SUCCEEDED、blue_completed_at は 2026-08-30T11:46:00 のままである

  Scenario: 速報クロスチェック設定の参照名で管理 DB に接続する(SPEC-005-06)
    Given $RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env に `RAPID_DB_CONN_REF=relaygate-db` の行があり、パスワード等の認証情報の値は含まれない
    ...
    Then gateway は参照名 `relaygate-db` から解決した接続情報で管理 DB に接続し、終了コード 0 で rapid_runs.green_status は SUCCEEDED になる
```
