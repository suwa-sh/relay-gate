...
| 名前 | 型 | 必須 | 既定値 | 説明 |
|------|---|------|-------|------|
| `--run-id` | string | Yes | — | 参照する run_id(`{ローカル yyyymmddThhmmss}-{job_id}-{8 hex}`) |
...
- **バリデーション**: `--run-id` は先頭 15 文字が `yyyymmddThhmmss`(ローカルタイムゾーンの時刻。`Z` などのタイムゾーン指示子は付かない)、末尾 8 文字が hex、中間が `-{job_id}-`。`--limit` は 1 以上の整数
- **確認プロンプト**: なし(参照のみ)
...
- **管理 DB 接続先**: `$RELAY_GATE_CONFIG_DIR/rapid-crosscheck.env`(情報「速報クロスチェック設定」)の `RAPID_DB_CONN_REF`(属性「管理 DB 接続参照名」。UC「速報比較依頼を claim する」tier-rapid-crosscheck.md の設定ファイル)。`RAPID_CROSSCHECK_MODE` が off 以外(foreground / background)で欠落なら終了コード 2
...
| requested_at / completed_at | datetime | ホストのローカルタイムゾーン(タイムゾーン指示子なし)。stdout にもそのまま出す | 追加(参照) |
...
| compared_at | datetime | ホストのローカルタイムゾーン(タイムゾーン指示子なし)。TSV 列にそのまま出す | 追加(参照) |
...
Feature: 速報比較結果を参照する - 速報クロスチェックティア
  ...
  Scenario: 依頼と比較結果を run_id で出力する
    Given rapid_crosscheck_requests に run_id=20260830T113000-JOB001-3f9a1c2e, job_id=JOB001, status=SUCCEEDED, exit_code=0 の行がある
    And comparison_results に同 run_id の行が 1 件ある
    When `rapid-crosscheck-result.sh --run-id 20260830T113000-JOB001-3f9a1c2e` を実行する
  ...
  Scenario: --limit で出力を打ち切る
    ...
    When `rapid-crosscheck-result.sh --run-id 20260830T113000-JOB001-3f9a1c2e --limit 2` を実行する
  ...
  Scenario: 未知のオプション
    When `rapid-crosscheck-result.sh --run-id 20260830T113000-JOB001-3f9a1c2e --json` を実行する
