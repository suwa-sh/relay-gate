...
| slot_executions | slot 実行 | run_id ごとの blue / green 各 slot の実行(mode・PID・成果物ディレクトリ・状態)を識別するレコード(arch E-014)。RAPID_CROSSCHECK_MODE が off 以外のときのみ RDB に永続化し、off ではファイルのみ(採用値 #7)。status は runner / abort-* の条件付き UPDATE(WHERE status='RUNNING')で一度だけ終端値を書く列(arch storage_mapping E-014 rdb)。abort 後に実装が走り切って exitcode.txt を公開した経路だけファイル正本の導出値(exitcode.txt 優先)と一致せず ABORTED のまま残る(再同期しない。詳細は columns.status)。ハング検知・中止・background 側リランの状態判定はファイル正本で行い、本テーブルは off 以外での参照(dispatcher の中止済み run 判定・系譜の role 導出)に使う | 9 | 0 | 8 |
...
**RDRA 情報**: slot 実行
**説明**: run_id ごとの blue / green 各 slot の実行(mode・PID・成果物ディレクトリ・状態)を識別するレコード(arch E-014)。RAPID_CROSSCHECK_MODE が off 以外のときのみ RDB に永続化し、off ではファイルのみ(採用値 #7)。status は runner / abort-* の条件付き UPDATE(WHERE status='RUNNING')で一度だけ終端値を書く列(arch storage_mapping E-014 rdb)。abort 後に実装が走り切って exitcode.txt を公開した経路だけファイル正本の導出値(exitcode.txt 優先)と一致せず ABORTED のまま残る(再同期しない。詳細は columns.status)。ハング検知・中止・background 側リランの状態判定はファイル正本で行い、本テーブルは off 以外での参照(dispatcher の中止済み run 判定・系譜の role 導出)に使う
...
| status | string | NO | slot 実行の状態。値: RUNNING(起動後), SUCCEEDED(exitcode.txt が 0), FAILED(exitcode.txt が非 0), ABORTED(運用者の明示中止。abort-blue / abort-green が aborted.txt を公開した後に条件付き UPDATE で設定)。条件「slot 実行の状態導出規則」: exitcode.txt があれば 0 = SUCCEEDED / 非 0 = FAILED、無く aborted.txt があれば ABORTED、どちらも無ければ RUNNING(両方あるときは exitcode.txt を優先)。本列は runner / abort-* の条件付き UPDATE(WHERE status='RUNNING')で一度だけ終端値を書く。abort-blue / abort-green が ABORTED にした後に実装が走り切って exitcode.txt を公開した場合、ファイル正本の導出値は exitcode.txt 優先で SUCCEEDED / FAILED になるが、本列は runner の終端 UPDATE が 0 件のため ABORTED のまま残す(導出値へ再同期しない)。この ABORTED は aborted.txt 公開済みのミラーとして dispatcher(rapid-crosscheck-runner.sh)の条件「中止済み run の比較依頼作成除外」の判定キーに使う(USDM SPEC-005-02 AC6)。ファイル正本と一致しないのはこの経路だけで、slot 実行の状態判定(abort / rerun / hang-detector)は引き続きファイル正本で行う |
