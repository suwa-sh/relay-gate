...
| slot 実行 `status` | `RUNNING` / `SUCCEEDED` / `FAILED` / `ABORTED` | 実行中 / 成功 / 失敗 / 中止 | 成果物ファイルが正本(条件「slot 実行の状態導出規則」): `exitcode.txt` があれば 0=SUCCEEDED / 非 0=FAILED、無く `aborted.txt` があれば ABORTED、どちらも無ければ RUNNING。両方あるときは `exitcode.txt` を優先する。off 以外では管理 DB `slot_executions.status` に条件付き UPDATE(WHERE status='RUNNING')で一度だけ終端値を書く(abort-blue / abort-green が ABORTED にした後に実装が走り切って `exitcode.txt` を公開した経路だけ、導出値と異なり ABORTED のまま残る。再同期しない。正本: `rdb-schema.yaml` slot_executions.status) |
