...
| REQ-005 | SPEC-005-04 | 単独本番モードで業務ジョブを実行する(off。管理 DB に触れない)/ 並行稼働モードで業務ジョブを実行する(background。完了通知と速報比較。foreground の AC は同じ挙動として UC〈slot 実行モードを選択して runner を起動する〉で覆う) |
