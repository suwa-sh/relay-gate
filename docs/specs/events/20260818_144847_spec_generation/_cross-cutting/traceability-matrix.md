...
生成対象: `docs/specs/events/20260818_144847_spec_generation/`
...
| ファイル | 分母 | 網羅率 | 用途 |
|---|---|:---:|---|
| 本ファイル | RDRA 要素 85 件 | 100% | RDRA モデルの要素が Spec でカバーされているかを確認する |
| [usdm-acceptance-matrix.md](usdm-acceptance-matrix.md) | USDM acceptance criteria 43 件 | 97.7%（42/43） | 受け入れ条件から UC Scenario / tier Scenario を逆引きする |
| [uc-dependencies.md](uc-dependencies.md) | UC 間依存 30 件 | — | UC が前提とする他 UC の状態・レコード・成果物を確認する |

> USDM acceptance criteria 側に残る未カバー 1 件（SPEC-012-03: job_id ごとの比較定義差し替え）は、対応する情報エンティティが RDRA に存在しないため Spec 側で解消できない。[rdra-feedback.md](rdra-feedback.md) に変更要望として記録済み。
