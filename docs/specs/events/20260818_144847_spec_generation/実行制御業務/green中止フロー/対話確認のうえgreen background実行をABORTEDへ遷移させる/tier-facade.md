...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| status | string | RUNNING → ABORTED へ更新（実行状態は6値: STARTING/RUNNING/SUCCEEDED/FAILED/UNKNOWN/ABORTED） | 変更（UPDATE。WHERE (run_id, slot_type='green', role_type='background', attempt_id)。runner_result_eventsのINSERTと同一transaction） |
