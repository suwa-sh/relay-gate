...
| job_map_version | string | ジョブマップ版 | 追加（元の値を複製） |
...
- 元のrun共通execution spec（job_id/additional_args/job_map_version/hang_detect_limit_minutes）とslot別実行設定（host/exec_user/script_path/work_dir/fixed_args/impl_version/credential_ref）を一切変更せず新規run_idへ複製する（「実行設定を保ったまま再実行する」というUC名の制約）。認証情報は参照名のみを複製し、実値は取り扱わない
...
Feature: execution-spec.jsonの実行設定を保ったまま再実行する - tier-facade
  ...
  Scenario: rerun_run_facadeが元の実行設定を保ったまま新規run_idでbackground roleを再起動すること
    ...
    And slot_execution_specsテーブルに新規run_idのslot_type="blue"でhost="blue-host-01", script_path="/opt/blue/run.sh", impl_version="blue-2.3.1"の行が追加される
