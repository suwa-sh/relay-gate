...
RUNNING中の確報比較依頼（E-005, AG-004）に対する中止依頼を受理するCLIコマンドをtier-workerに追加する。確報クロスチェックは全テーブル・全ファイルを対象とする日次バッチでリリース判断の正本となるため、本UCは状態遷移を発生させず対象の状態検証のみを行う。実際のABORTEDへの遷移は次UC「対話確認のうえ確報比較依頼をABORTEDへ遷移させる」が担う。
...
- **環境変数**:
  ...
  | 変数名 | 必須 | 説明 |
  |--------|------|------|
  | RELAYGATE_RDB_DSN | Yes | 確報比較依頼テーブル（final_crosscheck_requests）へ接続するRDB接続文字列 |
...
| カラム | 型 | 説明 | 変更種別 |
|--------|---|------|---------|
| run_id | uuid | 確報比較依頼の識別子（PK） | 変更なし（SELECT対象） |
...
Feature: RUNNING中の確報比較依頼の中止を依頼する - tier-worker
  ...
  Scenario: RUNNING中の確報比較依頼の中止依頼をworkerが受理する
    Given final_crosscheck_requestsテーブルにrun_id="fc-2026-0817-001", target_date="2026-08-17", status="RUNNING", worker_id="worker-05"のレコードが存在する
    When relaygate abort final-crosscheck request --run-id fc-2026-0817-001 を実行する
    Then 標準出力に "中止依頼を受理しました run_id=fc-2026-0817-001 target_date=2026-08-17 status=RUNNING" が出力され終了コード0で終了する

  Scenario: RUNNING以外の状態では中止依頼を拒否する
    Given final_crosscheck_requestsテーブルにrun_id="fc-2026-0816-001", status="CLAIMED"のレコードが存在する
    When relaygate abort final-crosscheck request --run-id fc-2026-0816-001 を実行する
    Then 標準エラーに "中止依頼できません run_id=fc-2026-0816-001 status=CLAIMED（RUNNING状態のみ中止依頼可能です）" が出力され終了コード1で終了する
```
