...
graph LR
  ...
  WK_Pres -->|"relaygate final-crosscheck result --target-date 2026-08-17"| WK_UC
...
sequenceDiagram
  ...
  User->>Pres: relaygate final-crosscheck result --target-date 2026-08-17
  Pres->>Pres: 入力バリデーション（target_date形式チェック）
  Pres->>UC: GetFinalCrossCheckResultQuery(target_date="2026-08-17")
  UC->>GW: 確報比較依頼取得
  GW->>DB: SELECT * FROM final_crosscheck_requests WHERE target_date = '2026-08-17'
  DB-->>GW: run_id="run-20260817-901", status="SUCCEEDED"
  GW-->>UC: 確報比較依頼ドメインモデル
  UC->>Domain: 状態表示ラベル解決（SUCCEEDED→正常終了）
  Domain-->>UC: 表示用状態
  UC-->>Pres: 結果
  Pres-->>User: stdout: run_id=run-20260817-901 target_date=2026-08-17 status=SUCCEEDED, exit_code=0
```
...
Feature: 確報クロスチェック結果を確認する
  ...
  Scenario: 対象日の確報比較依頼がSUCCEEDEDの場合に結果を確認できる
    Given 対象日 "2026-08-17" の確報比較依頼 run_id "run-20260817-901" が status "SUCCEEDED" で存在する
    When リリース判断者が `relaygate final-crosscheck result --target-date 2026-08-17` を実行する
    Then 標準出力に run_id "run-20260817-901" と status "SUCCEEDED" が表示され、終了コード 0 で終了する

  Scenario: 対象日の確報比較依頼がFAILEDの場合に異常終了として結果を確認できる
    Given 対象日 "2026-08-17" の確報比較依頼 run_id "run-20260817-901" が status "FAILED" で存在する
    When リリース判断者が `relaygate final-crosscheck result --target-date 2026-08-17` を実行する
    Then 標準出力に run_id "run-20260817-901" と status "FAILED" が表示され、終了コード 0 で終了する
```
