...
Feature: 確報クロスチェック結果をstdout/stderr/exitcodeで応答する - tier-worker
  ...
  Scenario: presentation層がSUCCEEDEDのstatusをexitcode 0に変換して応答する
    Given final_crosscheck_requests に run_id "run-20260817-901" status "SUCCEEDED" のレコードが存在する
    When tier-workerのpresentation層が RespondFinalCrossCheckResultCommand(run_id="run-20260817-901") を受け取る
  ...
  Scenario: presentation層がstatus未確定の場合にエラー応答する
    Given final_crosscheck_requests に run_id "run-20260817-903" status "RUNNING" のレコードが存在する
    When tier-workerのpresentation層が RespondFinalCrossCheckResultCommand(run_id="run-20260817-903") を受け取る
