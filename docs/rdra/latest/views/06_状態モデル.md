...
stateDiagram-v2
  ...
  s1 --> s3 : 実行を ABORTED へ遷移させる
  s2 --> s1 : 速報比較依頼を claim する
  s2 --> s1 : 確報比較依頼を claim する
  s2 --> s4 : 比較ツールでジョブ単位比較を実行して結果を登録する
  s2 --> s4 : 比較ツールで日次全量比較を実行して結果を保存する
  s2 --> s3 : 実行を ABORTED へ遷移させる
  s4 --> s5 : 比較ツールでジョブ単位比較を実行して結果を登録する
  s4 --> s6 : 比較ツールでジョブ単位比較を実行して結果を登録する
  s4 --> s5 : 比較ツールで日次全量比較を実行して結果を保存する
  s4 --> s6 : 比較ツールで日次全量比較を実行して結果を保存する
  s4 --> s3 : 実行を ABORTED へ遷移させる
  s5 --> [*]
  s6 --> [*]
  s3 --> [*]
```
...
| REQUESTED | 実行を ABORTED へ遷移させる | ABORTED | 両系の完了通知で速報比較依頼が REQUESTED で作成された後に運用者が abort-blue / abort-green で slot を中止した場合の競合窓を塞ぐ保険。yes と答えたとき、対象 run に REQUESTED で未着手(worker が claim していない)の速報比較依頼があればそれも ABORTED にする。dispatcher 側の中止済み run 判定(条件「中止済み run の比較依頼作成除外」)と併用する。運用者が abort-rapid-crosscheck を --run-id 付きで実行して yes と答えた場合も REQUESTED の速報比較依頼を ABORTED にする(条件「依頼中止可否判定」)。速報比較依頼のみの遷移で確報比較依頼には適用しない |
...
| CLAIMED | 比較ツールでジョブ単位比較を実行して結果を登録する | RUNNING | claim した速報クロスチェック worker が依頼を RUNNING にし、クロスチェックジョブマップの job_id ごとの比較定義に従って比較ツールでジョブ単位の比較を開始する。RUNNING への遷移は status = CLAIMED を条件とする条件付き UPDATE で行い、更新件数が 0 件(abort-rapid-crosscheck で ABORTED 済み)なら比較を開始せず終了する |
...
| CLAIMED | 実行を ABORTED へ遷移させる | ABORTED | 運用者が worker のプロセスを停止したことを確認し、abort-rapid-crosscheck を --run-id 付きで実行して yes と答えたとき、CLAIMED の速報比較依頼を status IN (REQUESTED, CLAIMED, RUNNING) の条件付き UPDATE で ABORTED にする。CLAIMED のまま残すと lease 失効で REQUESTED に戻り別の worker が再 claim して比較が実行されるため、中止済み run の依頼を運用者が止める手段として必要。claim 済みの worker は RUNNING への条件付き UPDATE が 0 件になり比較を開始せず終了する(条件「依頼中止可否判定」)。速報比較依頼のみの遷移で確報比較依頼には適用しない |
...
| RUNNING | 実行を ABORTED へ遷移させる | ABORTED | 運用者が worker プロセスの停止を確認し、abort-rapid-crosscheck / abort-final-crosscheck を --run-id 付きで実行して yes と答えたときだけ RUNNING の依頼を明示中止する。ハング検知はこの遷移を行わない。abort-rapid-crosscheck は速報比較依頼について REQUESTED / CLAIMED も中止できる(各遷移を参照)。abort-final-crosscheck は確報比較依頼について RUNNING のときだけ中止できる |
...
| ABORTED |  |  | 明示中止で終端する。速報比較依頼は中止確認済みとして background-rerun の対象になれる。未着手(REQUESTED)の速報比較依頼が abort-blue / abort-green または abort-rapid-crosscheck で中止された場合、claim 済み(CLAIMED)の速報比較依頼が abort-rapid-crosscheck で中止された場合もこの終端に至る |
...
| RUNNING | 実行を ABORTED へ遷移させる | ABORTED | 運用者がプロセスの強制終了を確認し、abort-blue / abort-green を --run-id 付きで実行して yes と答えたときだけ、background かつ RUNNING の slot 実行を明示中止する。abort-blue / abort-green は成果物ディレクトリに aborted.txt(中止日時 1 行)を書く。RAPID_CROSSCHECK_MODE が off 以外なら管理 DB の状態も ABORTED に更新する。スクリプト自身はプロセスを停止しない |
...
stateDiagram-v2
  ...
  s1 --> s3 : 実行を ABORTED へ遷移させる
  s2 --> s4 : foreground slot の結果をジョブスケジューラへ中継する
  s2 --> s4 : 比較ツールでジョブ単位比較を実行して結果を登録する
  s2 --> s4 : 実装スクリプトを実行して Runner Result を出力する
  s2 --> s3 : 実行を ABORTED へ遷移させる
  s4 --> [*]
  s3 --> [*]
```
...
| STARTED | 実行を ABORTED へ遷移させる | ABORTED | facade または background-rerun が parallel_run を STARTED で作成した直後(RUNNING 更新前)に運用者が中止スクリプトで明示中止した場合も ABORTED にし、STARTED のまま残る実行を取りこぼさない |
...
| RUNNING | 比較ツールでジョブ単位比較を実行して結果を登録する | COMPLETED | 速報比較依頼だけを新規作成したリラン由来(background-rerun --role rapid-crosscheck)の parallel_run は foreground slot を持たないため、依頼が終端状態(SUCCEEDED / FAILED)になった時点で速報クロスチェック worker が COMPLETED にする |
| RUNNING | 実装スクリプトを実行して Runner Result を出力する | COMPLETED | 元の execution-spec.json から復元して新しい run_id で起動したリラン由来(background-rerun --role blue / green)の parallel_run は foreground slot を持たないため、background slot の終端(exitcode.txt の公開)時に runner が COMPLETED にする |
...
| 監視中 | background 実行の経過時間と終了状態を判定する | 正常終了 | exitcode.txt が 0 で出力され、上限以内に正常終了したと判定する。監視対象が ABORTED(exitcode.txt が無く aborted.txt がある、または依頼が ABORTED)になった場合も中止済みとして正常終了で終端する |
| ハング疑い通知済み | background 実行の経過時間と終了状態を判定する | 正常終了 | 通知後に exitcode.txt が 0 で出力された(通知後正常終了)。警告時の経過時間を監視記録に残し、hang_detect_limit_minutes のジョブごとの調整に使用する。通知後に監視対象が ABORTED になった場合も中止済みとして正常終了で終端する |
| ハング疑い通知済み | ハング疑い・実行エラー・比較異常を通知する | 実行エラー通知済み | 通知後に exitcode.txt が非 0 で出力され、background 実行エラーとして error メールを追加通知する |
| ハング疑い通知済み | ハング疑い・実行エラー・比較異常を通知する | 比較異常通知済み | 通知後に速報比較依頼が FAILED または比較 NG で終端し、速報クロスチェック異常として error メールを追加通知して監視を終端する。通知済み run を再判定せず監視記録が未終端のまま残る欠陥を防ぐ |
