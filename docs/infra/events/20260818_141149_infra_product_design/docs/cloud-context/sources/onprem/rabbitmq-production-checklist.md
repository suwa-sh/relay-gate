...
RabbitMQ itself does not encrypt data it writes to disk, such as message payloads or [metadata](/docs/metadata-store). This is by design: two alternatives outside RabbitMQ address this need better.
...
- **Use end-to-end encryption.** Client applications that require message contents to stay confidential even from anyone with access to the broker itself, including its administrators, should encrypt payloads before publishing and decrypt them after consuming. The broker then only ever sees and stores opaque encrypted bytes.
