...
const meta: Meta = {
  title: 'Pages/運用ポータル/green background中止確認画面',
  tags: ['autodocs'],
  parameters: {
    layout: 'fullscreen',
  },
  ...
  render: () => (
    <OpsPortalShell activeNavKey="control">
      <AbortConfirmDialog
        ...
        target="green background実行"
        impactSummary="中止依頼を受理済みのgreen background実行をABORTEDへ遷移させます。実行中のジョブは即時停止し、以降の再開・リトライは新規実行として扱われます。"
      />
    </OpsPortalShell>
  ),
}
    ...
    <OpsPortalShell activeNavKey="control">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        ...
        <Banner variant="success" title="ABORTEDへ遷移させました">
          run_id=run-20260817-green-021 をABORTEDへ遷移させました。
        </Banner>
      </div>
    </OpsPortalShell>
  ),
    ...
    <OpsPortalShell activeNavKey="control">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        ...
        <Banner variant="error" title="ABORTEDへの遷移を中断しました">
          n応答によりrun_id=run-20260817-green-021の状態は変更されていません（exit code 1）。
        </Banner>
      </div>
    </OpsPortalShell>
  ),
