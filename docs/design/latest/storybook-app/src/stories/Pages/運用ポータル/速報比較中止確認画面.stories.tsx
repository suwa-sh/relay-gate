  ...
  render: () => (
    <OpsPortalShell activeNavKey="control">
      <AbortConfirmDialog
        ...
        target="速報比較依頼"
        impactSummary="RUNNING中の速報比較依頼をABORTEDへ遷移させます。比較処理は即時停止し、以降の再開・リトライは新規依頼として扱われます。"
      />
    </OpsPortalShell>
  ),
}
    ...
    <OpsPortalShell activeNavKey="control">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        <AbortConfirmDialog
          ...
          impactSummary="RUNNING中の速報比較依頼をABORTEDへ遷移させます。比較処理は即時停止し、以降の再開・リトライは新規依頼として扱われます。"
        />
        <Banner variant="success" title="速報比較依頼をABORTEDへ遷移させました">
          run_id=run-20260817-rapid-033
        </Banner>
      </div>
    </OpsPortalShell>
  ),
    ...
    <OpsPortalShell activeNavKey="control">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
        ...
        <Banner variant="error" title="中止をキャンセルしました">
          n応答によりrun_id=run-20260817-rapid-033の状態は変更されていません（exit code 1）。
        </Banner>
      </div>
    </OpsPortalShell>
  ),
